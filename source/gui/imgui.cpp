 ...
 *
 * Based on dear imgui Platform Backend for GLFW sample.
 *******************************************************************************
 */
...
#ifdef GLFW_RESIZE_NESW_CURSOR // Let's be nice to people who pulled GLFW between
                               // 2019-04-16 (3.4 define) and 2019-11-29 (cursors defines)
                               // // FIXME: Remove when GLFW 3.4 is released?
...
// backend.
// - Because glfwPollEvents() process all windows and some events may be called outside of
// it, you will need to register your own callbacks
//   (passing install_callbacks=false in ImGui_ImplGlfw_InitXXX functions), set the
//   current dear imgui context and then call our callbacks.
// - Otherwise we may need to store a GLFWWindow* -> ImGuiContext* map and handle this in
// the backend, adding a little bit of extra complexity to it.
...
void
ImGui_ImplGlfw_KeyCallback(GLFWwindow* window,
  int key,
  int scancode,
  int action,
  int mods)
...
{
    // Unused in 'master' branch but 'docking' branch will use this, so we declare it
    // ahead of it so if you have to install callbacks you can install this one too.
}
...
ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    // Keyboard mapping. Dear ImGui will use those indices to peek into the io.KeysDown[]
    // array.
    ...
    // When a cursor doesn't exist, GLFW will emit an error which will often be printed by
    // the app, so we temporarily disable error reporting. Missing cursors will return
    // NULL and our _UpdateMouseCursor() function will use the Arrow cursor instead.)
...
ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) {
        io.MouseDown[i] =
          bd->MouseJustPressed[i] || glfwGetMouseButton(bd->Window, i) != 0;
    ...
    GLFWwindow* mouse_window =
      (bd->MouseWindow == bd->Window || focused) ? bd->Window : NULL;
