...
void nsv_init() { Kairos::init(0, NULL); }
...
void nsv_add_interface(NextSubvolumeMethod* nsv, int id, double dt,
    double* start, double* end, double* norm, int dim)
{
    using namespace Kairos;
...
void nsv_add_species(
    NextSubvolumeMethod* nsv, int id, double D, char* btype, int dim)
{
    using namespace Kairos;
    ...
    if (dim > 0) {
        ...
        if (dim > 1) {
            ...
            if (btype[1] == 'p') {
                ...
                nsv->add_diffusion_between(
                    *ns, ns->D / pow(hy, 2), ylow, yhigh);
                nsv->add_diffusion_between(
                    *ns, ns->D / pow(hy, 2), yhigh, ylow);
            }
            if (dim > 2) {
                ...
                if (btype[2] == 'p') {
                    ...
                    nsv->add_diffusion_between(
                        *ns, ns->D / pow(hz, 2), zlow, zhigh);
                    nsv->add_diffusion_between(
                        *ns, ns->D / pow(hz, 2), zhigh, zlow);
                }
            }
        }
    }
...
public:
    SmoldynSurface(surfaceptr s, enum PanelFace face)
        : s(s)
        , face(face)
    {
    }
    bool lineXsurface(const Kairos::Vect3d& p1, const Kairos::Vect3d& p2) const
    {
        ...
        for (int i = 0; i < PSMAX; ++i) {
            for (int j = 0; j < s->npanel[i]; ++j) {
                if (lineXpanel(p1t, p2t, s->panels[i][j], s->srfss->sim->dim,
                        crsspt, &face1, &face2, NULL, NULL, NULL, 0)
                    == 1) {
                    if ((face1 != face2)
                        && ((face == PFboth) || (face1 == face)))
                        return true;
                }
            }
        }
...
public:
    SmoldynCompartment(compartptr c)
        : c(c)
    {
    }
...
extern void nsv_add_surface(NextSubvolumeMethod* nsv, surfacestruct* surface)
{
    ...
    for (int i = 0; i < ns; ++i) {
        ...
        if ((surface->action[id][0][PFfront] == SAreflect)
            && (surface->action[id][0][PFback] == SAreflect))
            face = PFboth;
        else if (surface->action[id][0][PFfront] == SAreflect)
            face = PFfront;
        else if (surface->action[id][0][PFback] == SAreflect)
            face = PFback;
        else
            face = PFnone;
        if (face != PFnone) {
            // std::cout << "finished reflective surface with
            // face="<<face<<std::endl;
            SmoldynSurface s(surface, face);
            nsv->scale_diffusion_across(
                *(nsv->get_diffusing_species()[i]), s, 0);
        }

        if ((surface->action[id][0][PFfront] == SAabsorb)
            && (surface->action[id][0][PFback] == SAabsorb))
            face = PFboth;
        else if (surface->action[id][0][PFfront] == SAabsorb)
            face = PFfront;
        else if (surface->action[id][0][PFback] == SAabsorb)
            face = PFback;
        else
            face = PFnone;
        if (face != PFnone) {
            // std::cout << "finished absorption surface with
            // face="<<face<<std::endl;
            SmoldynSurface s(surface, face);
...
void nsv_add_reaction(NextSubvolumeMethod* nsv, rxnstruct* reaction)
{
    ...
    for (int i = 0; i < nreactants; ++i) {
        ...
        bool found = false;
        for (std::vector<ReactionComponent>::iterator rc = lhs.begin();
             rc != lhs.end(); rc++) {
            if (rc->species == s) {
                rc->multiplier++;
                found = true;
    ...
    for (int i = 0; i < nproducts; ++i) {
        ...
        if (reaction->prdrep != NULL)
            to_particle = (reaction->prdrep[i] == SRparticle);

        bool found = false;
        for (std::vector<ReactionComponent>::iterator rc = rhs.begin();
             rc != rhs.end(); rc++) {
        ...
        if (!found) {
            ...
            if (to_particle) {
                //                std::cout << "adding a t0_paricle reaction
                //                "<<rhs<<std::endl;
                (rhs.end() - 1)->compartment_index = -1;
            } else {
                //                std::cout << "adding a normal
                //                reaction"<<std::endl;
                (rhs.end() - 1)->compartment_index = 1;
            }
        }
...
//						int nreactants,int
//*reactant_ids, 						int nproducts,int *product_ids) {
//
    ...
    latticestruct* lattice)
{
    ...
    // const double lattice_lengthscale =
    // 0.0001*std::min(lattice->dx[0],std::min(lattice->dx[1],lattice->dx[2]));
    if (!port)
        return;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        //		if (er != 0) simLog(NULL,11,"ERROR: failure in
        //nsv_integrate (while processing surface interactions)\n");
        //
        ...
        //			if
        //(lineXpanel(m->posx,m->pos,port->srf->panels[PSrect][j],sim->dim,crsspt,&face1,&face2,NULL,NULL,NULL,0))
        //{ 				if ((face1!=face2) && (face2==PFfront)) { 					in = false;
        //				}
        //			}
        //		}
        //
        //		if (!in) {
        //			//std::cout <<" particle going back through
        //port!!!"<<std::endl; 			species[nout] = m->ident; 			positions[nout] =
        //m->pos; 			positionsx[nout] = m->via; 			nout++; 			continue;
        //		}
        ...
        for (int d = 0; d < sim->dim; ++d) {
            ...
            if (newr[d] < low) {
                std::cout << "d = " << d << " via = " << m->via[d]
                          << " pos = " << m->pos[d] << " posx = " << m->posx[d]
                          << " newr = " << newr[d] << std::endl;
                simLog(NULL, 11,
                    "ERROR: particle unexpectedly outside lattice domain\n");
            } else if (newr[d] > high) {
                std::cout << "d = " << d << " via = " << m->via[d]
                          << " pos = " << m->pos[d] << " posx = " << m->posx[d]
                          << " newr = " << newr[d] << std::endl;
        ...
        for (int i = 0; i < lattice->nsurfaces; ++i) {
            if ((lattice->surfacelist[i]->action[m->ident][0][PFfront]
                    != SAreflect)
                && (lattice->surfacelist[i]->action[m->ident][0][PFback]
                    != SAreflect)) {
                continue;
            }
            for (int j = 0; j < PSMAX; ++j) {
                for (int k = 0; k < lattice->surfacelist[i]->npanel[j]; ++k) {
                    if (lineXpanel(m->posx, cc.data(),
                            lattice->surfacelist[i]->panels[j][k],
                            port->portss->sim->dim, crsspt, &face1, &face2,
                            NULL, NULL, NULL, 0)
                        == 1) {
                        // if particle crossed surface when adding to lattice,
                        // throw back through port
                        if ((face1 != face2)
                            && (lattice->surfacelist[i]
                                    ->action[m->ident][0][face1]
                                == SAreflect))
                            throw_back = true;
                    }
                }
        ...
        if (throw_back) {
            // std::cout << "throwing back particle at posx =
            // ("<<m->posx[0]<<','<<m->posx[1]<<','<<m->posx[2]<<") and pos =
            // ("<<m->pos[0]<<','<<m->pos[1]<<','<<m->pos[2]<<")"<<std::endl;
    ...
    if (er != n)
        simLog(NULL, 11,
            "ERROR: failure in nsv_integrate (while deleting particles from "
            "port)\n");
        ...
        for (int j = 0; j < np; ++j) {
            ...
            if (port->srf) {
                for (int k = 0; k < port->srf->npanel[PSrect]; ++k) {
                    if (lineXpanel(s->particlesx[j].data(),
                            s->particles[j].data(),
                            port->srf->panels[PSrect][k], sim->dim, crsspt,
                            &face1, &face2, NULL, NULL, NULL, 0)) {
                        if ((face1 != face2)
                            && (port->srf->action[s->id][0][face1] == SAport)) {
                            in = false;
                        }
                    }
                }
            ...
            if (in) {
                ...
                //				if (positions[nout][0] <= -10)
                //std::cout << "x <= -10: "<<positions[nout]<<std::endl; 				if
                //(positions[nout][0] >= 10) std::cout << "x >= -10:
                //"<<positions[nout]<<std::endl; 				if (positions[nout][1] <= -10)
                //std::cout << "x <= -10: "<<positions[nout]<<std::endl; 				if
                //(positions[nout][1] >= 10) std::cout << "x >= -10:
                //"<<positions[nout]<<std::endl;
                positionsx[nout] = s->particlesx[j].data();
                nout++;
            } else {
                // add to lattice
                // std::cout << "adding particle of species "<<s->id<<" back
                // into lattice at position "<<s->particles[j]<<std::endl;
    ...
    for (std::set<int>::iterator i = dirty_indicies.begin();
         i != dirty_indicies.end(); i++) {
    ...
    if (nout > 0) {
        er = portputmols(
            sim, port, nout, species[0], species, positions, positionsx);
        if (er != 0)
            simLog(NULL, 11,
                "ERROR: failure in nsv_integrate (while putting particles in "
                "the port)\n");
    }
...
void nsv_molcountspace(NextSubvolumeMethod* nsv, int id, double* low,
    double* high, int dim, int nbins, int axis, int* ret_array)
{
    using namespace Kairos;
 ...
 */
double nsv_concentration_point(
    NextSubvolumeMethod* nsv, int id, double* point, int dim)
{
    using namespace Kairos;
    Vect3d vpoint(0, 0, 0);
    for (int i = 0; i < dim; ++i) {
...
void nsv_molcount(NextSubvolumeMethod* nsv, int* ret_array)
{
    ...
    for (unsigned int i = 0; i < species.size(); ++i) {
        const int n = std::accumulate(species[i]->copy_numbers.begin(),
                          species[i]->copy_numbers.end(), 0)
...
void nsv_kill_molecule(NextSubvolumeMethod* nsv, int id, double* point, int dim)
{
    ...
    if (s->copy_numbers[index] < 0) {
        simLog(NULL, 11,
            "ERROR: lattice species became less than zero (in "
            "nsv_kill_molecule)\n");
    }
...
/*
 * returns vector of copy numbers for each lattice site and positions of lattice
 * sites
 *
 * return value = number of lattice sites returned for species "id"
 * copy_numbers = array of ints representing copy number of species "id" at each
 * lattice site equals NULL if species does not exist positions = array of
 * doubles representing positions of each site. In lattice-site major order. eg.
 * x-coord of lattice site n is positions[3*n+0]
 */
int nsv_get_species_copy_numbers(NextSubvolumeMethod* nsv, int id,
    const int** copy_numbers, const double** positions)
{
    using namespace Kairos;
