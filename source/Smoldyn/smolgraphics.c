...
enum LightParam graphicsstring2lp(char *string) {
  ...
  if (strbegin(string, "ambient", 0))
    ans = LPambient;
  else if (strbegin(string, "diffuse", 0))
    ans = LPdiffuse;
  else if (strbegin(string, "specular", 0))
    ans = LPspecular;
  else if (strbegin(string, "position", 0))
    ans = LPposition;
  else if (strbegin(string, "on", 0))
    ans = LPon;
  else if (strbegin(string, "off", 0))
    ans = LPoff;
  else if (strbegin(string, "auto", 0))
    ans = LPauto;
  else
    ans = LPnone;
  return ans;
...
char *graphicslp2string(enum LightParam lp, char *string) {
  if (lp == LPambient)
    strcpy(string, "ambient");
  else if (lp == LPdiffuse)
    strcpy(string, "diffuse");
  else if (lp == LPspecular)
    strcpy(string, "specular");
  else if (lp == LPposition)
...
int graphicsreadcolor(char **stringptr, double *rgba) {
  ...
  if ((name[0] >= '0' && name[0] <= '9') || name[0] == '.') { // numbers
    ...
    if (!strcmp(name, "maroon")) {
      ...
      g = 0.5;
      b = 0;
    } else if (!strcmp(name, "chartrouse")) {
      r = 0.5;
      g = 1;
      b = 0;
      ...
      g = 0.88;
      b = 0.82;
    } else if (!strcmp(name, "royal")) {
      r = 0;
      g = 0.14;
      b = 0.4;
    } else if (!strcmp(name, "sky")) {
      r = 0.53;
      g = 0.81;
      b = 0.92;
      ...
      g = 0.55;
      b = 0;
    } else if (!strcmp(name, "darkyellow")) {
      r = 1;
      g = 0.84;
      b = 0;
      ...
      g = 0.25;
      b = 0.25;
    } else if (!strcmp(name, "lightred")) {
      r = 1;
      g = 0.39;
      b = 0.28;
    } else if (!strcmp(name, "lightorange")) {
      r = 1;
      g = 0.75;
      b = 0.2;
      ...
      g = 0.81;
      b = 0.98;
    } else if (!strcmp(name, "lightviolet")) {
      r = 0.68;
      g = 0.1;
      b = 1;
...
graphicsssptr graphssalloc(void) {
  ...
  for (lt = 0; lt < MAXLIGHTS; lt++)
    graphicssetlight(NULL, graphss, lt, LPauto, NULL);
...
void graphssoutput(simptr sim) {
  ...
  if (graphss->graphics == 1)
    simLog(sim, 2, "OpenGL");
  else if (graphss->graphics == 2)
    simLog(sim, 2, "OpenGL_good");
  else if (graphss->graphics == 3)
    simLog(sim, 2, "OpenGL_better");
  simLog(sim, 2, ", every %i iterations\n", graphss->graphicit);
  if (graphss->graphicdelay > 0)
    simLog(sim, 2, " delay per frame: %ui ms\n", graphss->graphicdelay);

  simLog(sim, 2, " frame thickness: %g|L", graphss->framepts);
  if (graphss->gridpts)
    simLog(sim, 2, ", grid thickness: %g|L", graphss->gridpts);
  simLog(sim, 2, "\n");
  if (graphss->framepts)
  ...
  if (strcmp(string1, string2))
    simLog(sim, 2, " TIFF name: %s\n", string1);
  if (i1 != (int)gl2GetNumber("TiffNumberDefault") ||
      i2 != (int)gl2GetNumber("TiffNumMaxDefault"))
...
void writegraphss(simptr sim, FILE *fptr) {
  ...
  if (graphss->graphics == 0)
    fprintf(fptr, "graphics none\n");
  else if (graphss->graphics == 1)
    fprintf(fptr, "graphics opengl\n");
  else if (graphss->graphics == 2)
    fprintf(fptr, "graphics opengl_good\n");
  else if (graphss->graphics == 3)
    fprintf(fptr, "graphics opengl_better\n");
  if (graphss->graphicit > 1)
    fprintf(fptr, "graphic_iter %i\n", graphss->graphicit);
  if (graphss->graphicdelay > 0)
    fprintf(fptr, "graphic_delay %ui\n", graphss->graphicdelay);
  ...
  for (lt = 0; lt < MAXLIGHTS; lt++)
    if (graphss->lightstate[lt] != LPauto) {
      ...
      fprintf(fptr, "light %i %s\n", lt,
              graphicslp2string(graphss->lightstate[lt], string));
    }
...
void graphicssetcondition(graphicsssptr graphss, enum StructCond cond,
  ...
  if (!graphss)
    return;
  if (upgrade == 0 && graphss->condition > cond)
    graphss->condition = cond;
  else if (upgrade == 1 && graphss->condition < cond)
    graphss->condition = cond;
  else if (upgrade == 2)
    graphss->condition = cond;
...
int graphicsenablegraphics(simptr sim, const char *type) {
  ...
  if (type) {
    if (!strcmp(type, "none"))
      code = 0;
    else if (!strcmp(type, "opengl"))
      code = 1;
    else if (!strcmp(type, "opengl_good"))
      code = 2;
    else if (!strcmp(type, "opengl_better"))
      code = 3;
    else
      return 3;
  } else
  ...
  if (sim->graphss && (code == -1 || sim->graphss->graphics == code))
    return 0;
  if (!sim->graphss && code == 0)
    return 0;
...
int graphicssettextitem(simptr sim, char *itemname) {
  ...
  if (!strcmp(itemname, "time"))
    ...
    return 2; // unrecognized item name

  for (item = 0; item < graphss->ntextitems; item++)
    if (!strcmp(itemname, graphss->textitems[item]))
      return 3;
...
int graphicssetlight(simptr sim, graphicsssptr graphss, int lt,
  ...
  if (lt == -1) { // room lights
    ...
    for (i = 0; i < 4; i++)
      graphss->ambilight[lt][i] = value[i];
  } else if (ltparam == LPdiffuse) {
    if (graphss->lightstate[lt] == LPauto)
      graphss->lightstate[lt] = LPon;
    for (i = 0; i < 4; i++)
      graphss->difflight[lt][i] = value[i];
  } else if (ltparam == LPspecular) {
    if (graphss->lightstate[lt] == LPauto)
      graphss->lightstate[lt] = LPon;
    for (i = 0; i < 4; i++)
      graphss->speclight[lt][i] = value[i];
  } else if (ltparam == LPposition) {
    if (graphss->lightstate[lt] == LPauto)
      graphss->lightstate[lt] = LPon;
    for (i = 0; i < 4; i++)
      graphss->lightpos[lt][i] = value[i];
  } else if (ltparam == LPon)
    graphss->lightstate[lt] = LPon;
  else if (ltparam == LPoff)
...
int graphicsupdateinit(simptr sim) {
  ...
  wlist = sim->wlist;
  if (dim == 1)
    gl2Initialize(sim->filename, (float)wlist[0]->pos, (float)wlist[1]->pos, 0,
                  0, 0, 0);
  else if (dim == 2)
    ...
    gl2Initialize(sim->filename, (float)wlist[0]->pos, (float)wlist[1]->pos,
                  (float)wlist[2]->pos, (float)wlist[3]->pos,
                  (float)wlist[4]->pos, (float)wlist[5]->pos);
    if (sim->srfss) {
      glEnable(GL_BLEND);
      ...
      while (fdm || bdm) {
        ...
        if (srf->npanel[PStri]) {
          ...
          for (p = 0; p < srf->npanel[PStri]; p++) {
            if (graphics >= 3)
              glNormal3fv(
                  gl2Double2GLfloat(srf->panels[PStri][p]->front, glfvect, 4));
...
void RenderFilaments(simptr sim) {
  ...
  if (sim->dim == 1)
    ...
    for (ft = 0; ft < filss->ntype; ft++) {
      ...
      if (drawmode & DMface)
        dm = DMedge; // face and edge are the same in 2D
      else if (drawmode & DMedge)
        dm = DMedge;
      else if (drawmode & DMvert)
        dm = DMvert;
      else
        dm = DMno;
      while (dm) {
    ...
    for (ft = 0; ft < filss->ntype; ft++) {
      ...
      while (dm) {
        ...
        if (dm == DMvert) {
          ...
          if (graphics >= 3) {
            glMaterialfv(GL_FRONT, GL_SPECULAR,
                         gl2Double2GLfloat(filtype->color, glfvect, 4));
            glMaterialfv(GL_BACK, GL_SPECULAR,
                         gl2Double2GLfloat(blackcolor, glfvect, 4));
        ...
        if (dm == DMvert || dm == DMedge) {
          ...
          for (f = 0; f < filtype->nfil; f++) {
            ...
            for (vtx = 0; vtx < fil->nseg; vtx++) {
              ...
              glMatrixMode(GL_MODELVIEW);
              glPushMatrix();
              glTranslated((GLdouble)(point[0]), (GLdouble)(point[1]),
                           (GLdouble)(point[2]));
              ...
              gl2DrawCylinder(filtype->edgepts, filtype->edgepts,
                              fil->segments[vtx]->len, 8, 2, 0,
                              graphics >= 3 ? 1 : 0);
              glPopMatrix();
            }
          }
        ...
        if (glIsEnabled(GL_LINE_STIPPLE))
          glDisable(GL_LINE_STIPPLE);
        if (dm == DMface && drawmode & DMedge)
          dm = DMedge;
        else if (dm == DMedge && drawmode & DMvert)
          dm = DMvert;
        else
          dm = DMno;
      }
        ...
        for (f = 0; f < filtype->nfil; f++) {
          ...
          if (fil->filwork) {
            ...
            for (vtx = 0; vtx <= fil->nseg; vtx++) {
              ...
              glVertex3d((GLdouble)(point[0]), (GLdouble)(point[1]),
                         (GLdouble)(point[2]));
              glVertex3d((GLdouble)(vect[0]), (GLdouble)(vect[1]),
                         (GLdouble)(vect[2]));
            }
          }
        }
      ...
      for (f = 0; f < filtype->nfil; f++) {
        ...
        if (fil->nsequence) {
          for (seqi = 0; seqi < fil->nsequence; seqi++) {
            ...
            str[1] = '\0';
            filGetPositionOnFil(
                fil, ((double)seqi + 0.5) / (double)(fil->nsequence), pos);
            pos[2] -= fil->segments[0]
                          ->thk; //?? quick hack but might be worth keeping
      ...
      if (sim->mols->listtype[ll] == MLTsystem)
        for (m = 0; m < mols->nl[ll]; m++) {
          ...
          if (mols->display[i][ms] > 0) {
            ...
            glPushMatrix();
            if (dim == 1)
              glTranslated((GLdouble)(mptr->pos[0]), (GLdouble)ymid,
                           (GLdouble)zmid);
            else if (dim == 2)
              glTranslated((GLdouble)(mptr->pos[0]), (GLdouble)(mptr->pos[1]),
                           (GLdouble)zmid);
            else
              glTranslated((GLdouble)(mptr->pos[0]), (GLdouble)(mptr->pos[1]),
                           (GLdouble)(mptr->pos[2]));
...
void RenderLattice(simptr sim) {
  ...
  for (lat = 0; lat < sim->latticess->nlattice; lat++) {
    ...
    for (ilat = 0; ilat < lattice->nspecies;
      ...
      NSV_CALL(n = nsv_get_species_copy_numbers(lattice->nsv, ismol,
                                                &copy_numbers, &positions));
      for (i = 0; i < n; ++i) { // n is the total number of molecules of this
                                // species, copy_numbers is how many in each
                                // site, and positions are the site positions
...
void RenderText(simptr sim) {
  ...
  gl2DrawTextD(5, 95, graphss->textcolor, GLUT_BITMAP_HELVETICA_12, string2,
               -1);
#endif
  return;
}
...
void RenderSim(simptr sim, int swapbuffers) {
  ...
  timenow = clock();
  if (swapbuffers == 1 && sim->time == oldsimtime &&
      (double)(timenow - oldclocktime) / CLOCKS_PER_SEC < 0.005)
