...
// enumerated types
enum SrfAction
surfstring2act(char* string);

// low level utilities
panelptr
readpanelname(simptr sim, surfaceptr srf, const char* str);
int
panelpoints(enum PanelShape ps, int dim);
int
surfpanelparams(enum PanelShape ps, int dim);
void
panelmiddle(panelptr pnl, double* middle, int dim, int onpanel);
int
srfsamestate(enum MolecState ms1,
  enum PanelFace face1,
  enum MolecState ms2,
  enum MolecState* ms3ptr);
void
srfreverseaction(enum MolecState ms1,
  enum PanelFace face1,
  enum MolecState ms2,
  enum MolecState* ms3ptr,
  enum PanelFace* face2ptr,
  enum MolecState* ms4ptr);
void
srftristate2index(enum MolecState ms,
  enum MolecState ms1,
  enum MolecState ms2,
  enum MolecState* ms3ptr,
  enum PanelFace* faceptr,
  enum MolecState* ms4ptr);

// memory management
surfactionptr
surfaceactionalloc(int species);
void
surfaceactionfree(surfactionptr actdetails);
int
panelsalloc(surfaceptr srf, int dim, int maxpanel, int maxspecies, enum PanelShape ps);
void
panelfree(panelptr pnl);
int
...
void
surfacefree(surfaceptr srf, int maxspecies);
surfacessptr
...
// structure set up
int
surfsetmaxpanel(surfaceptr srf, int dim, enum PanelShape ps, int maxpanel);
int
surfsetemitterabsorption(simptr sim);
double
srfcalcrate(simptr sim,
  surfaceptr srf,
  int i,
  enum MolecState ms1,
  enum PanelFace face,
...
srfcalcprob(simptr sim,
  ...
  enum MolecState ms2);
int
surfupdateparams(simptr sim);
int
surfupdatelists(simptr sim);

// core simulation functions
void
panelnormal(panelptr pnl, double* pos, enum PanelFace face, int dim, double* norm);
void
movept2panel(double* pt, panelptr pnl, int dim, double margin);
void
surfacereflect(moleculeptr mptr,
  panelptr pnl,
  double* crsspt,
  int dim,
  enum PanelFace face);
...
/* surfstring2face */
enum PanelFace
surfstring2face(char* string)
{
    enum PanelFace ans;

    if (strbegin(string, "front", 0))
        ans = PFfront;
    else if (strbegin(string, "back", 0))
        ans = PFback;
    else if (strbegin(string, "all", 0) || strbegin(string, "both", 0))
        ans = PFboth;
    else
        ans = PFnone;
    return ans;
...
/* surfface2string */
char*
surfface2string(enum PanelFace face, char* string)
{
    if (face == PFfront)
        strcpy(string, "front");
    else if (face == PFback)
...
surfstring2act(char* string)
{
    ...
    if (strbegin(string, "reflect", 0))
        ans = SAreflect;
    else if (strbegin(string, "transmit", 0))
        ans = SAtrans;
    else if (strbegin(string, "absorb", 0))
        ans = SAabsorb;
    else if (strbegin(string, "jump", 0))
        ans = SAjump;
    else if (!strcmp(string, "periodic"))
        ans = SAjump;
    else if (!strcmp(string, "port"))
        ans = SAport;
    else if (strbegin(string, "multiple", 0))
        ans = SAmult;
    else if (strbegin(string, "no", 0))
        ans = SAno;
    else if (strbegin(string, "adsorb", 0))
        ans = SAadsorb;
    else if (strbegin(string, "revdes", 0))
        ans = SArevdes;
    else if (strbegin(string, "irrevdes", 0))
        ans = SAirrevdes;
    else if (strbegin(string, "flip", 0))
        ans = SAflip;
    else
        ans = SAnone;
    return ans;
...
/* surfact2string */
char*
surfact2string(enum SrfAction act, char* string)
{
    if (act == SAreflect)
        strcpy(string, "reflect");
    else if (act == SAtrans)
        strcpy(string, "transmit");
    else if (act == SAabsorb)
        strcpy(string, "absorb");
    else if (act == SAjump)
        strcpy(string, "jump");
    else if (act == SAport)
        strcpy(string, "port");
    else if (act == SAmult)
        strcpy(string, "multiple");
    else if (act == SAno)
        strcpy(string, "no");
    else if (act == SAadsorb)
        strcpy(string, "adsorb");
    else if (act == SArevdes)
        strcpy(string, "revdes");
    else if (act == SAirrevdes)
        strcpy(string, "irrevdes");
    else if (act == SAflip)
        strcpy(string, "flip");
    else
        strcpy(string, "none");
...
/* surfstring2ps */
enum PanelShape
surfstring2ps(char* string)
{
    enum PanelShape ans;

    if (strbegin(string, "rectangle", 0))
        ans = PSrect;
    else if (strbegin(string, "triangle", 0))
        ans = PStri;
    else if (strbegin(string, "sphere", 0))
        ans = PSsph;
    else if (strbegin(string, "cylinder", 0))
        ans = PScyl;
    else if (strbegin(string, "hemisphere", 0))
        ans = PShemi;
    else if (strbegin(string, "disk", 0))
        ans = PSdisk;
    else if (strbegin(string, "all", 0))
        ans = PSall;
    else
        ans = PSnone;
    return ans;
...
/* surfps2string */
char*
surfps2string(enum PanelShape ps, char* string)
{
    if (ps == PSrect)
        strcpy(string, "rect");
    else if (ps == PStri)
...
/* surfstring2dm */
enum DrawMode
surfstring2dm(char* string)
{
    enum DrawMode ans;

    if (strbegin(string, "none", 0))
        ans = DMno;
    else if (!strcmp(string, "ve") || !strcmp(string, "ev"))
        ans = DMve;
    else if (!strcmp(string, "vf") || !strcmp(string, "fv"))
        ans = DMvf;
    else if (!strcmp(string, "ef") || !strcmp(string, "fe"))
        ans = DMef;
    else if (!strcmp(string, "vef") || !strcmp(string, "vfe") || !strcmp(string, "evf") ||
             !strcmp(string, "efv") || !strcmp(string, "fev") || !strcmp(string, "fve"))
        ans = DMvef;
    else if (strbegin(string, "vertex", 0))
        ans = DMvert;
    else if (strbegin(string, "edge", 0))
        ans = DMedge;
    else if (strbegin(string, "face", 0))
        ans = DMface;
    else
        ans = DMnone;
    return ans;
...
/* surfdm2string */
char*
surfdm2string(enum DrawMode dm, char* string)
{
    if (dm == DMno)
        strcpy(string, "no");
    else if (dm == DMvert)
...
/* readsurfacename. */
int
readsurfacename(simptr sim, const char* str, enum PanelShape* psptr, int* pptr)
{
    ...
    if (!str)
        return -1;
    if (!sim->srfss || !sim->srfss->nsrf)
        return -2;

    itct = sscanf(str, "%s", snm);
    if (itct != 1)
        return -3; // cannot read name
    colon = strchr(snm, ':');
    if (colon) {
        strcpy(pnm, colon + 1);
        *colon = '\0';
    ...
    if (!strcmp(snm, "all")) { // all surfaces
        ...
        if (s == -1) // unknown surface
            ...
            if (pnm[0] == '\0')
                ...
                ps = PSall;
                p = -5;
            } else if (VCellDefined &&
                       strstr(pnm, "tri_") == pnm) { // surface:tri_#_#_#	(VCELL)
                ps = PStri;
                int memIndex;
                int globalIndex;
                sscanf(pnm, "tri_%d_%d_%d", &p, &globalIndex, &memIndex);
            } else { // surface:panel
                for (ps = (enum PanelShape)0; p == -1 && ps < PSMAX;
                     ps = (enum PanelShape)(ps + 1))
                    p = stringfind(sim->srfss->srflist[s]->pname[ps],
                      sim->srfss->srflist[s]->npanel[ps],
                      pnm);
                if (p == -1) {
                    ps = PSnone;
                    p = -3;
...
readpanelname(simptr sim, surfaceptr srf, const char* str)
{
    ...
    if (strchr(str, ':'))
        ...
        snprintf(name, STRCHAR, "%s:%s", srf->sname, str);
    else
        return NULL;
...
panelpoints(enum PanelShape ps, int dim)
{
    ...
    if (ps == PSrect) {
        ...
        npts = 2;
    else if (ps == PScyl && dim > 1)
        npts = 5;
    else if (ps == PShemi && dim > 1)
        npts = 3;
    else if (ps == PSdisk && dim > 1)
        npts = 2;
    else
        npts = 0;
    return npts;
...
panelmiddle(panelptr pnl, double* middle, int dim, int onpanel)
{
    ...
    if (ps == PSrect) {
        ...
        if (onpanel) {
            if (dim == 2)
                Geo_LineNormal2D(point[0], point[1], middle, norm);
            else
                Geo_LineNormal3D(point[0], point[1], middle, norm);
            for (d = 0; d < dim; d++)
...
/* panelarea */
double
panelarea(panelptr pnl, int dim)
{
    enum PanelShape ps;
    ...
    if (dim == 1) {
        ...
        if (ps == PSrect)
            area = fabs(point[1][(int)front[2]] - point[0][(int)front[2]]);
        else if (ps == PStri)
            area = sqrt((point[1][0] - point[0][0]) * (point[1][0] - point[0][0]) +
                        (point[1][1] - point[0][1]) * (point[1][1] - point[0][1]));
        else if (ps == PSsph)
            area = 2.0 * PI * point[1][0];
        else if (ps == PScyl)
            area = 2.0 * sqrt((point[1][0] - point[0][0]) * (point[1][0] - point[0][0]) +
                              (point[1][1] - point[0][1]) * (point[1][1] - point[0][1]));
        else if (ps == PShemi)
            area = PI * point[1][0];
        else if (ps == PSdisk)
        ...
        if (ps == PSrect) {
            d = 0;
            while (d == (int)front[1] || d == (int)front[2])
                d++;
            area = fabs((point[2][(int)front[2]] - point[0][(int)front[2]]) *
                        (point[2][d] - point[0][d]));
        } else if (ps == PStri)
            area = Geo_TriArea3(point[0], point[1], point[2], front);
        else if (ps == PSsph)
            area = 4.0 * PI * point[1][0] * point[1][0];
        else if (ps == PScyl)
            area = 2.0 * PI * point[2][0] *
                   sqrt((point[1][0] - point[0][0]) * (point[1][0] - point[0][0]) +
                        (point[1][1] - point[0][1]) * (point[1][1] - point[0][1]) +
                        (point[1][2] - point[0][2]) * (point[1][2] - point[0][2]));
        else if (ps == PShemi)
            area = 2.0 * PI * point[1][0] * point[1][0];
        else if (ps == PSdisk)
            area = PI * point[1][0] * point[1][0];
        else
            area = 0;
    } else
        area = 0;
...
/* surfacearea */
double
surfacearea(surfaceptr srf, int dim, int* totpanelptr)
{
    int ps, p, totpanel;
    double area;
...
surfacearea2(simptr sim, int surface, enum PanelShape ps, char* pname, int* totpanelptr)
{
    ...
    if (ps == PSnone) {
    ...
    else if (surface >= 0 && ps != PSall && pname &&
             strcmp(pname, "all")) { // specific panel
        ...
        totpanel = 0;
        for (s = slo; s < shi; s++)
            for (ps = (enum PanelShape)pslo; ps < (enum PanelShape)pshi;
                 ps = (enum PanelShape)(ps + 1)) {
                srf = sim->srfss->srflist[s];
                if (!pname || !strcmp(pname, "all")) {
                    plo = 0;
                    phi = srf->npanel[ps];
                } else if ((panel = stringfind(srf->pname[ps], srf->npanel[ps], pname)) <
                           0)
                    plo = phi = 0;
                else {
                    plo = panel;
...
/* panelrandpos */
void
panelrandpos(panelptr pnl, double* pos, int dim)
{
    int d;
    ...
    if (dim == 1) {
        ...
        if (ps == PSrect) {
            ...
            pos[(int)front[2]] =
              unirandCCD(point[0][(int)front[2]], point[1][(int)front[2]]);
        } else if (ps == PStri) {
            flt1 = randCCD();
            ...
            pos[0] = point[0][0] + flt1 * (point[1][0] - point[0][0]) +
                     flt2 * point[2][0] * front[0];
            pos[1] = point[0][1] + flt1 * (point[1][1] - point[0][1]) +
                     flt2 * point[2][0] * front[1];
        } else if (ps == PShemi) {
...
/* surfrandpos */
panelptr
surfrandpos(surfaceptr srf, double* pos, int dim)
{
    panelptr pnl;
...
/* issurfprod */
int
issurfprod(simptr sim, int i, enum MolecState ms)
{
    surfacessptr srfss;
    int s, i1;
    surfaceptr srf;
            ...
            for (face = (enum PanelFace)0; face < 3; face = (enum PanelFace)(face + 1)) {
                ...
                if (actdetails)
                    if (actdetails->srfrate[ms] > 0 || actdetails->srfprob[ms] > 0 ||
                        actdetails->srfdatasrc[ms] == 3)
                        if (actdetails->srfnewspec[ms] == i)
                            return 1;
            }
        for (i1 = 0; i1 < srfss->maxspecies;
             i1++) // failed, so try with species change at surface
            for (ms1 = (enum MolecState)0; ms1 < MSMAX; ms1 = (enum MolecState)(ms1 + 1))
                for (face = (enum PanelFace)0; face < 3;
                     face = (enum PanelFace)(face + 1)) {
                    actdetails = srf->actdetails[i1][ms1][face];
                    if (actdetails)
                        if (actdetails->srfrate[ms] > 0 || actdetails->srfprob[ms] > 0 ||
                            actdetails->srfdatasrc[ms] == 3)
  ...
  enum MolecState* ms3ptr)
{
    ...
    if (face1 == PFfront && ms2 == MSsoln)
        same = 1;
    else if (face1 == PFback && ms2 == MSbsoln)
        same = 1;
    else if (face1 == PFnone && ms1 == ms2)
        same = 1;
    else
        same = 0;
  ...
  enum MolecState* ms4ptr)
{
    ...
    if (ms == MSsoln && (ms1 == MSsoln || ms1 == MSbsoln))
        ms3 = MSsoln;
    else if (ms == MSsoln)
        ms3 = ms1;
    else
...
/* srfindex2tristate */
void
srfindex2tristate(enum MolecState ms3,
  enum PanelFace face,
  enum MolecState ms4,
  enum MolecState* msptr,
  enum MolecState* ms1ptr,
...
/* srfcompareaction */
int
srfcompareaction(enum SrfAction act1,
  surfactionptr details1,
  enum SrfAction act2,
  surfactionptr details2)
{
    int cmp;
    enum MolecState ms;

    if (act1 == act2) {
        if (act1 != SAmult || !details1 || !details2)
            return 0;
        cmp = 0;
        for (ms = (enum MolecState)0; ms < MSMAX && cmp == 0;
             ms = (enum MolecState)(ms + 1)) {
            if (details1->srfrate[ms] < details2->srfrate[ms])
                cmp = -1;
            if (details1->srfrate[ms] > details2->srfrate[ms])
                cmp = 1;
        }
    ...
    if (act1 == SAtrans)
        return 1;
    if (act1 == SAmult &&
        (act2 == SAreflect || act2 == SAjump || act2 == SAabsorb || act2 == SAport))
        return 1;
    if (act1 == SAreflect && (act2 == SAjump || act2 == SAabsorb || act2 == SAport))
        return 1;
    if (act1 == SAjump && (act2 == SAabsorb || act2 == SAport))
        return 1;
    if (act1 == SAabsorb && (act2 == SAport))
        return 1;
...
surfaceactionalloc(int species)
{
    ...
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfnewspec[ms] = species;

    CHECKMEM(actdetails->srfrate = (double*)calloc(MSMAX1, sizeof(double)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfrate[ms] = 0;

#ifdef OPTION_VCELL
    CHECKMEM(actdetails->srfRateValueProvider =
               (ValueProvider**)calloc(MSMAX1, sizeof(ValueProvider*)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfRateValueProvider[ms] = 0;
#endif

    CHECKMEM(actdetails->srfprob = (double*)calloc(MSMAX1, sizeof(double)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfprob[ms] = 0;

    CHECKMEM(actdetails->srfcumprob = (double*)calloc(MSMAX1, sizeof(double)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfcumprob[ms] = 0;

    CHECKMEM(actdetails->srfdatasrc = (int*)calloc(MSMAX1, sizeof(int)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfdatasrc[ms] = 0;

    CHECKMEM(actdetails->srfrevprob = (double*)calloc(MSMAX1, sizeof(double)));
    for (ms = (enum MolecState)0; ms < MSMAX1; ms = (enum MolecState)(ms + 1))
        actdetails->srfrevprob[ms] = 0;
...
panelsalloc(surfaceptr srf, int dim, int maxpanel, int maxspecies, enum PanelShape ps)
{
    ...
    if (maxpanel <= 0 || maxpanel < oldmaxpanel)
        return 0;
    else if (maxpanel == oldmaxpanel)
        return 1;
...
failure:
    if (ErrorType != 1)
        simLog(NULL, 10, "Unable to allocate memory in panelsalloc");
    return 0;
}
...
emittersalloc(surfaceptr srf, enum PanelFace face, int oldmaxspecies, int maxspecies)
{
    ...
    if (maxspecies > oldmaxspecies) { // surface structure data, for either from scratch
                                      // or larger maxspecies
        CHECKMEM(newmaxemitter = (int*)calloc(maxspecies, sizeof(int)));
        for (i1 = 0; i1 < oldmaxspecies; i1++)
            newmaxemitter[i1] = srf->maxemitter[face][i1];
        for (; i1 < maxspecies; i1++)
            newmaxemitter[i1] = 0;

        CHECKMEM(newnemitter = (int*)calloc(maxspecies, sizeof(int)));
        for (i1 = 0; i1 < oldmaxspecies; i1++)
            newnemitter[i1] = srf->nemitter[face][i1];
        for (; i1 < maxspecies; i1++)
            newnemitter[i1] = 0;

        CHECKMEM(newemitteramount = (double**)calloc(maxspecies, sizeof(double*)));
        for (i1 = 0; i1 < oldmaxspecies; i1++)
            newemitteramount[i1] = srf->emitteramount[face][i1];
        for (; i1 < maxspecies; i1++)
            newemitteramount[i1] = NULL;

        CHECKMEM(newemitterpos = (double***)calloc(maxspecies, sizeof(double**)));
        for (i1 = 0; i1 < oldmaxspecies; i1++)
            newemitterpos[i1] = srf->emitterpos[face][i1];
        for (; i1 < maxspecies; i1++)
            newemitterpos[i1] = NULL;
    ...
    for (ps = (enum PanelShape)0; ps < PSMAX;
         ps = (enum PanelShape)(ps + 1)) // panel structure data
        for (p = 0; p < srf->maxpanel[ps]; p++) {
            ...
            if (!pnl->emitterabsorb[face] || maxspecies > oldmaxspecies) {
                ...
                if (maxspecies > oldmaxspecies)
                    for (i1 = 0; i1 < oldmaxspecies; i1++)
                        newemitterabsorb[i1] = pnl->emitterabsorb[face][i1];
                for (; i1 < maxspecies; i1++)
                    newemitterabsorb[i1] = 0;
...
/* surfexpandmollist */
int
surfexpandmollist(surfaceptr srf, int newmax, int ll)
{
    int m;
    ...
    for (m = 0; m < srf->maxmol[ll] && m < newmax; m++)
        newmolll[m] = srf->mol[ll][m];
    for (; m < newmax; m++)
        newmolll[m] = NULL;
...
surfacealloc(surfaceptr srf, int oldmaxspecies, int maxspecies, int dim)
{
    ...
    if (srf && oldmaxspecies == maxspecies)
        return srf;
    ...
    if (maxspecies) {
        CHECKMEM(
          newaction = (enum SrfAction***)calloc(maxspecies, sizeof(enum SrfAction**)));
        ...
        for (; i < maxspecies; i++) {
            CHECKMEM(
              newaction[i] = (enum SrfAction**)calloc(MSMAX, sizeof(enum SrfAction*)));
            for (ms = (enum MolecState)0; ms < MSMAX; ms = (enum MolecState)(ms + 1))
                newaction[i][ms] = NULL;
            for (ms = (enum MolecState)0; ms < MSMAX; ms = (enum MolecState)(ms + 1)) {
                CHECKMEM(
                  newaction[i][ms] = (enum SrfAction*)calloc(3, sizeof(enum SrfAction)));
        ...
        CHECKMEM(
          newactdetails = (surfactionptr***)calloc(maxspecies, sizeof(surfactionptr**)));
        for (i = 0; i < maxspecies; i++)
            newactdetails[i] = NULL;
        for (i = 0; i < oldmaxspecies; i++)
            newactdetails[i] = srf->actdetails[i];
        for (; i < maxspecies; i++) {
            CHECKMEM(
              newactdetails[i] = (surfactionptr**)calloc(MSMAX, sizeof(surfactionptr*)));
            for (ms = (enum MolecState)0; ms < MSMAX; ms = (enum MolecState)(ms + 1))
                newactdetails[i][ms] = NULL;
            for (ms = (enum MolecState)0; ms < MSMAX; ms = (enum MolecState)(ms + 1)) {
                CHECKMEM(newactdetails[i][ms] =
                           (surfactionptr*)calloc(3, sizeof(surfactionptr)));
...
surfacessalloc(surfacessptr srfss, int maxsurface, int maxspecies, int dim)
{
    ...
    if (!srfss) { // new allocation
        ...
        if (maxspecies > srfss->maxspecies) {
            for (s = 0; s < srfss->maxsrf; s++) {
                CHECK(surfacealloc(
                        srfss->srflist[s], srfss->maxspecies, maxspecies, dim) != NULL);
    ...
    if (maxsurface > srfss->maxsrf) { // allocate any new surface names and surfaces
        ...
        for (s = 0; s < oldmaxsrf; s++)
            newnames[s] = srfss->snames[s];
        for (; s < maxsurface; s++)
            CHECKMEM(newnames[s] = EmptyString());

        CHECKMEM(newsrflist =
                   (surfaceptr*)calloc(maxsurface, sizeof(surfaceptr))); // surface list
        for (s = 0; s < maxsurface; s++)
            newsrflist[s] = NULL;
        for (s = 0; s < oldmaxsrf; s++)
        ...
        if (srfss->sim && srfss->sim->mols && srfss->sim->mols->surfdrift) {
            CHECK(molexpandsurfdrift(
                    srfss->sim, srfss->sim->mols->maxspecies, oldmaxsrf) == 0);
        }
    }
...
failure:
    if (newsrfss)
        surfacessfree(srfss);
    if (ErrorType != 1)
        simLog(NULL, 10, "Unable to allocate memory in surfacessalloc");
    return NULL;
}

/* surfacessfree */
void
surfacessfree(surfacessptr srfss)
{
    int s;
...
surfaceoutput(simptr sim)
{
    ...
    simLog(sim,
      2,
      " Surface epsilon, margin, and neighbor distances: %g %g %g\n",
      srfss->epsilon,
      srfss->margin,
      srfss->neighdist);
    ...
    if (sim->mols) {
        ...
        for (ll = 0; ll < srfss->nmollist; ll++)
            if (srfss->srfmollist[ll] & SMLdiffuse)
                simLog(sim, 2, " %s", sim->mols->listname[ll]);
        simLog(sim, 2, "\n");
        simLog(sim, 2, " Molecule lists checked after reactions:");
        for (ll = 0; ll < srfss->nmollist; ll++)
            if (srfss->srfmollist[ll] & SMLreact)
                simLog(sim, 2, " %s", sim->mols->listname[ll]);
        simLog(sim, 2, "\n");
        simLog(sim,
          1,
          " Local molecule list record: %i lists of %i allocated\n",
          srfss->nmollist,
          srfss->maxmollist);
    ...
    simLog(sim,
      2,
      " Surfaces allocated: %i, surfaces defined: %i\n\n",
      srfss->maxsrf,
      srfss->nsrf);
    for (s = 0; s < srfss->nsrf; s++) {
        srf = srfss->srflist[s];
        ...
        if (srf->port[PFfront])
            simLog(sim,
              2,
              "  The front of this surface is part of port %s\n",
              srf->port[PFfront]->portname);
        if (srf->port[PFback])
            simLog(sim,
              2,
              "  The back of this surface is part of port %s\n",
        ...
        if (sim->mols && srf->action) {
            ...
            for (i = 1; i < nspecies; i++) {
                for (face = (enum PanelFace)0; face < 2;
                     face = (enum PanelFace)(face + 1)) {
                    same = 1;
                    act = action[i][MSsoln][face];
                    newident = ((actdetails = srf->actdetails[i][MSsoln][face]) &&
                                 actdetails->srfdatasrc[MSsoln] == 3)
                                 ? actdetails->srfnewspec[MSsoln]
                                 : 0;
                    for (ms = (enum MolecState)0; ms < MSMAX;
                         ms = (enum MolecState)(ms + 1)) {
                        if (action[i][ms][face] != act)
                            same = 0;
                        if (newident && !((actdetails = srf->actdetails[i][ms][face]) &&
                                          actdetails->srfdatasrc[ms] == 3 &&
                                          actdetails->srfnewspec[ms] == newident))
                            same = 0;
                    }
                    if (same) {
                        if (act != SAmult) {
                            simLog(sim,
                              ...
                              "   %s(all) at %s: %s",
                              sim->mols->spname[i],
                              face == PFfront ? "front" : "back",
                              surfact2string(act, string));
                            if (newident)
                                simLog(sim,
                                  2,
                                  " (convert to %s)",
                                  sim->mols->spname[newident]);
                            simLog(sim, 2, "\n");
                        }
                    } else {
                        for (ms = (enum MolecState)0; ms < MSMAX;
                            ...
                            if (sim->mols->exist[i][ms] && act != SAmult) {
                                ...
                                simLog(sim,
                                  ...
                                  face == PFfront ? "front" : "back",
                                  surfact2string(act, string));
                                if ((actdetails = srf->actdetails[i][ms][face]) &&
                                    actdetails->srfdatasrc[ms] == 3)
                                    simLog(sim,
                                      2,
                                      " (convert to %s)",
                                      sim->mols->spname[actdetails->srfnewspec[ms]]);
                                simLog(sim, 2, "\n");
                            }
                        }
                    }
                ...
                for (ms = (enum MolecState)0; ms < MSMAX; ms = (enum MolecState)(ms + 1))
                    for (face = (enum PanelFace)0; face < 3;
                        ...
                        if (action[i][ms][face] == SAmult && srf->actdetails) {
                            ...
                            for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                 ms2 = (enum MolecState)(ms2 + 1)) {
                                ...
                                if (prob > 0 && !srfsamestate(ms, face, ms2, NULL)) {
                                    ...
                                    if (actdetails->srfnewspec[ms2] != i)
                                        simLog(sim,
                                          2,
                                          " (convert to %s)",
                                          sim->mols->spname[actdetails->srfnewspec[ms2]]);
                                    if (actdetails->srfdatasrc[ms2] == 1)
                                        simLog(sim,
                                          2,
                                          ", requested rate=%g",
                                          actdetails->srfrate[ms2]);
                                    else if (actdetails->srfdatasrc[ms2] == 2)
                                        simLog(sim,
                                          2,
                                          ", requested prob=%g",
                                          actdetails->srfprob[ms2]);
                                    simLog(sim,
                                      2,
                                      ", actual rate=%g, prob=%g\n",
                                      srfcalcrate(sim, srf, i, ms, face, ms2),
                                      prob);
                                }
                            }
                        }
            if (none)
                simLog(sim, 2, " no stochastic interactions\n");
        }

        if (sim->mols) {
            simLog(sim,
              2,
              "  surface-bound molecules %s hop between neighboring panels upon "
              "collision\n",
        ...
        simLog(sim,
          2,
          "  edge points: %g, polygon modes: %s %s\n",
          srf->edgepts,
        ...
        if (srf->edgestipple[1] != 0xFFFF)
            simLog(sim,
              ...
              srf->edgestipple[1]);
        if (srf->fshiny != 0)
            simLog(sim, 2, "  front shininess: %g\n", srf->fshiny);
        if (srf->bshiny != 0)
            simLog(sim, 2, "  back shininess: %g\n", srf->bshiny);
            ...
            if (srf->nemitter[face])
                for (i = 0; i < nspecies; i++)
                    if (srf->nemitter[face][i]) {
                        simLog(sim,
                          2,
                          "  %i %s-side emitters defined, of %i allocated:\n",
                          srf->nemitter[face][i],
                          surfface2string(face, string),
                          srf->maxemitter[face][i]);
                        for (emit = 0; emit < srf->nemitter[face][i]; emit++) {
                            simLog(sim,
                              2,
                              "   %g at (%g",
                              srf->emitteramount[face][i][emit],
                              srf->emitterpos[face][i][emit][0]);
                            for (d = 1; d < dim; d++)
        ...
        simLog(sim,
          1,
          "  number of molecule lists: %i of %i allocated\n",
          srf->nmollist,
          srf->maxmollist);
        for (ll = 0; ll < srf->nmollist; ll++)
            simLog(sim,
              1,
              "   list %s has %i molecules of %i allocated\n",
              sim->mols->listname[ll],
              srf->nmol[ll],
              srf->maxmol[ll]);
        ...
        if (srf->action) {
            for (i = 1; i < nspecies; i++) {
                if (srf->action[i][MSsoln][PFfront] == SAjump)
                    jumpfrnt = 1;
                if (srf->action[i][MSsoln][PFback] == SAjump)
                    jumpback = 1;
            }
        }
        ...
        if (srf->maxpanel[PSrect]) {
            ...
            simLog(sim,
              2,
              "  rectangle panels allocated: %i, defined: %i\n",
              srf->maxpanel[PSrect],
              srf->npanel[PSrect]);
            for (p = 0; p < srf->npanel[PSrect] && p < 21; p++) {
                ...
                if (dim == 1)
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g), (%g,%g), facing: %c%1.0f, length: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[1][0],
                      ...
                      front[0] == 1 ? '+' : '-',
                      front[1],
                      Geo_LineLength(point[0], point[1], 2));
                else
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g), (%g,%g,%g), (%g,%g,%g), (%g,%g,%g), facing: "
                      "%c%1.0f, area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      ...
                      front[0] == 1 ? '+' : '-',
                      front[1],
                      Geo_QuadArea(point[0], point[1], point[2], point[3], 3));
                if (jumpfrnt && pnl->jumpp[PFfront])
                    simLog(sim,
                      2,
                      "; front jump: %s, %s",
                      pnl->jumpp[PFfront]->pname,
                      surfface2string(pnl->jumpf[PFfront], string));
                else if (jumpfrnt)
                    simLog(sim, 2, "; front jump: NO PANEL");
                if (jumpback && pnl->jumpp[PFback])
                    simLog(sim,
                      2,
                      "; back jump: %s, %s",
                      pnl->jumpp[PFback]->pname,
                      surfface2string(pnl->jumpf[PFback], string));
                else if (jumpback)
                    simLog(sim, 2, "; back jump: NO PANEL");
                for (face = PFfront; face <= PFback; face = (enum PanelFace)(face + 1)) {
                    if (pnl->emitterabsorb[face]) {
                        simLog(
                          sim, 2, "; %s absorb probs.:", surfface2string(face, string));
                        for (i = 1; i < nspecies; i++)
                ...
                if (pnl->maxneigh) {
                    simLog(sim,
                      1,
                      "    neighbors allocated: %i, defined: %i",
                      pnl->maxneigh,
                      pnl->nneigh);
        ...
        if (srf->maxpanel[PStri]) {
            ...
            simLog(sim,
              2,
              "  triangle panels allocated: %i, defined: %i\n",
              srf->maxpanel[PStri],
              srf->npanel[PStri]);
            for (p = 0; p < srf->npanel[PStri] && p < 21; p++) {
                ...
                if (dim == 1)
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g), (%g,%g,%g), (%g,%g,%g), facing: (%g,%g,%g), "
                      "area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      ...
                      front[1],
                      front[2],
                      Geo_TriArea3(point[0], point[1], point[2], front));
                if (jumpfrnt && pnl->jumpp[PFfront])
                    simLog(sim,
                      2,
                      "; front jump: %s, %s",
        ...
        if (srf->maxpanel[PSsph]) {
            ...
            simLog(sim,
              2,
              "  sphere panels allocated: %i, defined: %i\n",
              srf->maxpanel[PSsph],
              srf->npanel[PSsph]);
            for (p = 0; p < srf->npanel[PSsph] && p < 21; p++) {
                ...
                if (dim == 1)
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g), R=%g, facing: %s, draw: %g, length: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[1][0],
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g), R=%g, facing: %s, draw: %g %g, area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[0][2],
        ...
        if (srf->maxpanel[PScyl]) {
            ...
            simLog(sim,
              2,
              "  cylinder panels allocated: %i, defined: %i\n",
              srf->maxpanel[PScyl],
              srf->npanel[PScyl]);
            for (p = 0; p < srf->npanel[PScyl] && p < 21; p++) {
                ...
                front = pnl->front;
                if (dim == 1)
                    simLog(sim, 10, "   error, cylinders are not implemented in 1-D");
                else if (dim == 2)
                    simLog(sim,
                      2,
                      "   %s: (%g,%g) to (%g,%g), R=%g, facing: %s, length: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[1][0],
                      ...
                      point[2][0],
                      front[2] == 1 ? "out" : "in",
                      2.0 * Geo_LineLength(point[0], point[1], 2));
                else
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g) to (%g,%g,%g), R=%g, facing: %s, draw: %g %g, "
                      "area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      ...
                      point[2][1],
                      point[2][2],
                      2.0 * PI * point[2][0] * Geo_LineLength(point[0], point[1], 3));
                if (jumpfrnt && pnl->jumpp[PFfront])
                    simLog(sim,
                      2,
                      "; front jump: %s, %s",
        ...
        if (srf->maxpanel[PShemi]) {
            ...
            simLog(sim,
              2,
              "  hemisphere panels allocated: %i, defined: %i\n",
              srf->maxpanel[PShemi],
              srf->npanel[PShemi]);
            for (p = 0; p < srf->npanel[PShemi] && p < 21; p++) {
                ...
                front = pnl->front;
                if (dim == 1)
                    simLog(sim, 2, "   error, hemispheres are not implemented in 1-D");
                else if (dim == 2)
                    simLog(sim,
                      2,
                      "   %s: (%g,%g), R=%g, facing: %s, opening: (%g,%g), draw: %g, "
                      "length: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g), R=%g, facing: %s, opening: (%g,%g,%g), draw: "
                      "%g %g, area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
        ...
        if (srf->maxpanel[PSdisk]) {
            ...
            simLog(sim,
              2,
              "  disk panels allocated: %i, defined: %i\n",
              srf->maxpanel[PSdisk],
              srf->npanel[PSdisk]);
            for (p = 0; p < srf->npanel[PSdisk] && p < 21; p++) {
                ...
                front = pnl->front;
                if (dim == 1)
                    simLog(sim, 10, "   error, disks are not implemented in 1-D");
                else if (dim == 2)
                    simLog(sim,
                      2,
                      "   %s: (%g,%g), R=%g, facing: (%g,%g), length: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[1][0],
                    ...
                    simLog(sim,
                      2,
                      "   %s: (%g,%g,%g), R=%g, facing: (%g,%g,%g), draw: %g, area: %g",
                      pname[p],
                      point[0][0],
                      point[0][1],
                      point[0][2],
...
/* writesurfaces */
void
writesurfaces(simptr sim, FILE* fptr)
{
    ...
    for (s = 0; s < srfss->nsrf; s++) {
                    ...
                    for (face = (enum PanelFace)0; face < 2;
                        ...
                        if (act != SAtrans && act != SAmult) {
                            ...
                            fprintf(fptr,
                              ...
                              face == PFfront ? "front" : "back",
                              surfact2string(act, string));
                            if (srf->actdetails[i][ms1][face] &&
                                srf->actdetails[i][ms1][face]->srfdatasrc[ms1] == 3)
                                fprintf(fptr,
                                  " %s",
                                  sim->mols->spname[srf->actdetails[i][ms1][face]
                                                      ->srfnewspec[ms1]]);
                            fprintf(fptr, "\n");
                        }
                    }
        ...
        if (srf->action && srf->actdetails) {
            for (i = 1; i < nspecies; i++) {
                for (ms1 = (enum MolecState)0; ms1 < MSMAX;
                    ...
                    for (face = (enum PanelFace)0; face < 3;
                         face = (enum PanelFace)(face + 1))
                        if (srf->action[i][ms1][face] == SAmult &&
                            srf->actdetails[i][ms1][face]) {
                            ...
                            for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                ...
                                if (actdetails->srfrate[ms2] > 0) {
                                    ...
                                    fprintf(fptr,
                                      ...
                                      (face == PFnone)
                                        ? molms2string(ms1, string)
                                        : (face == PFfront ? "fsoln" : "bsoln"));
                                    fprintf(fptr,
                                      " %s %g",
                                      molms2string(ms2, string),
                                      actdetails->srfrate[ms2]);
                                    if (actdetails->srfnewspec[ms2] != i)
                                        fprintf(fptr,
                                          " %s",
                                          sim->mols->spname[actdetails->srfnewspec[ms2]]);
                                    fprintf(fptr, "\n");
                                } else if (actdetails->srfrate[ms2] < 0 &&
                                           actdetails->srfprob[ms2] > 0) {
                                    fprintf(fptr,
                                      "rate_internal %s(%s)",
                                      sim->mols->spname[i],
                                      molms2string(ms1, string));
        ...
        fprintf(fptr,
          ...
          srf->bcolor[3]);
        if (srf->fshiny != 0)
            fprintf(fptr, "shininess front %g\n", srf->fshiny);
        if (srf->bshiny != 0)
            fprintf(fptr, "shininess back %g\n", srf->bshiny);
        ...
        for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1)) {
            ...
            for (p = 0; p < srf->npanel[ps]; p++) {
                ...
                if (ps == PSrect) {
                    fprintf(
                      fptr, " %c%i", pnl->front[0] > 0 ? '+' : '-', (int)(front[1]));
                    for (d = 0; d < dim; d++)
                        fprintf(fptr, " %g", point[0][d]);
                    for (d = 0; d < dim; d++)
                        if (d != pnl->front[1])
                            fprintf(
                              fptr, " %g", point[dim == 2 ? 1 : 2][d] - point[0][d]);
                } else if (ps == PStri) {
                    for (d2 = 0; d2 < dim; d2++)
                        for (d = 0; d < dim; d++)
                    ...
                    if (dim > 1)
                        fprintf(fptr, " %g", point[1][1]);
                    if (dim > 2)
                        fprintf(fptr, " %g", point[1][2]);
                } else if (ps == PScyl) {
                    for (d = 0; d < dim; d++)
                        fprintf(fptr, " %g", point[0][d]);
                    for (d = 0; d < dim; d++)
                        fprintf(fptr, " %g", point[1][d]);
                    fprintf(fptr,
                      " %c%g",
                      front[dim == 2 ? 2 : 0] > 0 ? '+' : '-',
                      point[2][0]);
                    if (dim > 2)
                        fprintf(fptr, " %g %g", point[2][1], point[2][2]);
                } else if (ps == PShemi) {
                    for (d = 0; d < dim; d++)
                        fprintf(fptr, " %g", point[0][d]);
                    fprintf(fptr, " %c%g", front[0] > 0 ? '+' : '-', point[1][0]);
                    for (d = 0; d < dim; d++)
                        fprintf(fptr, " %g", point[2][d]);
                    if (dim >= 2)
                        fprintf(fptr, " %g", point[1][1]);
                    if (dim > 2)
                        fprintf(fptr, " %g", point[1][2]);
                ...
                for (face = (enum PanelFace)0; face < 2;
                    ...
                    if (srf->action[i][MSsoln][face] == SAjump)
                        for (ps = (enum PanelShape)0; ps < PSMAX;
                            ...
                            for (p = 0; p < srf->npanel[ps]; p++) {
                                pnl = srf->panels[ps][p];
                                if (pnl->jumpp[face] && (pnl->jumpf[face] == PFfront ||
                                                          pnl->jumpf[face] == PFback))
                                    fprintf(fptr,
                                      "jump %s %s -> %s %s\n",
                                      pnl->pname,
                                      surfface2string(face, string),
                                      pnl->jumpp[face]->pname,
                                      surfface2string(pnl->jumpf[face], string));
                            }
        }
...
/* checksurfaceparams */
int
checksurfaceparams(simptr sim, int* warnptr)
{
    int error, warn;
    ...
    if (srfss->condition != SCok) { // condition
        ...
        simLog(sim,
          7,
          " WARNING: surface structure %s\n",
          simsc2string(srfss->condition, string));
    }

    if (sim->mols) // maxspecies
        if (sim->mols->maxspecies != sim->srfss->maxspecies) {
            error++;
            simLog(sim,
              10,
              " SMOLDYN BUG: number of molecule species differ between mols and srfss\n");
        }

    if (sim->mols) // nmollist
        if (srfss->nmollist != sim->mols->nlist) {
            error++;
            simLog(sim,
              10,
              " SMOLDYN BUG: mismatch between number of molecule lists in surface and in "
              "molecule structures");
        }
    ...
    if (srfss->nsrf == 0) {
        ...
        simLog(sim,
          5,
          " WARNING: Surface superstructure is defined, but no surfaces are defined\n");
    }
    if (srfss->nsrf > srfss->maxsrf) {
        error++;
        simLog(sim, 10, " SMOLDYN BUG: More surfaces are defined than allocated\n");
    }
    if (srfss->epsilon > 0.01 * syslen) {
        warn++;
        simLog(
          sim, 5, " WARNING: surface epsilon value is large compared to system size\n");
    }
    if (srfss->margin > 0.01 * syslen) {
        warn++;
        simLog(
          sim, 5, " WARNING: surface margin value is large compared to system size\n");
    }
    if (srfss->neighdist > 0.01 * syslen) {
        warn++;
        simLog(sim,
          5,
          " WARNING: surface neighbor distance value is large compared to system size\n");
    }

    for (s = 0; s < srfss->nsrf;
         s++) { // surface missing and incorrect elements, not panels
        if (strlen(srfss->snames[s]) == 0) {
            error++;
            simLog(sim, 9, " ERROR: surface %i is unnamed\n", s);
        }
        if (!srfss->srflist[s]) {
            error++;
            simLog(sim, 10, " SMOLDYN BUG: pointer to surface %i is NULL", s);
        }
        srf = srfss->srflist[s];
        if (srf->sname != srfss->snames[s]) {
            error++;
            simLog(sim,
              10,
              " SMOLDYN BUG: surface %i name pointer does not match surface "
              "superstructure pointer\n",
              s);
        }
        for (c = 0; c < 4; c++) {
            if (srf->fcolor[c] < 0 || srf->fcolor[c] > 1) {
                error++;
                simLog(sim,
                  10,
                  " SMOLDYN BUG: surface %i front color %i value is out of bounds\n",
                  s,
                  c);
            }
            if (srf->bcolor[c] < 0 || srf->bcolor[c] > 1) {
                error++;
                simLog(sim,
                  10,
                  " SMOLDYN BUG: surface %i front color %i value is out of bounds\n",
        ...
        if (srf->edgepts < 0) {
            error++;
            simLog(
              sim, 10, " SMOLDYN BUG: surface %i drawing thickness is negative\n", s);
        }
        if (srf->fdrawmode < DMno || srf->fdrawmode > DMvef) {
            error++;
            simLog(sim,
              10,
              " SMOLDYN BUG: surface %i front drawing mode is out of bounds\n",
              s);
        }
        if (srf->bdrawmode < DMno || srf->bdrawmode > DMvef) {
            error++;
            simLog(sim,
              10,
              " SMOLDYN BUG: surface %i back drawing mode is out of bounds\n",
              s);
        }
        if (srf->fshiny < 0 || srf->fshiny > 128) {
            error++;
            simLog(
              sim, 10, " SMOLDYN BUG: surface %i front shininess is out of bounds\n", s);
        }
        if (srf->bshiny < 0 || srf->bshiny > 128) {
            error++;
            simLog(
              sim, 10, " SMOLDYN BUG: surface %i back shininess is out of bounds\n", s);
        }
    }

    for (s = 0; s < srfss->nsrf;
         s++) { // surface missing and incorrect elements, about panels
        srf = srfss->srflist[s];
        for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1)) {
            if (srf->npanel[ps] > srf->maxpanel[ps]) {
                ...
                simLog(sim,
                  10,
                  " SMOLDYN BUG: surface %i has more panels of shape %i defined than "
                  "allocated\n",
                  s,
                  ps);
            }
            if (srf->npanel[ps] > 0) {
                if (!srf->pname[ps]) {
                    ...
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i has no name strings allocated for shape "
                      "%i\n",
                      s,
                      ps);
                }
                if (!srf->panels[ps]) {
                    ...
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i has panels listed but not allocated for "
                      "shape %i\n",
                      s,
                      ps);
                }
            ...
            for (p = 0; p < srf->npanel[ps]; p++) {
                if (strlen(srf->pname[ps][p]) == 0) {
                    error++;
                    simLog(sim,
                      10,
                      " ERROR: surface %i, panel shape %i, panel %i is unnamed\n",
                      s,
                      ps,
                      p);
                }
                if (!srf->panels[ps][p]) {
                    ...
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i, panel shape %i, panel %i pointer is "
                      "NULL\n",
                      s,
                      ps,
                      p);
    ...
    for (s = 0; s < srfss->nsrf; s++) { // panel missing and incorrect elements
        ...
        for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1))
            for (p = 0; p < srf->npanel[ps]; p++) {
                pnl = srf->panels[ps][p];
                if (pnl->pname != srf->pname[ps][p]) {
                    error++;
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i, panel shape %i, panel %i name pointer "
                      "does not match surface pointer\n",
                      s,
                      ps,
                      p);
                }
                if (pnl->ps != ps) {
                    ...
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i, shape %i, panel %i listing of shape "
                      "does not match actual shape\n",
                      s,
                      ps,
                      p);
                }
                if (pnl->srf != srf) {
                    ...
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i, shape %i, panel %i listing of surface "
                      "does not match actual surface\n",
                      s,
                      ps,
                      p);
                }
                if (pnl->npts != panelpoints(ps, dim)) {
                    error++;
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %i, shape %i, panel %i number of points "
                      "(npts) is incorrect\n",
                      s,
                      ps,
                      p);
                ...
                for (pt = 0; pt < pnl->npts; pt++)
                    if (!pnl->point || !pnl->point[pt]) {
                        error++;
                        simLog(sim,
                          10,
                          " SMOLDYN BUG: surface %i, shape %i, panel %i points element "
    ...
    for (s = 0; s < srfss->nsrf; s++) { // panel points and front
        ...
        for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1))
            for (p = 0; p < srf->npanel[ps]; p++) {
                ...
                if (ps == PSrect) {
                    if (dim == 1) {
                        if (point[0][0] < lowwall[0]) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: surface %s, panel %s is entirely outside the "
                              "system volume\n",
                              srf->sname,
                              pnl->pname);
                        }
                        if (point[0][0] > highwall[0]) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: surface %s, panel %s is entirely outside the "
                        ...
                        if (!(front[0] == -1 || front[0] == 1)) {
                            ...
                            simLog(sim,
                              10,
                              " SMOLDYN BUG: surface %s, panel %s front vector is set "
                              "incorrectly\n",
                              srf->sname,
                              pnl->pname);
                        }
                        ...
                        if (!Geo_LineXaabb2(
                              point[0], point[1], norm, lowwall, highwall)) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: surface %s, panel %s is entirely outside the "
        ...
        if (srf->action && nspecies > 1) {
            ...
            if (fjump)
                for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1))
                    for (p = 0; p < srf->npanel[ps]; p++) {
                        pnl = srf->panels[ps][p];
                        if (!pnl->jumpp[PFfront]) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: front of surface %s has jump action but panel "
                              "%s has no jump destination\n",
                              srf->sname,
                              pnl->pname);
                        } else if (pnl->jumpp[PFfront] == pnl) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: surface %s, panel %s front is a jump type panel "
                              "that is its own destination\n",
                              srf->sname,
                              pnl->pname);
                        }
                        if (pnl->jumpp[PFfront] && !(pnl->jumpf[PFfront] == PFfront ||
                                                     pnl->jumpf[PFfront] == PFback)) {
                            error++;
                            simLog(sim,
                              10,
                              " SMOLDYN BUG: surface %s, panel %s jumps to an undefined "
                              "panel face\n",
                              srf->sname,
                              pnl->pname);
                        }
                ...
                for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1))
                    for (p = 0; p < srf->npanel[ps]; p++) {
                        ...
                        if (!pnl->jumpp[PFback]) {
                            ...
                            simLog(sim,
                              5,
                              " WARNING: back of surface %s has jump action but panel %s "
                              "has no jump destination\n",
                              srf->sname,
                              pnl->pname);
                        } else if (pnl->jumpp[PFback] == pnl) {
                            warn++;
                            simLog(sim,
                              5,
                              " WARNING: surface %s, panel %s back is a jump type panel "
                              "that is its own destination\n",
                              srf->sname,
                              pnl->pname);
                        }
                        if (pnl->jumpp[PFback] && !(pnl->jumpf[PFback] == PFfront ||
                                                    pnl->jumpf[PFback] == PFback)) {
                            error++;
                            simLog(sim,
                              10,
                              " SMOLDYN BUG: surface %s, panel %s jumps to an undefined "
    ...
    for (s = 0; s < srfss->nsrf;
         s++) { // make sure panel panel neighbors are stored correctly
        srf = srfss->srflist[s];
        for (ps = (enum PanelShape)0; ps < PSMAX; ps = (enum PanelShape)(ps + 1))
            for (p = 0; p < srf->npanel[ps]; p++) {
                pnl = srf->panels[ps][p];
                if (pnl->nneigh > 0 && !pnl->neigh) {
                    error++;
                    simLog(sim,
                      10,
                      " SMOLDYN BUG: surface %s, panel %s has %i neighbors, but none "
                      "listed\n",
                      srf->sname,
                      pnl->pname,
                      pnl->nneigh);
    ...
    for (s = 0; s < srfss->nsrf; s++) { // make sure porting actions are linked to ports
        ...
        if (srf->action) {
            ...
            if (fport && !srf->port[PFfront]) {
                ...
                simLog(sim,
                  9,
                  " ERROR: surface %s front face has porting action, but is not linked "
                  "to a port\n",
                  srf->sname);
                for (i = 0; i < nspecies; i++)
                    if (srf->action[i][MSsoln][PFfront] == SAport)
            ...
            if (bport && !srf->port[PFback]) {
                ...
                simLog(sim,
                  9,
                  " ERROR: surface %s back face has porting action, but is not linked to "
                  "a port\n",
                  srf->sname);
                for (i = 0; i < nspecies; i++)
                    if (srf->action[i][MSsoln][PFback] == SAport)
                        ...
                        for (face = (enum PanelFace)0; face < 3;
                            ...
                            if (srf->action[i][ms][face] == SAmult) {
                                ...
                                if (!actdetails) {
                                    simLog(sim,
                                      10,
                                      " SMOLDYN BUG: action is SAmult but action details "
                                      "not allocated");
                                    error++;
                                } else {
                                    for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                        ...
                                        if (prob < 0 || prob > 1) {
                                            ...
                                            simLog(sim,
                                              10,
                                              " SMOLDYN BUG: surface interaction "
                                            ...
                                            if (actdetails->srfdatasrc[ms2] == 1)
                                                if (fabs(
                                                      (actdetails->srfrate[ms2] -
                                                        srfcalcrate(
                                                          sim, srf, i, ms, face, ms2)) /
                                                      actdetails->srfrate[ms2]) > 0.01) {
                                                    warn++;
                                                    simLog(sim,
                                                      5,
                                                      " WARNING: requested surface '%s' "
                                                      "rate for %s(%s) -> %s(%s) cannot "
                                                      "be achieved\n",
                                                      srf->sname,
                                                      sim->mols->spname[i],
                                                      molms2string(ms, string),
                    ...
                    for (i = 1; i < sim->mols->nspecies; i++)
                        if (srf->nemitter[face] && srf->nemitter[face][i])
                            if (srf->action[i][MSsoln][face] != SAreflect) {
                                ...
                                simLog(sim,
                                  5,
                                  " WARNING: surface '%s' %s-side action for %s is "
                                  "listed as %s, but molecules will absorb instead "
                                  "because of unbounded emitters\n",
                                  srf->sname,
                                  surfface2string(face, string),
                                  sim->mols->spname[i],
                                  surfact2string(srf->action[i][MSsoln][face], string2));
                            }
                }
            }
        }
                        ...
                        for (ps = (enum PanelShape)0; ps < PSMAX;
                            ...
                            for (p = 0; p < srf->npanel[ps]; p++)
                                if (srf->panels[ps][p]->emitterabsorb[face][i] == 1.0) {
                                    ...
                                    simLog(sim,
                                      5,
                                      " WARNING: surface '%s', panel %s, %s-side "
                                      "absorption probability for %s cannot be made "
                                      "large enough for accurate effective unbounded "
                                      "diffusion\n",
                                      srf->sname,
                                      srf->panels[ps][p]->pname,
                                      surfface2string(face, string),
                                      sim->mols->spname[i]);
                                }
        }
...
/* surfenablesurfaces */
int
surfenablesurfaces(simptr sim, int maxsurf)
{
    surfacessptr srfss;

    if (sim->srfss) // check for redundant function call
        if (maxsurf == -1 || sim->srfss->maxsrf == maxsurf)
            if ((sim->mols && sim->srfss->maxspecies == sim->mols->maxspecies) ||
                (!sim->mols && sim->srfss->maxspecies == 0))
                return 0;
    srfss = surfacessalloc(sim->srfss,
      maxsurf < 0 ? 5 : maxsurf,
      sim->mols ? sim->mols->maxspecies : 0,
      sim->dim);
    if (!srfss)
        return 1;
    sim->srfss = srfss;
...
int
surfexpandmaxspecies(simptr sim, int maxspecies)
{
    surfacessptr srfss;
    ...
    if (!sim->srfss)
        return 0;
    srfss = sim->srfss;
    if (srfss->maxspecies >= maxspecies)
        return 0;
...
/* surfaddsurface */
surfaceptr
surfaddsurface(simptr sim, const char* surfname)
{
    int er, s;
    surfacessptr srfss;
    surfaceptr srf;
...
/* surfsetcondition. */
void
surfsetcondition(surfacessptr surfss, enum StructCond cond, int upgrade)
{
    if (!surfss)
        return;
    if (upgrade == 0 && surfss->condition > cond)
        surfss->condition = cond;
    else if (upgrade == 1 && surfss->condition < cond)
        surfss->condition = cond;
    else if (upgrade == 2)
        surfss->condition = cond;
...
/* surfsetepsilon */
int
surfsetepsilon(simptr sim, double epsilon)
{
    int er;
...
/* surfsetmargin */
int
surfsetmargin(simptr sim, double margin)
{
    int er;
...
/* surfsetneighdist */
int
surfsetneighdist(simptr sim, double neighdist)
{
    int er;
...
/* surfsetneighhop */
int
surfsetneighhop(surfaceptr srf, int neighhop)
{
    if (!srf)
        return 1;
...
/* surfsetcolor */
int
surfsetcolor(surfaceptr srf, enum PanelFace face, double* rgba)
{
    int col;
    ...
    if (!srf)
        return 1;
    for (col = 0; col < 4; col++)
        if (rgba[col] < 0 || rgba[col] > 1)
            return 2;

    if (face == PFfront || face == PFboth)
        for (col = 0; col < 4; col++)
            srf->fcolor[col] = rgba[col];
    if (face == PFback || face == PFboth)
        for (col = 0; col < 4; col++)
            srf->bcolor[col] = rgba[col];
...
/* surfsetedgepts */
int
surfsetedgepts(surfaceptr srf, double value)
{
    if (!srf)
        return 1;
    if (value < 0)
...
/* surfsetstipple */
int
surfsetstipple(surfaceptr srf, int factor, int pattern)
{
    if (!srf)
        return 1;
    if (factor >= 0) {
...
/* surfsetdrawmode */
int
surfsetdrawmode(surfaceptr srf, enum PanelFace face, enum DrawMode dm)
{
    if (!srf)
        return 1;
    if (dm == DMnone)
...
/* surfsetshiny */
int
surfsetshiny(surfaceptr srf, enum PanelFace face, double shiny)
{
    if (!srf)
        return 1;
    if (shiny < 0 || shiny > 128)
        return 2;
    if (face == PFfront || face == PFboth)
        srf->fshiny = shiny;
    if (face == PFback || face == PFboth)
  ...
  int newident)
{
    ...
    sim = srf->srfss->sim;
    if (ms == MSbsoln || ms == MSnone)
        return 2;
    else if (ms == MSall) {
        mslo = (enum MolecState)(0);
        mshi = (enum MolecState)(MSMAX - 1);
    } else
        mslo = mshi = ms;

    if (face == PFfront || face == PFback ||
        face == PFboth) { // face is front, back, or both
        if (!(act == SAreflect || act == SAtrans || act == SAabsorb || act == SAjump ||
              act == SAport || act == SAmult))
            return 3;
  ...
  int which)
{
    ...
    sim = srf->srfss->sim;
    if (ms == MSbsoln || ms == MSall)
        return 2;

    if (ms1 > MSbsoln)
        return 3;
    else if (ms != MSsoln && ms1 != MSsoln && ms1 != MSbsoln && ms1 != ms)
        return 3;
    ...
    if ((newident != -5 && newident < 0) || newident >= srf->srfss->maxspecies)
        return 5;

    if (value < 0)
        return 6;
    else if (which == 2 && value > 1)
        return 6;
    ...
    while (1) {
        if (ident > 0) { // single species
            ...
            if (++i == sim->mols->nspecies)
                break;
        } else if (ident == 0) { // species in a group
            if (j == index[PDnresults])
                break;
            i = index[PDMAX + j];
            j++;
        }
  ...
  int which)
{
    ...
    if (ident == 0 || ident >= srf->srfss->maxspecies)
        return 1;
    else if (ident < 0) {
        ilo = 1;
        ihi = srf->srfss->maxspecies - 1;
    } else
        ilo = ihi = ident;
...
/* surfUpdateRate */
int
surfUpdateRate(simptr sim, moleculeptr mptr, enum PanelFace face, panelptr pnl)
{
        ...
        if (srf->actdetails) {
            for (f = face; f < PFboth; f = (enum PanelFace)(f + 1)) {
                ...
                for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                     ms2 = (enum MolecState)(ms2 + 1)) {
                    if (actdetails != NULL &&
                        actdetails->srfRateValueProvider[ms2] != NULL) {
                        actdetails->srfrate[ms2] = evaluateSurfActionRate(
                          sim, actdetails, ms2, mptr->pos, pnl->pname);
                        hasRateExp = true;
                    }
                }
            }
  ...
  const char* name)
{
    ...
    if (ps == PSrect) { // rectangle
        ...
        if (!(ch == '+' || ch == '-'))
            return 3;
        if (string[1] == '0' || string[1] == 'x')
            pdim = 0; // pdim is the perpendicular axis
        else if (string[1] == '1' || string[1] == 'y')
            pdim = 1;
        else if (string[1] == '2' || string[1] == 'z')
            pdim = 2;
        else
            pdim = 4;
        if (pdim >= dim)
        ...
        if (dim == 1) {
            ...
            front[1] = 0;
        } else if (dim == 2) { // params[0] and [1] are the given starting point, [2] is
                               // the length
            pt = 1;
            if (ch == '-')
                pt *= -1;
            ...
            if (params[2] < 0)
                pt *= -1;
            if (pt < 0)
                pt = 0; // pt is the point index of the given starting point
            point[pt][0] = params[0];
            point[pt][1] = params[1];
            ...
            point[2][1] =
              params[1] + (pdim == 1 ? 0 : (pdim == 0 ? params[3] : params[4]));
            ...
            if (pdim == 0)
                axis01 =
                  (pt == 1
                      ? 1
                      : 2); // axis01 is the axis parallel to edge from point 0 to point 1
            else if (pdim == 1)
                axis01 = (pt == 1 ? 2 : 0);
            else
                axis01 = (pt == 1 ? 0 : 1);
            axis12 = (axis01 + 1) %
                     3; // axis12 is the axis parallel to edge from point 1 to point 2
            if (axis12 == pdim)
                axis12 = (axis12 + 1) % 3;
    ...
    p = -1;
    if (name && name[0] != '\0') {
        for (ps2 = (enum PanelShape)0; ps2 < PSMAX && p == -1;
             ps2 = (enum PanelShape)(ps2 + 1))
            p = stringfind(srf->pname[ps2], srf->npanel[ps2], name);
        if (p != -1 && (ps2 - 1) != ps)
            return 9;
    } // panel name was used before for a different shape

    if (p == -1) { // new panel
        if (srf->npanel[ps] == srf->maxpanel[ps]) {
            ok = panelsalloc(
              srf, dim, srf->maxpanel[ps] * 2 + 1, srf->srfss->maxspecies, ps);
            if (!ok)
                return -1;
        }
        p = srf->npanel[ps]++;
...
/* surftransformpanel */
void
surftransformpanel(panelptr pnl,
  int dim,
  double* translate,
  double* origin,
  double* expand)
{
    ...
    if (dim == 1)
        ...
        isotropic = (expand[0] == expand[1]);
    else
        isotropic = ((expand[0] == expand[1] && expand[1] == expand[2]));

    if (pnl->ps == PSrect) {
        if (dim == 1)
            point[0][0] =
              origin[0] + translate[0] + (point[0][0] - origin[0]) * expand[0];
        else if (dim == 2) {
            point[0][0] =
              origin[0] + translate[0] + (point[0][0] - origin[0]) * expand[0];
            point[0][1] =
              origin[1] + translate[1] + (point[0][1] - origin[1]) * expand[1];
            point[1][0] =
              origin[0] + translate[0] + (point[1][0] - origin[0]) * expand[0];
            point[1][1] =
              origin[1] + translate[1] + (point[1][1] - origin[1]) * expand[1];
        } else {
            point[0][0] =
              origin[0] + translate[0] + (point[0][0] - origin[0]) * expand[0];
            point[0][1] =
              origin[1] + translate[1] + (point[0][1] - origin[1]) * expand[1];
            point[0][2] =
              origin[2] + translate[2] + (point[0][2] - origin[2]) * expand[2];
            point[1][0] =
              origin[0] + translate[0] + (point[1][0] - origin[0]) * expand[0];
            point[1][1] =
              origin[1] + translate[1] + (point[1][1] - origin[1]) * expand[1];
            point[1][2] =
              origin[2] + translate[2] + (point[1][2] - origin[2]) * expand[2];
            point[2][0] =
              origin[0] + translate[0] + (point[2][0] - origin[0]) * expand[0];
            point[2][1] =
              origin[1] + translate[1] + (point[2][1] - origin[1]) * expand[1];
            point[2][2] =
              origin[2] + translate[2] + (point[2][2] - origin[2]) * expand[2];
            point[3][0] =
              origin[0] + translate[0] + (point[3][0] - origin[0]) * expand[0];
            point[3][1] =
              origin[1] + translate[1] + (point[3][1] - origin[1]) * expand[1];
            point[3][2] =
              origin[2] + translate[2] + (point[3][2] - origin[2]) * expand[2];
        }
    }
        ...
        if (dim == 2) {
            ...
            if (isotropic)
                ...
                point[1][0] =
                  point[1][0] * (1.0 / 3.0 * fabs(expand[0] + expand[1] + expand[2]));
        }
    }
        ...
        if (dim == 2) {
            ...
            if (isotropic)
                ...
                point[2][0] =
                  point[2][0] * sqrt(expand[0] * expand[0] * front[0] * front[0] +
                                     expand[1] * expand[1] * front[1] * front[1]);
                Geo_LineNormal(point[0], point[1], front);
                point[3][0] = front[1];
                point[3][1] = -front[0];
                point[4][0] = -front[1];
            ...
            if (isotropic)
                ...
                point[2][0] =
                  point[2][0] * sqrt(expand[0] * expand[0] * radiusv[0] * radiusv[0] +
                                     expand[1] * expand[1] * radiusv[1] * radiusv[1] +
                                     expand[2] * expand[2] * radiusv[2] * radiusv[2]);
                lengthinv = 1.0 / Geo_LineLength(point[0], point[1], 3);
                point[3][0] = -(point[1][0] - point[0][0]) * lengthinv;
                point[3][1] = -(point[1][1] - point[0][1]) * lengthinv;
                point[3][2] = -(point[1][2] - point[0][2]) * lengthinv;
        ...
        if (dim == 2) {
            ...
            if (isotropic)
                ...
                radiusv[1] = front[0];
                point[1][0] =
                  point[1][0] * sqrt(expand[0] * expand[0] * radiusv[0] * radiusv[0] +
                                     expand[1] * expand[1] * radiusv[1] * radiusv[1]);
                point[2][0] = point[2][0] * expand[0];
                point[2][1] = point[2][1] * expand[1];
                lengthinv =
                  1.0 / sqrt(point[2][0] * point[2][0] + point[2][1] * point[2][1]);
                point[2][0] *= lengthinv;
                point[2][1] *= lengthinv;
            }
        } else {
    ...
    if (!(isotropic && expand[0] == 1))
        surfsetcondition(pnl->srf->srfss, SClists, 0);
...
/* surftranslatepanel */
void
surftranslatepanel(panelptr pnl, int dim, double* translate)
{
    int vert, nvert, d;
    double** point;
...
/* surfupdateoldpos */
void
surfupdateoldpos(surfaceptr srf, int dim)
{
    enum PanelShape ps;
    int p, i, d;
    panelptr pnl;
...
/* surftranslatesurf */
void
surftranslatesurf(surfaceptr srf, int dim, double* translate)
{
    enum PanelShape ps;
    int p;
...
surfsetemitterabsorption(simptr sim)
{
    ...
    enum PanelFace face;
    enum PanelShape ps;
    double difc, middle[DIMMAX], normal[DIMMAX], numer, denom, amount, *pos, dist,
      vdiff[DIMMAX], kappa, prob;
                        ...
                        for (ps = (enum PanelShape)0; ps < PSMAX;
                            ...
                            for (p = 0; p < srf->npanel[ps]; p++) {
                                ...
                                for (emit = 0; emit < srf->nemitter[face][i]; emit++) {
                                    ...
                                    numer += amount * dotVVD(vdiff, normal, dim) /
                                             (dist * dist * dist);
                                }
                                kappa = difc * numer / denom;
                                prob =
                                  surfaceprob(kappa, 0, sim->dt, difc, NULL, SPAirrAds);
    ...
    if (er)
        simLog(sim,
          5,
          "WARNING: an unbounded emitter is at a surface panel which will cause "
...
/* surfsetjumppanel */
int
surfsetjumppanel(surfaceptr srf,
  panelptr pnl1,
  enum PanelFace face1,
  int bidirect,
  panelptr pnl2,
  enum PanelFace face2)
{
    ...
    if (!pnl1)
        return 2;
    if (face1 != PFfront && face1 != PFback)
        return 3;
    if (bidirect != 0 && bidirect != 1)
        return 4;
    if (!pnl2 || pnl2 == pnl1 || pnl2->ps != pnl1->ps)
        return 5;
    if (face2 != PFfront && face2 != PFback)
        return 6;
  ...
  enum MolecState ms2)
{
    ...
    if (ms1 == MSsoln && face == PFnone)
        return -1; // impossible situation
    if (srf->action[i][ms1][face] != SAmult)
        return -1; // could be computed but isn't
    actdetails = srf->actdetails[i][ms1][face];
    if (!actdetails)
        return -1; // no data available
    prob = actdetails->srfprob[ms2];
    if (prob < 0 || prob > 1)
        return -2; // impossible probability
    if (prob == 0)
        return 0; // rate is 0

    srfreverseaction(
      ms1, face, ms2, &ms3, &face2, &ms4); // find probability of reverse action
    if (face2 == PFboth)
        probrev = 0;
  ...
  enum MolecState ms2)
{
    ...
    if (!actdetails)
        return -1; // no data available
    if (actdetails->srfdatasrc[ms2] == 2)
        return actdetails->srfprob[ms2];
    rate = actdetails->srfrate[ms2];
    if (rate < 0)
        return -2; // impossible rate
    if (rate == 0)
        return 0; // prob is 0
    ...
    if (ms1 == MSsoln) { // find probs
        if ((face == PFfront && ms2 == MSsoln) || (face == PFback && ms2 == MSbsoln))
            ...
            if (raterev <= 0)
                prob = surfaceprob(rate, 0, dt, difc, NULL, SPAirrTrans);
            else
                prob = surfaceprob(rate, raterev, dt, difc, NULL, SPArevTrans);
        } else { // solution to surface (adsorb)
            if (raterev <= 0)
                prob = surfaceprob(rate, 0, dt, difc, NULL, SPAirrAds);
            else
                prob = surfaceprob(rate, raterev, dt, difc, NULL, SPArevAds);
        }
    }
        ...
        if (face == PFnone) {
            if (ms2 == MSsoln || ms2 == MSbsoln) { // surface to solution (desorb)
                if (raterev <= 0)
                    prob = surfaceprob(rate, sum, dt, difc, NULL, SPAirrDes);
                else
                    prob = surfaceprob(rate, raterev, dt, difc, NULL, SPArevDes);
            } else { // surface to surface (flip)
                if (ms1 == ms2)
                    prob = 0; // same state
                else if (raterev <= 0)
                    prob = surfaceprob(rate, sum, dt, difc, NULL, SPAirrFlip);
                else
                    prob = surfaceprob(rate, raterev, dt, difc, NULL, SPArevFlip);
            }
        } else {
            if ((face == PFfront && ms2 == MSsoln) || (face == PFback && ms2 == MSbsoln))
...
/* surfsetneighbors */
int
surfsetneighbors(panelptr pnl, panelptr* neighlist, int nneigh, int add)
{
    ...
    if (add) {
        if (pnl->nneigh + nneigh > pnl->maxneigh) { // allocate more space
            ...
            if (!newneigh)
                return 1;
            for (p2 = 0; p2 < pnl->nneigh; p2++)
                newneigh[p2] = pnl->neigh[p2];
            for (; p2 < newmaxneigh; p2++)
                newneigh[p2] = NULL;
            free(pnl->neigh);
  ...
  int dim)
{
    ...
    if (srf->nemitter[face][i] ==
        srf->maxemitter[face][i]) { // allocate emitter space if needed
...
failure:
    ...
    free(newpos);
    if (ErrorType != 1)
        simLog(NULL, 10, "Unable to allocate memory in surfaddemitter");
    return 1;
}

/* surfreadstring */
surfaceptr
surfreadstring(simptr sim,
  ParseFilePtr pfp,
  surfaceptr srf,
  const char* word,
  char* line2)
{
    ...
    if (!strcmp(word, "name")) { // name
        ...
        if (face == PFnone || (i < 0 && i != -5)) { // try old format
            ...
            if (face == PFnone && (i < 0 && i != -5)) { // back to new format
                ...
                face = surfstring2face(nm2);
                CHECKS(face != PFnone,
                  "in action, face name needs to be 'front', 'back', or 'both'");
        ...
        CHECKS(act <= SAmult || act == SAadsorb,
          "in action statement, action not recognized or not permitted");
        ...
        if (face == PFnone || i < 0) { // try old format
            ...
            if (face == PFnone && i < 0) { // back to new format
                ...
                face = surfstring2face(nm2);
                CHECKS(face != PFnone,
                  "in action_rule, face name needs to be 'front', 'back', or 'both'");
    ...
    else if (!strcmp(word, "rate") ||
             !strcmp(word, "rate_internal")) { // rate, rate_internal
...
#endif
        {
            ...
            CHECKS(
              itct == 4, "format: species[(state)] state1 state2 value [new_species]");
        }
        ...
        if (ms == MSsoln) {
            ...
            CHECKS(
              ms1 == ms || ms1 == MSsoln || ms1 == MSbsoln, "state1 does not make sense");
        }
#ifdef OPTION_VCELL
        if (found != string::npos) {
            if (constRate == true)
...
#endif
        {
            if (!strcmp(word, "rate"))
                ...
                CHECKS(
                  f1 <= 1, "surface interaction probabilities cannot be greater than 1");
                er = surfsetrate(srf, i, index, ms, ms1, ms2, i3, f1, 2);
            }
            CHECKS(er != -1, "out of memory");
            CHECKS(!er, "BUG: error in surfsetrate");
        ...
        if (!strcmp(word, "rate"))
            er =
              RuleAddRule(sim, RTsurfrate, NULL, pattern, &ms, NULL, f1, detailsi, NULL);
        else {
            CHECKS(f1 <= 1, "surface interaction probabilities cannot be greater than 1");
            er = RuleAddRule(
              sim, RTsurfrateint, NULL, pattern, &ms, NULL, f1, detailsi, NULL);
        }
        ...
        CHECKS(
          face != PFnone, "in color, face name needs to be 'front', 'back', or 'both'");
        line2 = strnword(line2, 2);
        ...
        CHECKS(er == 0,
          "format is either 3 numbers or color name, and then optional alpha value");
        ...
        CHECKS(
          i2 >= 0 && i2 <= 0xFFFF, "stipple pattern needs to be between 0x00 and 0xFFFF");
        ...
        face = surfstring2face(facenm);
        CHECKS(
          face != PFnone, "in polygon, face name needs to be 'front', 'back', or 'both'");
        ...
        CHECKS(dim != 1 || ps <= PSsph,
          "in max_panels, panel shape is not permitted for a 1-D system");
        ...
        CHECKS(dim != 1 || ps <= PSsph,
          "in panel, panel shape is not permitted for a 1-D system");
        line2 = strnword(line2, 2);
        ...
        for (d = 0; d < i1; d++) {
            ...
            CHECKS(itct == 1,
              "panel either has too few parameters or has unreadable parameters");
            line2 = strnword(line2, 2);
        }
        if (line2) {
            itct = sscanf(line2, "%s", nm);
        ...
        p = 0;
        while (ps < PSMAX && (p = stringfind(srf->pname[ps], srf->npanel[ps], nm)) == -1)
            ps = (enum PanelShape)(ps + 1);
        ...
        CHECKS(
          line2 = strnword(line2, 3), "format for jump: panel1 face1 -> panel2 face2");
        itct = sscanf(line2, "%s", nm);
        ...
        if (!strcmp(nm, "->"))
            ...
            CHECKS(0, "jump operator needs to be -> or <->");
        CHECKS(
          line2 = strnword(line2, 2), "format for jump: panel1 face1 -> panel2 face2");
        itct = sscanf(line2, "%s %s", nm, facenm);
        CHECKS(itct == 2, "format for jump: panel1 face1 -> panel2 face2");
        p2 = stringfind(srf->pname[ps], srf->npanel[ps], nm);
        CHECKS(p2 >= 0,
          "second panel name listed in jump is not recognized, or not same shape as "
          "first panel");
        face2 = surfstring2face(facenm);
        CHECKS(
          face2 <= PFback, "second face listed in jump needs to be 'front' or 'back'");
        CHECKS(p2 != p, "two different panels need to be listed in a jump statement");
        er =
          surfsetjumppanel(srf, srf->panels[ps][p], face, i1, srf->panels[ps][p2], face2);
        ...
        if (!strcmp(nm, "hop"))
            i = 1;
        else if (!strcmp(nm, "stay"))
            i = 0;
        else
            CHECKS(0, "format for neighbor_action: 'hop' or 'stay'");
        ...
        for (d = 0; d < dim; d++) {
            itct = strmathsscanf(line2, "%mlg", varnames, varvalues, nvar, &fltv1[d]);
            CHECKS(
              itct == 1, "format for unbounded_emitter: face species amount position");
            line2 = strnword(line2, 2);
        }
        ...
        CHECKS(0,
          "the action_front statement has been replaced with action (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "action_back")) { // action_back
        CHECKS(0,
          "the action_back statement has been replaced with action (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "action_both")) { // action_both
        CHECKS(0,
          "the action_both statement has been replaced with action (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "polygon_front")) { // polygon_front
        CHECKS(0,
          "the polygon_front statement has been replaced with polygon (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "polygon_back")) { // polygon_back
        CHECKS(0,
          "the polygon_back statement has been replaced with polygon (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "polygon_both")) { // polygon_both
        CHECKS(0,
          "the polygon_both statement has been replaced with polygon (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "color_front") ||
             !strcmp(word, "colour_front")) { // color_front
        CHECKS(0,
          "the color_front statement has been replaced with color (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "color_back") || !strcmp(word, "colour_back")) { // color_back
        CHECKS(0,
          "the color_back statement has been replaced with color (remove the "
          "underscore)");
    }

    else if (!strcmp(word, "color_both") ||
             !strcmp(word, "colour_both")) { // color, color_both
        CHECKS(0,
          "the color_both statement has been replaced with color (remove the "
          "underscore)");
    }
...
/* loadsurface */
int
loadsurface(simptr sim, ParseFilePtr* pfpptr, char* line2)
{
    ParseFilePtr pfp;
                ...
                for (ms1 = (enum MolecState)0; ms1 < MSMAX;
                    ...
                    for (face = (enum PanelFace)0; face < 3;
                        ...
                        if (srf->action[i][ms1][face] == SAmult &&
                            ...
                            for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                 ms2 = (enum MolecState)(
                                   ms2 + 1)) // record actual probability of each event
                                actdetails->srfprob[ms2] =
                                  srfcalcprob(sim, srf, i, ms1, face, ms2);
                        }
                ...
                for (ms1 = (enum MolecState)0; ms1 < MSMAX;
                    ...
                    for (face = (enum PanelFace)0; face < 3;
                        ...
                        if (srf->action[i][ms1][face] == SAmult &&
                            ...
                            for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                 ms2 = (enum MolecState)(
                                   ms2 + 1)) // find ms1->ms1 probability
                                if (!srfsamestate(ms1, face, ms2, NULL))
                            ...
                            for (ms2 = (enum MolecState)0; ms2 < MSMAX1;
                                 ms2 = (enum MolecState)(
                                   ms2 + 1)) { // record reverse probabilities
                                srfreverseaction(ms1, face, ms2, &ms3, &face2, &ms4);
                                if (face2 != PFboth && srf->actdetails[i][ms3][face2]) {
                                    actdetails->srfrevprob[ms2] =
                                      srf->actdetails[i][ms3][face2]->srfprob[ms4];
                                }
                            }
    ...
    if (sim->mols) {
        ...
        if (srfss->nmollist) { // set srfmollist flags
            ...
            for (s = 0; s < srfss->nsrf && checksurfaces == 0; s++) {
                srf = srfss->srflist[s];
                if (srf->nemitter[PFfront] || srf->nemitter[PFback])
                    checksurfaces = 1;
                for (i = 1; i < sim->mols->nspecies && checksurfaces == 0; i++)
                    for (ms = (enum MolecState)0; ms < MSMAX && checksurfaces == 0;
                         ms = (enum MolecState)(ms + 1))
                        for (face = (enum PanelFace)0;
                             face <= PFnone && checksurfaces == 0;
                             face = (enum PanelFace)(face + 1)) {
                            act = srf->action[i][ms][face];
                            if (!(act == SAno || act == SAtrans) ||
                                srf->actdetails[i][ms][face])
                                checksurfaces = 1;
                        }
            }
            ...
            for (i = 1; i < sim->mols->nspecies; i++)
                for (ms = (enum MolecState)0; ms < MSMAX;
                    ...
                    if (checksurfaces && molismobile(sim, i, ms))
                        srfss->srfmollist[ll] =
                          (enum SMLflag)(srfss->srfmollist[ll] | SMLdiffuse);
                    if (rxnisprod(sim, i, ms, 1))
                        srfss->srfmollist[ll] =
                          (enum SMLflag)(srfss->srfmollist[ll] | SMLreact);
                    if (ms != MSsoln)
                        srfss->srfmollist[ll] =
                          (enum SMLflag)(srfss->srfmollist[ll] | SMLsrfbound);
                }
        }
        ...
        for (s = 0; s < srfss->nsrf;
             s++) { // set up molecule lists for individual surfaces
            srf = srfss->srflist[s];
...
/* surfaddmol */
int
surfaddmol(moleculeptr mptr, int ll)
{
    surfaceptr srf;

    srf = mptr->pnl->srf;
    if (srf->nmol[ll] == srf->maxmol[ll])
        if (surfexpandmollist(srf, 2 * srf->maxmol[ll] + 1, ll))
            return 1;
...
/* surfremovemol */
void
surfremovemol(moleculeptr mptr, int ll)
{
    int m;
    surfaceptr srf;
...
panelside(double* pt, panelptr pnl, int dim, double* distptr, int strict, int useoldpos)
{
    ...
    if (pnl->ps == PSrect) { // rectangle
        ...
        for (d = 0; d < dim; d++)
            dist += (pt[d] - point[0][d]) * front[d];
    } else if (pnl->ps == PSsph || pnl->ps == PShemi) { // sphere, hemisphere
        for (d = 0; d < dim; d++)
            dist += (pt[d] - point[0][d]) * (pt[d] - point[0][d]);
...
panelnormal(panelptr pnl, double* pos, enum PanelFace face, int dim, double* norm)
{
    ...
    if (pnl->ps == PSrect) {
        for (d = 0; d < dim; d++)
            norm[d] = 0;
        norm[(int)front[1]] =
          ((face == PFfront && front[0] == 1) || (face == PFback && front[0] == -1))
            ? 1.0
            : -1.0;
        ...
        Geo_SphereNormal(point[0],
          pos,
          ((face == PFfront && front[0] == 1) || (face == PFback && front[0] == -1)) ? 1
                                                                                     : -1,
          dim,
          norm);
    } else if (pnl->ps == PScyl) {
...
/* lineXpanel */
int
lineXpanel(double* pt1,
  double* pt2,
  panelptr pnl,
  int dim,
  double* crsspt,
  enum PanelFace* face1ptr,
  enum PanelFace* face2ptr,
  double* crossptr,
  double* cross2ptr,
  int* veryclose,
  int useoldpos)
{
    ...
    if (pnl->ps == PSrect) { // rectangle
        ...
        for (d = 0; d < dim; d++)
            crsspt[d] = pt1[d] + cross * (pt2[d] - pt1[d]);
        if (dim == 1)
            intsct = 1;
        else if (dim == 2) {
            d = (int)front[2];
            intsct = ((point[0][d] <= crsspt[d] && crsspt[d] <= point[1][d]) ||
                      (point[1][d] <= crsspt[d] && crsspt[d] <= point[0][d]));
        } else {
            d = (int)front[2];
            intsct = ((point[0][d] <= crsspt[d] && crsspt[d] <= point[1][d]) ||
                      (point[1][d] <= crsspt[d] && crsspt[d] <= point[0][d]));
            ...
            if (d == (int)front[1])
                d = (d + 1) % 3;
            intsct = intsct && ((point[1][d] <= crsspt[d] && crsspt[d] <= point[2][d]) ||
                                 (point[2][d] <= crsspt[d] && crsspt[d] <= point[1][d]));
        }
    }
        ...
        if (dim == 1)
            ...
            // intsct=((point[0][0]<=crsspt[0] && crsspt[0]<=point[1][0]) ||
            // (point[1][0]<=crsspt[0] && crsspt[0]<=point[0][0])); intsct=intsct &&
            // ((point[0][1]<=crsspt[1] && crsspt[1]<=point[1][1]) ||
            // (point[1][1]<=crsspt[1] && crsspt[1]<=point[0][1])); } VCELL
        ...
        if (face1 == facein && face2 == facein)
            return 0;
        cross =
          Geo_LineXSphs(pt1, pt2, point[0], point[1][0], dim, &cross2, &nrdist, &nrpos);
        if (face1 == face2 &&
            (!(nrdist <= point[1][0]) || !(nrpos >= 0) || !(nrpos <= 1)))
            return 0; // negative logic is in case of NaN returns
        if (face1 == facein)
            cross = cross2;
        for (d = 0; d < dim; d++)
            crsspt[d] = pt1[d] + cross * (pt2[d] - pt1[d]);
        if (pnl->ps == PSsph)
            ...
            dot = 0;
            for (d = 0; d < dim; d++)
                dot += (crsspt[d] - point[0][d]) * point[2][d];
            intsct = (dot <= 0);
        ...
        if (face1 == facein && face1 == face2)
            return 0;
        if (dim == 2)
            cross = Geo_LineXCyl2s(
              pt1, pt2, point[0], point[1], front, point[2][0], &cross2, &nrdist, &nrpos);
        else
            cross = Geo_LineXCyls(
              pt1, pt2, point[0], point[1], point[2][0], &cross2, &nrdist, &nrpos);
        if (face1 == face2 &&
            (!(nrdist <= point[2][0]) || !(nrpos >= 0) || !(nrpos <= 1)))
            return 0; // negative logic is in case of Nan returns
        if (face1 == facein)
            cross = cross2;
        for (d = 0; d < dim; d++)
        ...
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += (crsspt[d] - point[0][d]) * (crsspt[d] - point[0][d]);
  ...
  int* exitside)
{
    ...
    *exitside = 1;
    ...
    if (pnl->ps == PSrect) {
        ...
        if (dim == 2)
            Geo_LineExitArc2(
              pt1, pt2, point[0], point[1][0], point[2], pnledgept, exitside);
        else if (dim == 3)
        ...
        if (dim == 2) {
            ...
            Geo_LineExitCylinder(
              pt1, pt2, point[0], point[1], point[2][0], pnledgept, exitside);
    }
...
/* ptinpanel */
int
ptinpanel(double* pt, panelptr pnl, int dim)
{
    double **point, *front;
    double len2, dot;
    int intsct, d;
    ...
    if (pnl->ps == PSrect) { // rectangle
        if (dim == 1)
            ...
            if (d == (int)front[1])
                d = (d + 1) % 3;
            intsct = intsct && ((point[1][d] <= pt[d] && pt[d] <= point[2][d]) ||
                                 (point[2][d] <= pt[d] && pt[d] <= point[1][d]));
        }
    }
  ...
  enum MolecState* ms2ptr)
{
    ...
    if (act == SAmult) {
        ...
        r = randCOD();
        ms2 = MSnone;
        for (ms3 = (enum MolecState)0; ms3 < MSMAX1 && ms2 == MSnone;
             ms3 = (enum MolecState)(ms3 + 1))
            if (r < srfcumprob[ms3])
                ms2 = ms3;
        ...
        if (i2 == 0)
            ...
            if (face == PFnone) { // bound, none
                if (ms2 == ms)
                    ...
                    if (actdetails->srfrevprob[ms2] > 0)
                        act = SArevdes;
                    else
                        act = SAirrevdes;
                } else
...
/* rxnXsurface */
int
rxnXsurface(simptr sim, moleculeptr mptr1, moleculeptr mptr2)
{
    ...
    result = 0;
    for (bptr = pos2box(sim, mptr1->pos); bptr && result == 0;
         bptr = line2nextbox(sim, pos1, pos2, bptr)) {
        for (p = 0; p < bptr->npanel && result == 0; p++) {
            ...
            if (lineXpanel(pos1,
                  ...
                  NULL,
                  0)) {
                if ((ms1 == MSup || ms1 == MSdown) && pnl == mptr1->pnl)
                    result = 0;
                else if ((ms2 == MSup || ms2 == MSdown) && pnl == mptr2->pnl)
                    result = 0;
                else if (face1 != face2) { // opposite sides of a surface
                    if (rxnpt < cross || (rxnpt == cross && face1 == PFback))
                        result =
                          !(surfaction(pnl->srf, face2, i2, ms2, NULL, NULL) == SAtrans);
                    else
                        result =
                          !(surfaction(pnl->srf, face1, i1, ms1, NULL, NULL) == SAtrans);
                } else { // across a sphere, cylinder, etc.
                    facein = (face1 == PFfront) ? PFback : PFfront;
                    if (rxnpt < cross || (rxnpt == cross && face1 == PFback))
                        result =
                          !(surfaction(pnl->srf, face2, i2, ms2, NULL, NULL) == SAtrans &&
                            surfaction(pnl->srf, facein, i2, ms2, NULL, NULL) == SAtrans);
                    else if (rxnpt < cross2 || (rxnpt == cross2 && face1 == PFfront))
                        result =
                          !(surfaction(pnl->srf, face1, i1, ms1, NULL, NULL) == SAtrans &&
                            surfaction(pnl->srf, face2, i2, ms2, NULL, NULL) == SAtrans);
                    else
                        result =
                          !(surfaction(pnl->srf, face1, i1, ms1, NULL, NULL) == SAtrans &&
                            surfaction(pnl->srf, facein, i1, ms1, NULL, NULL) == SAtrans);
                }
            }
        }
    }
...
/* getpanelnormal */
void
getpanelnormal(double* pt, panelptr pnl, int dim, double* norm)
{
    int d, sign;
    ...
    if (pnl->ps == PSrect) {
        ...
        if (dim == 2) {
            dist2 = 0;
            for (d = 0; d < dim; d++)
                dist2 += (pt[d] - point[0][d]) * front[d];
            sign =
              ((dist2 > 0 && front[2] == 1) || (dist2 < 0 && front[2] == -1)) ? 1 : -1;
            norm[0] = sign * front[0];
            norm[1] = sign * front[1];
        } else if (dim == 3) {
        ...
        for (p = 0; p < bptr1->npanel; p++) {
            ...
            if (!ignore) {
                if (lineXpanel(pt,
                    ...
                    dist = 0;
                    for (d = 0; d < dim; d++)
                        dist += (pt2[d] - crsspt[d]) * (pt2[d] - crsspt[d]);
                    if (dist == 0)
                        for (d = 0; d < dim; d++)
                            dist += (pt2[d] - pt[d]) * (pt2[d] - pt[d]);
                    dist = sqrt(dist);
                    for (d = 0; d < dim; d++)
                        pt2[d] += 2 * sign * dist * norm[d];
                    iter++;
                    if (iter >= 20)
                        return 1;
                    goto moveandtest;
...
/* closestpanelpt */
int
closestpanelpt(panelptr pnl, int dim, double* testpt, double* pnlpt, double margin)
{
    double **point, *front;
    ...
    if (pnl->ps == PSrect) { // rectangle
        if (dim == 1) {
            ...
            pnlpt[d] = point[0][d];
            d = (int)front[2];
            if ((point[0][d] + margin < testpt[d] && testpt[d] < point[1][d] - margin) ||
                (point[1][d] + margin < testpt[d] && testpt[d] < point[0][d] - margin))
                pnlpt[d] = testpt[d];
            ...
            if (d == (int)front[1])
                d = (d + 1) % 3;
            if ((point[0][d] + margin < testpt[d] && testpt[d] < point[3][d] - margin) ||
                (point[3][d] + margin < testpt[d] && testpt[d] < point[0][d] - margin))
                pnlpt[d] = testpt[d];
        ...
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += (testpt[d] - point[0][d]) * point[2][d];
        if (dot < -margin) {
            Geo_NearestSpherePt(point[0], point[1][0], (int)front[0], dim, testpt, pnlpt);
            edgenum = 0;
        } else if (dim == 2) {
        ...
        if (dim == 2) {
            edgenum = Geo_NearestLineSegPt(point[0], point[1], testpt, pnlpt, 2, margin);
            dot =
              (testpt[0] - point[0][0]) * front[0] + (testpt[1] - point[0][1]) * front[1];
            dot = (dot > 0) ? 1 : -1;
            pnlpt[0] += dot * point[2][0] * front[0];
            pnlpt[1] += dot * point[2][0] * front[1];
        } else {
            edgenum = Geo_NearestCylinderPt(
              point[0], point[1], point[2][0], 3, testpt, pnlpt, margin);
        }
    }
        ...
        if (dim == 2) {
            ...
            edgenum =
              Geo_NearestLineSegPt(end1point, end2point, testpt, pnlpt, 2, margin);
        } else {
            edgenum =
              Geo_NearestDiskPt(point[0], front, point[1][0], dim, testpt, pnlpt, margin);
        }
    }
...
/* closestsurfacept */
double
closestsurfacept(surfaceptr srf,
  int dim,
  double* testpt,
  double* pnlpt,
  panelptr* pnlptr,
  boxptr bptr)
{
    ...
    if (bptr) {
        if (!bptr->npanel)
            return closestsurfacept(srf, dim, testpt, pnlpt, pnlptr, NULL);
        for (p = 0; p < bptr->npanel; p++) {
            pnl = bptr->panel[p];
            closestpanelpt(pnl, dim, testpt, pnlpt1, 0);
            dist2 = 0;
            for (d = 0; d < dim; d++)
                dist2 += (testpt[d] - pnlpt1[d]) * (testpt[d] - pnlpt1[d]);
            if (dist2 < dist2best) {
                dist2best = dist2;
                pnlbest = pnl;
                for (d = 0; d < dim; d++)
                    pnlptbest[d] = pnlpt1[d];
            }
        }
    } else
        for (ps = (enum PanelShape)(0); ps < PSMAX; ps = (enum PanelShape)(ps + 1))
...
movemol2closepanel(simptr sim, moleculeptr mptr)
{
    ...
    double pt[DIMMAX], newedgept[DIMMAX], pnledgept[DIMMAX], oldnormal[DIMMAX],
      newnormal[DIMMAX], newedgenormal[DIMMAX], newpos[DIMMAX];
    ...
    if (ptinpanel(mptr->pos, pnlx, dim)) {
        mptr->pnl = pnlx;
        return;
    } // nothing to worry about so return

    if (ptinpanel(
          mptr->posx, pnlx, dim)) // set mptr->via to starting point that was in the panel
        for (d = 0; d < dim; d++)
            mptr->via[d] = mptr->posx[d];
    else
    ...
    while (iter == 0 || !ptinpanel(mptr->pos, pnl, dim)) {
        iter++;
        er = lineexitpanel(mptr->via,
          mptr->pos,
          pnl,
          dim,
          pnledgept,
          &exitside); // pnledgept is where it goes off the edge
        if (er || iter > 50) {
        ...
        for (p = 0; p < pnl->nneigh; p++) { // figure out values for newpnl and newedgept
            edgenum = closestpanelpt(pnl->neigh[p],
              dim,
              pnledgept,
              pt,
              1000 * neighdist); // pt is point on neighbor panel's edge
            dist2 = 0;
            for (d = 0; d < dim; d++)
                dist2 += (pt[d] - pnledgept[d]) * (pt[d] - pnledgept[d]);
            if (dist2 < neighdist) {
                nn++;
        ...
        if (nn) { // rotate line to account for new normal vector
            for (d = 0; d < dim; d++)
                mptr->pos[d] +=
                  (newedgept[d] -
                    pnledgept[d]); // translate position to account for any edge mismatch
            if (dim == 2) {
                if (newedge)
                    thetasign = 1;
                else
                    thetasign = coinrandD(0.5) ? 1 : -1;
                paneledgenormal(newpnl, newedgept, dim, newedge, newedgenormal);
                len = sqrt((mptr->pos[0] - newedgept[0]) * (mptr->pos[0] - newedgept[0]) +
                           (mptr->pos[1] - newedgept[1]) * (mptr->pos[1] - newedgept[1]));
                ...
                if (newedge) { // rotate for trajectory onto the panel edge
                    ...
                    paneledgenormal(newpnl, newedgept, dim, newedge, newedgenormal);
                    dot = (mptr->pos[0] - newedgept[0]) * newedgenormal[0] +
                          (mptr->pos[1] - newedgept[1]) * newedgenormal[1] +
                          (mptr->pos[2] - newedgept[2]) * newedgenormal[2];
                    thetasign = (dot > 0) ? -1 : 1;
                    Sph_RotateVectWithNormals3D(
                      newedgept, mptr->pos, newpos, oldnormal, newnormal, thetasign);
                    if (fabs(dot) < 0.01) { // check result if there's any doubt
                        dot = (newpos[0] - newedgept[0]) * newedgenormal[0] +
                              (newpos[1] - newedgept[1]) * newedgenormal[1] +
                              (newpos[2] - newedgept[2]) * newedgenormal[2];
                        if (dot > 0) // rotated wrong way, so do it again
                            Sph_RotateVectWithNormals3D(newedgept,
                              mptr->pos,
                              newpos,
                              oldnormal,
                              newnormal,
                    ...
                    Sph_RotateVectWithNormals3D(
                      newedgept, mptr->pos, mptr->pos, NULL, newnormal, 0);
            }
            pnl = mptr->pnl = newpnl;
            ...
            dot = 0;
            for (d = 0; d < dim; d++)
                dot += (mptr->pos[d] - pnledgept[d]) * newnormal[d];
            for (d = 0; d < dim; d++)
                mptr->pos[d] -= 2.0 * newnormal[d] * dot;
            for (d = 0; d < dim; d++)
                newedgept[d] = pnledgept[d];
        }

        fixpt2panel(mptr->pos, pnl, dim, face, epsilon);
        for (d = 0; d < dim; d++)
            mptr->via[d] = newedgept[d];
        mptr->pnl = pnl;
    }
  ...
  enum PanelFace face)
{
    ...
    if (mptr->mstate == MSsoln) {
        if (pnl->ps == PSrect) {
            ...
            dot = 0;
            for (d = 0; d < dim; d++)
                dot += (pos[d] - crsspt[d]) * front[d];
            for (d = 0; d < dim; d++)
                pos[d] -= 2.0 * front[d] * dot;
        } else if (pnl->ps == PSsph || pnl->ps == PShemi) {
            Geo_SphereNormal(pnl->point[0], crsspt, 1, dim, norm);
            dot = 0;
            for (d = 0; d < dim; d++)
                dot += (pos[d] - crsspt[d]) * norm[d];
            for (d = 0; d < dim; d++)
                pos[d] -= 2.0 * norm[d] * dot;
        } else if (pnl->ps == PScyl) {
            if (dim == 2) {
                dot = 0;
                for (d = 0; d < dim; d++)
        ...
        panelnormal(mptr->pnlx,
          crsspt,
          PFfront,
          dim,
          norm2); // norm2 is normal for surface-bound panel
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += norm[d] * norm2[d];
        for (d = 0; d < dim; d++)
            norm[d] -= dot * norm2[d]; // make norm in plane of pnlx
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += norm[d] * norm[d];
        dot = sqrt(dot);
        if (dot == 0)
            dot = 1; // should never happen
        for (d = 0; d < dim; d++)
            norm[d] /= dot; // normalize norm; it is now unit vector in plane of pnlx
                            // perpendicular to reflecting surface
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += (pos[d] - crsspt[d]) * norm2[d];
        for (d = 0; d < dim; d++)
            pos[d] -= norm2[d] * dot; // put pos into plane of pnlx
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += (pos[d] - crsspt[d]) * norm[d];
        for (d = 0; d < dim; d++)
...
surfacejump(moleculeptr mptr, panelptr pnl, double* crsspt, enum PanelFace face, int dim)
{
    ...
    if (ps == PSrect) {
        ...
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += front[d] * front2[d];
        dir = dot > 0 ? 1 : -1;
    } else if (ps == PSsph || ps == PShemi) {
        for (d = 0; d < dim; d++)
            delta[d] = (crsspt[d] - point[0][d]) * point2[1][0] / point[1][0] +
                       point2[0][d] - crsspt[d];
        dir = (front[0] == front2[0]) ? 1 : -1;
    } else if (ps == PScyl) {
        for (d = 0; d < dim; d++)
            delta[d] = (crsspt[d] - point[0][d]) * point2[2][0] / point[2][0] +
                       point2[0][d] - crsspt[d];
        dir = (front[2] == front2[2]) ? 1 : -1;
    } else if (ps == PSdisk) {
        for (d = 0; d < dim; d++)
            delta[d] = point2[0][d] - point[0][d];
        dot = 0;
        for (d = 0; d < dim; d++)
            dot += front[d] * front2[d];
        dir = dot > 0 ? 1 : -1;
    ...
    dir *= (face != face2) ? 1 : -1;
    if (dir == -1)
        surfacereflect(mptr, pnl2, crsspt, dim, face);
    return 1;
}
  ...
  double* crsspt)
{
    ...
    if (face != PFnone && mptr->pnl && pnl->nneigh > 0)
        for (p = 0; p < mptr->pnl->nneigh; p++)
            if (mptr->pnl->neigh[p] == pnl)
                isneigh = 1;

    if (isneigh) { // if surface-bound && neighbor, find action, new identity, and new
                   // state
        if (mptr->pnl->srf->neighhop == 0)
            act = SAtrans;
        else
            act = coinrandD(0.5) ? SAadsorb : SAtrans;
        i2 = i;
        ms2 = ms;
    } else if ((face == PFfront || face == PFback) && pnl->emitterabsorb[face] &&
               pnl->emitterabsorb[face][i] > 0) { // emitter stuff
    ...
    if (act == SAno)
        ...
        if (ms == MSfront)
            fixpt2panel(mptr->pos, oldpnl, dim, PFfront, epsilon);
        else if (ms == MSback)
            fixpt2panel(mptr->pos, oldpnl, dim, PFback, epsilon);
        else if (ms == MSup || ms == MSdown)
            fixpt2panel(
              mptr->pos, oldpnl, dim, coinrandD(0.5) ? PFfront : PFback, epsilon);
        if (ms2 == MSfront)
            fixpt2panel(mptr->pos, mptr->pnl, dim, PFfront, epsilon);
        else if (ms2 == MSback)
            fixpt2panel(mptr->pos, mptr->pnl, dim, PFback, epsilon);
        done = 1;
    }
        ...
        x = desorbdist(
          sim->mols->difstep[i2][MSsoln], act == SArevdes ? SPArevAds : SPAirrDes);
...
/* checksurfaces1mol */
int
checksurfaces1mol(simptr sim, moleculeptr mptr, double crossminimum)
{
    ...
    while (!done) {
        if (++it > 50) {
            ...
            simLog(sim,
              7,
              "SURFACE CALCULATION ERROR: molecule could not be placed after 50 "
              "iterations\n");
            break;
        }
        ...
        facemin = PFfront;
        pnlmin = NULL;
        for (bptr1 = pos2box(sim, via); bptr1;
             bptr1 = line2nextbox(sim, via, pos, bptr1)) {
            for (p = 0; p < bptr1->npanel; p++) {
                pnl = bptr1->panel[p];
                if (pnl != mptr->pnl) {
                    lxp = lineXpanel(
                      via, pos, pnl, dim, crsspt, &face, NULL, &cross, NULL, NULL, 0);
        ...
        if (crossmin < 2) { // a panel was crossed, so deal with it
            ...
            if (flag) {
                ...
                surfUpdateRate(sim,
                  mptr,
                  facemin,
                  pnlmin); // this is a very inefficient function.  If we defined suface
                           // rate as function then we have to evaluate the rate every
                           // time step when surface activity happens
#endif
                done = dosurfinteract(sim,
                  mptr,
                  sim->mols->listtype[mptr->list] == MLTsystem ? mptr->list : -1,
                  -1,
                  pnlmin,
                  facemin,
                  crssptmin);
...
/* checksurfaces. */
int
checksurfaces(simptr sim, int ll, int reborn)
{
    ...
    for (; m < nmol; m++) {
        ...
        while (!done) {
            ...
            if (crossmin < 2) { // a panel was crossed, so deal with it
                flag =
                  (crossmin2 != crossmin && crossmin2 - crossmin < VERYCLOSE) ? 1 : 0;
                if (flag) {
                    for (d = 0; d < dim; d++)
                        pos[d] = via[d];
                    done = 1;
                    ...
                    surfUpdateRate(sim,
                      mptr,
                      facemin,
                      pnlmin); // this is a very inefficient function.  If we defined
                               // suface rate as function then we have to evaluate the
                               // rate every time step when surface activity happens
#endif
...
/* checksurfacebound. */
int
checksurfacebound(simptr sim, int ll)
{
    int nmol, m, done;
    ...
    for (m = 0; m < nmol; m++) {
        ...
        if (mptr->mstate != MSsoln) {
            ...
            if (done == -1)
                simLog(sim, 10, "Unable to allocate memory in dosurfinteract\n");
        }
    }
    return 0;
}
