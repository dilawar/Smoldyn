...
#define NSV_CALL(function)                                                     \
  simLog(                                                                      \
      NULL, 11,                                                                \
      "ERROR: NextSubvolume option not set. Recompile with OPTION_NSV = ON\n")
#endif
...
#define CHECKMEM(A)                                                            \
  if (!(A)) {                                                                  \
    ...
    (void)0
#define CHECKM(A, ...)                                                         \
  if (!(A) || strmatherror(ErrorMath, 1)) {                                    \
    ErrorType = 2;                                                             \
    snprintf(strcat(ErrorString, ErrorMath), STRCHARLONG, __VA_ARGS__);        \
    goto failure;                                                              \
  } else                                                                       \
    (void)0
#define CHECKBUG(A, ...)                                                       \
...
#define CHECKS(A, ...)                                                         \
  if (!(A)) {                                                                  \
    ErrorType = 2;                                                             \
    snprintf(ErrorString, STRCHARLONG, __VA_ARGS__) < 0 ? abort() : (void)0;   \
    goto failure;                                                              \
  } else                                                                       \
    (void)0
...
void molchangeident(simptr sim, moleculeptr mptr, int ll, int m, int i,
                    enum MolecState ms, panelptr pnl, double *crsspt);
...
// low level utilities
int filReadFilName(simptr sim, const char *str, filamenttypeptr *filtypeptr,
                   filamentptr *filptr, char *filname);
...
int filAddRandomSegments(filamentptr fil, int number, const char *xstr,
                         const char *ystr, const char *zstr, const char *thtstr,
                         const char *phistr, const char *chistr,
                         double thickness);

// structure set up
int filEnableFilaments(simptr sim);
filamenttypeptr filtypeReadString(simptr sim, ParseFilePtr pfp,
                                  filamenttypeptr filtype, const char *word,
                                  char *line2);
