 ...
 *
 * This work is distributed under the terms of the Gnu Lesser General Public
 * License (LGPL).
 *
...
/* simSetLogging */
void simSetLogging(simptr sim, const char *logfile,
                   void (*logFunction)(simptr, int, const char *, ...)) {
  FILE *logfileptr;

  if (!logfile && !logFunction) { // turn off both file and function
    if (sim) {
      if (sim->logfile && !(sim->logfile == stdout || sim->logfile == stderr))
        fclose(sim->logfile);
      sim->logfile = NULL;
      sim->logfn = NULL;
    } else {
...
void simLog(simptr sim, int importance, const char *format, ...) {
  ...
  while ((strptr = strchr(message, '|'))) { // process unit output
    ...
    ans = strunits(NULL, hasparen ? word + 2 : word + 1, 0,
                   hasparen ? word + 2 : word + 1, "getunits");
    if (ans == 1)
      strMidCat(message, strptr - message,
                strptr - message + wordlen + (hasparen ? 1 : 0), NULL, 0, 0);
    else {
      strMidCat(message, strptr - message, strptr - message + wordlen, word + 1,
                0, -1);
    }
  }

  if (sim && sim->logfn)
    (*sim->logfn)(sim, importance, message);
  else if (LoggingCallback)
  ...
  //??	if(importance>=ThrowThreshold) throw message;	//?? This is removed for
  // now because I can't statically link Libsmoldyn if it's enabled
...
void simParseError(simptr sim, ParseFilePtr pfp) {
  ...
  if (pfp) {
    ...
    snprintf(ErrorLineAndString, sizeof(ErrorLineAndString),
             "%s\nMessage: %s\n", parseerrstr, ErrorString) < 0
        ? abort()
        : (void)0;
...
enum SmolStruct simstring2ss(char *string) {
  ...
  if (!strcmp(string, "molecule"))
    ans = SSmolec;
  else if (!strcmp(string, "wall"))
    ans = SSwall;
  else if (!strcmp(string, "reaction"))
    ans = SSrxn;
  else if (!strcmp(string, "surface"))
    ans = SSsurf;
  else if (!strcmp(string, "box"))
    ans = SSbox;
  else if (!strcmp(string, "compartment"))
    ans = SScmpt;
  else if (!strcmp(string, "port"))
    ans = SSport;
  else if (!strcmp(string, "filament"))
    ans = SSfilament;
  else if (!strcmp(string, "command"))
    ans = SScmd;
  else if (!strcmp(string, "simulation"))
    ans = SSsim;
  else if (!strcmp(string, "check"))
    ans = SScheck;
...
char *simss2string(enum SmolStruct ss, char *string) {
  if (ss == SSmolec)
    strcpy(string, "molecule");
  else if (ss == SSwall)
    strcpy(string, "wall");
  else if (ss == SSrxn)
    strcpy(string, "reaction");
  else if (ss == SSsurf)
    strcpy(string, "surface");
  else if (ss == SSbox)
    strcpy(string, "box");
  else if (ss == SScmpt)
    strcpy(string, "compartment");
  else if (ss == SSport)
    strcpy(string, "port");
  else if (ss == SSfilament)
    strcpy(string, "filament");
  else if (ss == SScmd)
    strcpy(string, "command");
  else if (ss == SSsim)
    strcpy(string, "simulation");
  else if (ss == SScheck)
    strcpy(string, "check");
  else if (ss == SSall)
    strcpy(string, "all");
...
char *simsc2string(enum StructCond sc, char *string) {
  if (sc == SCinit)
    strcpy(string, "not initialized");
  else if (sc == SClists)
    strcpy(string, "lists need updating");
  else if (sc == SCparams)
    strcpy(string, "parameters need updating");
  else if (sc == SCok)
    strcpy(string, "fully updated");
  else
    strcpy(string, "none");
  return string;
}
...
simptr simalloc(const char *fileroot) {
  ...
  for (et = (EventType)0; et < ETMAX; et = (EventType)(et + 1))
    sim->eventcount[et] = 0;
  sim->maxvar = 0;
  sim->nvar = 0;
  sim->varnames = NULL;
...
int simexpandvariables(simptr sim, int spaces) {
  ...
  if (!newvarvalues)
    return 1;
  for (i = 0; i < sim->nvar && i < newmaxvar; i++)
    newvarnames[i] = sim->varnames[i];
  for (i = 0; i < sim->nvar && i < newmaxvar; i++)
    newvarvalues[i] = sim->varvalues[i];
  newnvar = i;
...
void simoutput(simptr sim) {
  ...
  if (string[0])
    simLog(sim, 2, " units: %s\n", string);
  if (sim->accur < 10)
    simLog(sim, 2, " Accuracy level: %g\n", sim->accur);
  else
    simLog(sim, 1, " Accuracy level: %g\n", sim->accur);
  ...
  simLog(sim, 2, " Time from %g|T to %g|T step %g|T\n", sim->tmin, sim->tmax,
         sim->dt);
  if (sim->time != sim->tmin)
    simLog(sim, 2, " Current time: %g|T\n", sim->time);
  simLog(sim, 2, "\n");
  return;
}
...
void writesim(simptr sim, FILE *fptr) {
  ...
  if (sim->boxs->mpbox)
    fprintf(fptr, "molperbox %g\n", sim->boxs->mpbox);
  else if (sim->boxs->boxsize)
    fprintf(fptr, "boxsize %g\n", sim->boxs->boxsize);
  fprintf(fptr, "\n");
  return;
}
...
int checksimparams(simptr sim) {
  ...
  error += checkmolparams(sim, &warndiff);
  warn += warndiff;
  error += checkboxparams(sim, &warndiff);
  warn += warndiff;
  error += checkwallparams(sim, &warndiff);
  warn += warndiff;
  error += checkrxnparams(sim, &warndiff);
  warn += warndiff;
  error += checkruleparams(sim, &warndiff);
  warn += warndiff;
  error += checksurfaceparams(sim, &warndiff);
  warn += warndiff;
  error += checkcompartparams(sim, &warndiff);
  warn += warndiff;
  error += checkportparams(sim, &warndiff);
  warn += warndiff;
  error += checklatticeparams(sim, &warndiff);
  warn += warndiff;
  error += filCheckParams(sim, &warndiff);
  warn += warndiff;
  error += checkgraphicsparams(sim, &warndiff);
  warn += warndiff;
  error += checkbngparams(sim, &warndiff);
  warn += warndiff;
  ...
  if (error > 0)
    simLog(sim, 2, " %i total errors\n", error);
  else
    simLog(sim, 2, " No errors\n");
  if (warn > 0)
    simLog(sim, 2, " %i total warnings\n", warn);
  else
    simLog(sim, 2, " No warnings\n");
  simLog(sim, 2, "\n");
  return error;
}
...
void simsetcondition(simptr sim, enum StructCond cond, int upgrade) {
  if (!sim)
    return;
  if (upgrade == 0 && sim->condition > cond)
    sim->condition = cond;
  else if (upgrade == 1 && sim->condition < cond)
    sim->condition = cond;
  else if (upgrade == 2)
    sim->condition = cond;
  return;
...
int simreadstring(simptr sim, ParseFilePtr pfp, const char *word, char *line2) {
  char nm[STRCHAR], nm1[STRCHAR], shapenm[STRCHAR], ch, rname[STRCHAR],
      fname[STRCHAR], pattern[STRCHAR];
  char str1[STRCHAR], str2[STRCHAR], str3[STRCHAR], str4[STRCHAR],
      str5[STRCHAR], str6[STRCHAR];
  char errstr[STRCHARLONG];
  int er, i, nmol, d, i1, s, c, ll, order, *index, ft;
  int rulelist[MAXORDER + MAXPRODUCT], r, ord, rct, prd, itct, prt, lt,
      detailsi[8];
  ...
  if (!strcmp(word, "variable")) { // variable
    ...
    CHECKS(strcmp(nm, "time"), "'time' cannot be used as a variable name; it "
                               "is pre-defined as the simulation time");
    CHECKS(strcmp(nm, "x") && strcmp(nm, "y") && strcmp(nm, "z") &&
               strcmp(nm, "r"),
           "x, y, z, and r are reserved variable names");
    CHECKS(strokname(nm), "variable name has to start with a letter and then "
                          "be alphanumeric with optional underscores");
  ...
  // space and time
    ...
    itct = strmathsscanf(line2, "%s %mlg|L %mlg|L", varnames, varvalues, nvar,
                         nm, &flt1, &flt2);
    CHECKM(itct == 3,
           "boundaries format: dimension position position [type]. ");
    if (!strcmp(nm, "0") || !strcmp(nm, "x"))
      d = 0;
    else if (!strcmp(nm, "1") || !strcmp(nm, "y"))
      d = 1;
    else if (!strcmp(nm, "2") || !strcmp(nm, "z"))
      d = 2;
    else
      d = 3;
    ...
    CHECKS(
        flt1 < flt2,
        "the first boundary position needs to be smaller than the second one");
    line2 = strnword(line2, 4);
    ch = 't';
    if (line2) {
      itct = sscanf(line2, "%c", &ch);
      CHECKS(itct == 1, "boundary type character could not be read");
      CHECKS(ch == 'r' || ch == 'p' || ch == 'a' || ch == 't',
             "boundaries type needs to be r, p, a, or t");
      line2 = strnword(line2, 2);
    }
    ...
    itct = strmathsscanf(line2, "%s %mlg|L %c", varnames, varvalues, nvar, nm,
                         &flt1, &ch);
    ...
    if (!strcmp(nm, "0") || !strcmp(nm, "x"))
      ...
      d = 3;
    CHECKS(d >= 0 && d < dim,
           "low_wall dimension needs to be between 0 and dim-1");
    CHECKS(ch == 'r' || ch == 'p' || ch == 'a' || ch == 't',
           "low_wall type needs to be r, p, a, or t");
    er = walladd(sim, d, 0, flt1, ch);
    CHECKS(er != 1, "out of memory adding wall");
    CHECKS(er != 2, "SMOLDYN BUG: adding wall");
    ...
    if (!strcmp(nm, "0") || !strcmp(nm, "x"))
      ...
      d = 3;
    CHECKS(d >= 0 && d < dim,
           "high_wall dimension needs to be between 0 and dim-1");
    CHECKS(ch == 'r' || ch == 'p' || ch == 'a' || ch == 't',
           "high_wall type needs to be r, p, a, or t");
  ...
  // molecules
  ...
  else if (!strcmp(word, "max_species") ||
           !strcmp(word, "max_names")) { // max_species, max_names
  ...
  else if (!strcmp(word, "species") || !strcmp(word, "names") ||
           !strcmp(word, "name")) { // species, names, name
    while (line2) {
      itct = sscanf(line2, "%s", nm);
    ...
    CHECKS(er != 1, "out of memory");
    CHECKS(er != 5,
           "more molecule already exist than are requested with max_mol");
    ...
    for (d = 0; d < dim; d++) {
      itct =
          strmathsscanf(line2, "%mlg|L/T", varnames, varvalues, nvar, &v1[d]);
    ...
    CHECKS(sim->srfss->nsrf > 0,
           "at least one surface needs to be defined before surface_drift");
    er = molstring2index1(sim, line2, &ms, &index);
    CHECKS(er != -1,
           "surface_drift format: species(state) surface panel_shape values");
    CHECKS(er != -2,
           "mismatched or improper parentheses around molecule state");
    CHECKS(er != -3, "cannot read molecule state value");
    CHECKS(er != -4, "molecule name not recognized");
    ...
    CHECKS(molsetsurfdrift(sim, 0, index, ms, s, ps, v1) == 0,
           "out of memory in surface_drift");
    ...
    CHECKS(
        sim->srfss->nsrf > 0,
        "at least one surface needs to be defined before surface_drift_rule");
    er = molstring2pattern(line2, &ms, pattern, 0);
    CHECKS(
        er != -1,
        "surface_drift_rule format: species(state) surface panel_shape values");
    CHECKS(er != -2,
           "mismatched or improper parentheses around molecule state");
    CHECKS(er != -3, "cannot read molecule state value");
    CHECKS(ms < MSMAX || ms == MSall, "invalid state");
    ...
    ps = surfstring2ps(shapenm);
    CHECKS(ps != PSnone,
           "in surface_drift_rule, panel shape name not recognized");
    CHECKS(line2 = strnword(line2, 3), "missing vector in surface_drift");
    for (d = 0; d < dim - 1; d++) {
      itct =
          strmathsscanf(line2, "%mlg|L/T", varnames, varvalues, nvar, &v1[d]);
    ...
    detailsi[0] = s;
    detailsi[1] = (int)ps;
    er = RuleAddRule(sim, RTsurfdrift, NULL, pattern, &ms, NULL, 0, detailsi,
                     v1);
    ...
    CHECKS(line2 = strnword(line2, 2),
           "mol format: number name position_vector");
    ...
    for (d = 0; d < dim; d++) {
      ...
      if (line2[0] == 'u') {
        CHECKS(sim->wlist,
               "need to enter boundaries before using 'u' in a mol statement");
      } else {
        ...
        if (itct == 2) {
          ...
          itct =
              strmathsscanf(line2, "%mlg|L", varnames, varvalues, nvar, &flt1);
    ...
    CHECKS(sim->srfss,
           "surfaces need to be defined before surface_mol statement");
    CHECKS(sim->srfss->nsrf > 0,
           "at least one surface needs to be defined before surface_mol");
    itct = strmathsscanf(line2, "%mi", varnames, varvalues, nvar, &nmol);
    CHECKM(itct == 1, "surface_mol format: nmol species(state) surface "
                      "panel_shape panel_name [position]. ");
    CHECKS(nmol >= 0,
           "in surface_mol, the number of molecules needs to be at least 0");
    CHECKS(line2 = strnword(line2, 2),
           "surface_mol format: nmol species(state) surface panel_shape "
           "panel_name [position]");
    i = molstring2index1(sim, line2, &ms, NULL);
    CHECKS(i != 0, "wildcards and species groups are not permitted");
    CHECKS(i != -1, "error reading molecule name");
    ...
    CHECKS(i != -6, "cannot read wildcard logic");
    CHECKS(i != -7, "out of memory");
    CHECKS(ms < MSMAX && ms != MSsoln,
           "state needs to be front, back, up, or down");
    CHECKS(line2 = strnword(line2, 2),
           "surface_mol format: nmol species(state) surface panel_shape "
           "panel_name [position]");
    itct = sscanf(line2, "%s %s %s", nm, shapenm, nm1);
    CHECKS(itct == 3, "surface_mol format: nmol species(state) surface "
                      "panel_shape panel_name [position]");
    if (!strcmp(nm, "all"))
      s = -1;
    else {
    ...
    if (line2) {
      for (d = 0; d < dim; d++) {
        itct =
            strmathsscanf(line2, "%mlg|L", varnames, varvalues, nvar, &v1[d]);
        CHECKM(itct == 1, "incomplete vector in drift. ");
        line2 = strnword(line2, 2);
      }
      CHECKS(s >= 0 && ps != PSall && strcmp(nm1, "all"),
             "in surface_mol, use of coordinates requires that a specific "
             "panel be specified");
      er = addsurfmol(sim, nmol, i, ms, v1, NULL, s, ps, nm1);
      CHECKS(er != 1,
             "in surface_mol, unable to allocate temporary storage space");
    ...
    CHECKS(sim->cmptss,
           "compartments need to be defined before compartment_mol statement");
    CHECKS(
        sim->cmptss->ncmpt > 0,
        "at least one compartment needs to be defined before compartment_mol");
    itct = strmathsscanf(line2, "%mi", varnames, varvalues, nvar, &nmol);
    ...
    CHECKS(line2 = strnword(line2, 2),
           "compartment_mol format: nmol species compartment");
    i = molstring2index1(sim, line2, &ms, NULL);
    CHECKS(i != 0, "wildcards and species groups are not permitted");
    CHECKS(i != -1, "error reading molecule name");
    CHECKS(i != -2, "mismatched or improper parentheses around molecule state");
    ...
    CHECKS(sim->mols && sim->mols->nlist > 0,
           "need to enter molecule_lists before mol_list");
    er = molstring2index1(sim, line2, &ms, &index);
    ...
    CHECKS(ll >= 0, "molecule list name is not recognized");
    detailsi[0] = ll;
    er = RuleAddRule(sim, RTmollist, NULL, pattern, &ms, NULL, 0, detailsi,
                     NULL);
  ...
  // graphics
    ...
    if (strbegin(nm, "yes", 0) || !strcmp(nm, "1"))
      sim->quitatend = 1;
    else if (strbegin(nm, "no", 0) || !strcmp(nm, "0"))
      sim->quitatend = 0;
    else
      CHECKS(0, "cannot read quit_at_end value");
  ...
  else if (!strcmp(word, "frame_color") ||
           !strcmp(word, "frame_colour")) { // frame_color
    ...
    CHECKS(er == 0, "format is either 3 numbers or color name, and then "
  ...
  else if (!strcmp(word, "grid_color") ||
           !strcmp(word, "grid_colour")) { // grid_color
    er = graphicsreadcolor(&line2, v2);
    CHECKS(er != 3, "color values need to be between 0 and 1");
    CHECKS(er != 4, "color name not recognized");
    CHECKS(er != 6, "alpha values need to be between 0 and 1");
  ...
  else if (!strcmp(word, "background_color") ||
           !strcmp(word, "background_colour")) { // background_color
    er = graphicsreadcolor(&line2, v2);
    CHECKS(er != 3, "color values need to be between 0 and 1");
    CHECKS(er != 4, "color name not recognized");
    CHECKS(er != 6, "alpha values need to be between 0 and 1");
  ...
  else if (!strcmp(word, "text_color") ||
           !strcmp(word, "text_colour")) { // text_color
    er = graphicsreadcolor(&line2, v2);
    CHECKS(er != 3, "color values need to be between 0 and 1");
    CHECKS(er != 4, "color name not recognized");
    CHECKS(er != 6, "alpha values need to be between 0 and 1");
    ...
    while (line2) {
      ...
      CHECKS(er != 1, "out of memory");
      CHECKS(er != 2, "unrecognized text display item (check that species have "
                      "been defined)");
      line2 = strnword(line2, 2);
    }
    ...
    if (!strcmp(nm, "global") || !strcmp(nm, "room"))
      lt = -1;
    else {
    ...
    line2 = strnword(line2, 2);
    if (ltparam == LPon || ltparam == LPoff || ltparam == LPauto)
      v2[0] = 0;
    else if (ltparam == LPambient || ltparam == LPdiffuse ||
             ltparam == LPspecular) {
      CHECKS(line2, "light format: light_number parameter values");
      ...
      for (i1 = 0; i1 < 4; i1++)
        CHECKS(v2[i1] >= 0 && v2[i1] <= 1,
               "light color values need to be between 0 and 1");
    } else if (ltparam == LPposition) {
      CHECKS(line2, "light format: light_number parameter values");
      itct = strmathsscanf(line2, "%mlg|L %mlg|L %mlg|L", varnames, varvalues,
                           nvar, &v2[0], &v2[1], &v2[2]);
    ...
    er = RuleAddRule(sim, RTdispsize, NULL, pattern, &ms, NULL, flt1, NULL,
                     NULL);
  ...
  else if (!strcmp(word, "color_rule") ||
           !strcmp(word, "colour_rule")) { // color_rule
    er = molstring2pattern(line2, &ms, pattern, 0);
    CHECKS(er != -1, "color format: species[(state)] color");
    CHECKS(er != -2,
           "mismatched or improper parentheses around molecule state");
  ...
  // about runtime commands
  ...
  else if (!strcmp(word, "output_files") ||
           !strcmp(word, "output_file")) { // output_files
    ...
    CHECKS(er != 4, "command uses integer timing type but has non-integer "
                    "parameter values");
    ...
    CHECKS(0,
           "max_cmd is an obsolete statement. It simply needs to be removed.");
  }
  ...
  // surfaces
    ...
    CHECKS(sim->srfss,
           "individual surfaces need to be defined before using surface");
    itct = sscanf(line2, "%s %s", nm, nm1);
    CHECKS(itct == 2,
           "surface format: surface_name statement_name statement_text");
  ...
  // compartments
    ...
    CHECKS(
        sim->cmptss,
        "individual compartments need to be defined before using compartment");
    itct = sscanf(line2, "%s %s", nm, nm1);
    CHECKS(itct == 2,
           "compartment format: compart_name statement_name statement_text");
    line2 = strnword(line2, 3);
    CHECKS(line2,
           "compartment format: compart_name statement_name statement_text");
    c = stringfind(sim->cmptss->cnames, sim->cmptss->ncmpt, nm);
  ...
  // ports
    ...
    CHECKS(sim->portss,
           "individual ports need to be defined before using port");
    itct = sscanf(line2, "%s %s", nm, nm1);
  ...
  // filaments
    ...
    CHECKS(sim->filss, "individual filament types need to be defined before "
                       "using filament_type");
    itct = sscanf(line2, "%s %s", nm, nm1);
    CHECKS(itct == 2, "filament_type format: filament_type_name statement_name "
                      "statement_text");
    line2 = strnword(line2, 3);
    CHECKS(line2, "filament_type format: filament_type_name statement_name "
                  "statement_text");
    ...
    CHECKS(sim->filss,
           "individual filaments need to be defined before using filament");
    itct = sscanf(line2, "%s", nm);
    CHECKS(itct == 1,
           "filament format: type:name statement_name statement_text");
    filtype = NULL;
    ...
    CHECKS(filtype, "missing filament type. Format: type:name statement_name "
                    "statement_text");
    line2 = strnword(line2, 2);
    CHECKS(line2,
           "filament format: filament_name statement_name statement_text");
    itct = sscanf(line2, "%s", nm);
    CHECKS(itct == 1,
           "filament format: type:name statement_name statement_text");
    line2 = strnword(line2, 2);
    ...
    itct = sscanf(line2, "%s", nm);
    CHECKS(itct == 1, "random_filament format: type:name segments [x y z phi "
                      "theta psi] [thickness]");
    filtype = NULL;
    er = filReadFilName(sim, nm, &filtype, &fil, nm1);
    CHECKS(er != -1 && er != -3, "cannot read filament name");
    CHECKS(er != -4, "unknonwn filament type");
    CHECKS(filtype, "missing filament type. Format: type:name segments [x y z "
                    "phi theta psi] [thickness]");
    CHECKS(filtype->klen == -1 || filtype->klen > 0,
           "cannot compute random segments because the filament type has "
           "length force constant equals 0");
    fil = filAddFilament(filtype, nm1);
    CHECKS(fil, "unable to add filament to simulation");
    line2 = strnword(line2, 2);

    CHECKS(line2, "random_filament format: type:name segments [x y z phi theta "
                  "psi] [thickness]");
    itct = strmathsscanf(line2, "%mi", varnames, varvalues, nvar,
                         &i1); // number of segments
    CHECKM(itct == 1, "random_filament format: type:name segments [x y z phi "
                      "theta psi] [thickness]. ");
    CHECKS(i1 > 0, "number of segments needs to be >0");
    line2 = strnword(line2, 2);
    if (fil->nseg == 0 && dim == 3) {
      CHECKS(line2, "missing position and angle information");
      itct = sscanf(line2, "%s %s %s %s %s %s", str1, str2, str3, str4, str5,
                    str6);
      CHECKS(itct == 6, "random_filament format: type:name number [x y z phi "
                        "theta psi] [thickness]");
      ...
      CHECKS(itct == 3,
             "random_filament format: type:name number [x y phi] [thickness]");
      sprintf(str3, "%i", 0);
      sprintf(str5, "%i", 0);
      sprintf(str6, "%i", 0);
      line2 = strnword(line2, 4);
    ...
    if (line2) {
      ...
      CHECKM(itct == 1, "random_segments format: type:name number [x y z phi "
                        "theta psi] [thickness]. ");
      CHECKS(thick > 0, "thickness needs to be >0");
      line2 = strnword(line2, 2);
    }
    er = filAddRandomSegments(fil, i1, str1, str2, str3, str4, str5, str6,
                              thick);
  ...
  // reactions
  ...
  else if (!strcmp(word, "reaction") || !strcmp(word, "reaction_rule") ||
           !strcmp(word, "reaction_cmpt") ||
           !strcmp(word,
                   "reaction_surface")) { // reaction, reaction_rule,
                                          // reaction_cmpt, reaction_surface
    ...
    r = readrxnname(sim, rname, &order, NULL, NULL,
                    1); // just get the order value
    if (r < 0)
      r = readrxnname(sim, rname, &order, NULL, NULL, 2);
    if (r < 0)
      r = readrxnname(sim, rname, &order, NULL, NULL, 3);
    CHECKS(r >= 0, "unrecognized reaction name");

    if (order == 0 && dim == 1)
      itct = strmathsscanf(line2, "%s %mlg|L-1/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 0 && dim == 2)
      itct = strmathsscanf(line2, "%s %mlg|L-2/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 0 && dim == 3)
      itct = strmathsscanf(line2, "%s %mlg|L-3/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 1)
      itct = strmathsscanf(line2, "%s %mlg|T-1", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 2 && dim == 1)
      itct = strmathsscanf(line2, "%s %mlg|L/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 2 && dim == 2)
      itct = strmathsscanf(line2, "%s %mlg|L2/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else if (order == 2 && dim == 3)
      itct = strmathsscanf(line2, "%s %mlg|L3/T", varnames, varvalues, nvar,
                           rname, &flt1);
    else {
      CHECKS(0, "BUG in reaction_rate: order and dim are impossible");
    }
    ...
    itct =
        strmathsscanf(line2, "%s %mi", varnames, varvalues, nvar, rname, &i1);
    ...
    CHECKS(rxn, "unrecognized reaction name");
    CHECKS(!strnword(line2, 3),
           "unexpected text following reaction_multiplicity");
  }

  else if (!strcmp(word, "confspread_radius")) { // confspread_radius
    itct = strmathsscanf(line2, "%s %mlg|L", varnames, varvalues, nvar, rname,
                         &flt1);
    ...
    itct = strmathsscanf(line2, "%s %mlg|", varnames, varvalues, nvar, rname,
                         &flt1);
    ...
    CHECKS(rxn, "unrecognized reaction name");
    CHECKS(!strnword(line2, 3),
           "unexpected text following reaction_probability");
  }
    ...
    CHECKS(flt1 != 0 && flt1 < 1,
           "reaction chi value must be between 0 and 1 (or -1 to disable)");
    rxn = NULL;
    r = readrxnname(sim, rname, &order, &rxn, NULL, 1);
    if (r >= 0) {
      CHECKS(order == 2,
             "reaction chi value is only allowed for order 2 reactions");
    ...
    CHECKS(rxn, "unrecognized reaction name");
    CHECKS(!strnword(line2, 3),
           "unexpected text following reaction_production");
  }
    ...
    for (ord = 0; ord < order; ord++) {
      CHECKS(line2 = strnword(line2, 2),
             "missing state term in reaction_permit");
      itct = sscanf(line2, "%s", nm1);
    ...
    for (ord = 0; ord < order; ord++) {
      CHECKS(line2 = strnword(line2, 2),
             "missing state term in reaction_forbid");
      itct = sscanf(line2, "%s", nm1);
      rctstate[ord] = molstring2ms(nm1);
    }
    ...
    if (!rxn)
      readrxnname(sim, rname, &order, &rxn, NULL, 2);
    if (!rxn)
      readrxnname(sim, rname, &order, &rxn, NULL, 3);
    CHECKS(rxn, "unrecognized reaction name");
    ...
    CHECKS(rxn, "unrecognized reaction name");
    CHECKS(order == 2,
           "reaction_intersurface is only for bimolecular reactions");
    if (rxn->nprod == 0) {
      line2 = strnword(line2, 2);
      ...
      if (!strcmp(nm, "on"))
        rulelist[0] = 0;
      else if (!strcmp(nm, "off"))
        rulelist[0] = -1;
      else
        CHECKS(0, "value not recognized");
    } else {
      for (prd = 0; prd < rxn->nprod; prd++) {
        line2 = strnword(line2, 2);
        CHECKS(line2, "not enough product codes entered");
        ...
        if (!strcmp(nm, "+"))
          ...
          if (nm[1] == '1')
            ...
            CHECKS(0, "error reading a reactant code");
        } else
    ...
    CHECKS(!strnword(line2, 2),
           "unexpected text following reaction_intersurface");
  }
    ...
    for (prd = 0; prd < rxn->nprod; prd++) {
      ...
      if (!strcmp(nm, "+"))
        ...
        CHECKS(replist[order + prd] != SRboth,
               "product representation cannot be 'both'");
    ...
    if (r >= 0) {
      for (i = 0; i < vlist->n; i++) {
        er = RxnSetRepresentationRules((rxnptr)vlist->xs[i], order, replist,
                                       replist + order);
        CHECKS(er != 1,
               "out of memory allocating reaction representation rules");
      }
      ListFreeV(vlist);
    }
    r = readrxnname(sim, rname, &order, &rxn, NULL, 3);
    ...
    CHECKS(!strnword(line2, 2),
           "unexpected text following reaction_representation");
  }
    ...
    CHECKS(
        !(rpart == RPbounce && (order != 2 || rxn->nprod < 2)),
        "bounce can only be used with two reactants and at least two products");
    flt1 = 0;
    prd = 0;
    for (d = 0; d < sim->dim; d++)
      v1[prd] = 0;
    ...
    if (rpart == RPirrev)
    ...
    else if (rpart == RPpgem || rpart == RPbounce || rpart == RPpgemmax ||
             rpart == RPpgemmaxw || rpart == RPratio || rpart == RPpgem2 ||
             rpart == RPpgemmax2 || rpart == RPratio2) {
      ...
      if (strbegin("product_", nm1, 0)) {
        ...
        CHECKS(i != -2,
               "mismatched or improper parentheses around product state");
        CHECKS(i != -3, "cannot read molecule state value");
        CHECKS(i != -4, "molecule name not recognized");
        ...
        for (prd = 0; prd < rxn->nprod && rxn->prdident[prd] != i &&
                      rxn->prdstate[prd] != ms;
             prd++)
          ;
        CHECKS(
            prd < rxn->nprod,
            "molecule in product_placement is not a product of this reaction");
      }
      CHECKS(line2 = strnword(line2, 2),
             "position vector missing for product_placement");
      for (d = 0; d < dim; d++) {
        itct =
            strmathsscanf(line2, "%mlg|L", varnames, varvalues, nvar, &v1[d]);
        CHECKM(itct == 1,
               "insufficient data for position vector for product_placement. ");
        line2 = strnword(line2, 2);
      }
    } else
      CHECKS(0, "unrecognized or not permitted product placement parameter");
    ...
    if (r >= 0) {
      for (i = 0; i < vlist->n; i++) {
        i1 = RxnSetRevparam(sim, (rxnptr)vlist->xs[i], rpart, flt1, prd, v1,
                            dim);
        CHECKS(i1 != 2, "reversible parameter value is out of bounds");
      }
      ListFreeV(vlist);
    ...
    if (!strcmp(nm, "all"))
      i = -1;
    else if (!strcmp(nm, "otf") || !strcmp(nm, "on-the-fly"))
      i = -2;
    else {
    ...
    CHECKS(er != -5,
           "generated species name exceeded maximum allowed length of %i "
           "characters",
           STRCHAR);
  ...
  else if (!strcmp(word, "rand_seed") ||
           !strcmp(word, "random_seed")) { // rand_seed, random_seed
    itct = sscanf(line2, "%s", nm);
    if (!strcmp(nm, "time"))
  ...
  else if (!strcmp(word, "neighbor_dist") ||
           !strcmp(word, "neighbour_dist")) { // neighbor_dist
...
int loadsim(simptr sim, const char *fileroot, const char *filename,
  ...
  if (fileroot)
    strncpy(sim->filepath, fileroot, STRCHARLONG);
  if (filename)
    strncpy(sim->filename, filename, STRCHARLONG);
  if (flags) {
  ...
  while (!done) {
    ...
    if (pfpcode == 0)
      ...
      er = loadJMS(sim, &pfp, line2, errstring);
                 ...
                 word,
                 "start_highResVolumeSamples")) { // highResVolumeSamplesFile
      ...
      CHECKS(
          1,
          "reactions cannot be entered in blocks; this feature was deprecated");
    }
...
int simupdate(simptr sim) {
  ...
  if (sim->condition == SCinit &&
      (sim->rxnss[0] || sim->rxnss[1] || sim->rxnss[2]))
...
int simInitAndLoad(const char *fileroot, const char *filename, simptr *smptr,
  ...
  if (!sim) {
    ...
    CHECKMEM(sim);
    sim->valueProviderFactory =
        valueProviderFactory; // create value provider factory
    sim->mesh = mesh;
...
/* simInitAndLoad */
int simInitAndLoad(const char *fileroot, const char *filename, simptr *smptr,
                   const char *flags, const char *logfile) {
  simptr sim;
  int er;
  ...
  if (!sim) {
    ...
    strncpy(sim->flags, flags, STRCHAR);
    if (logfile)
      simSetLogging(sim, logfile, NULL);
    simLog(sim, 2,
           "--------------------------------------------------------------\n");
...
int simdocommands(simptr sim) {
  ...
  if (er)
    return 6;
  if (ccode == CMDstop || ccode == CMDabort)
    return 7;
  return 0;
}
...
void debugcode(simptr sim, const char *prefix) {
  ...
  for (m = 0; m < sim->mols->nl[0]; m++) {
    ...
    if (mptr->serno == 1377166 || mptr->serno == 1374858) {
      printf("%s: time=%g serno=%s", prefix, sim->time,
      ...
      printf(
          " posx side=%s",
          surfface2string(panelside(mptr->posx,
                                    sim->srfss->srflist[4]->panels[PSdisk][0],
                                    3, NULL, 1, 0),
                          string));
      printf(" pos side=%s",
             surfface2string(
                 panelside(mptr->pos, sim->srfss->srflist[4]->panels[PSdisk][0],
                           3, NULL, 1, 0),
                 string));
      printf("\n");
    }
...
int simulatetimestep(simptr sim) {
  ...
  if (sim->srfss) { // deal with surface or wall collisions
    ...
    for (ll = 0; ll < sim->srfss->nmollist;
         ll++) // surface-bound molecule actions
  ...
  er = (*sim->assignmols2boxesfn)(
      sim, 1, 0); // assign to boxes (diffusing molecs., not reborn)
  if (er)
    return 2;
  ...
  if (sim->srfss) {
    for (ll = 0; ll < sim->srfss->nmollist; ll++)
      (*sim->surfacecollisionsfn)(sim, ll, 1);
  } // surfaces again, reborn molecs. only
  else {
    if (sim->mols)
      for (ll = 0; ll < sim->mols->nlist; ll++)
...
void endsimulate(simptr sim, int er) {
  ...
  simLog(sim, 2, "\n");
  if (er == 1)
    simLog(sim, 2, "Simulation complete\n");
  else if (er == 2)
    simLog(
        sim, 5,
    ...
    simLog(sim, 5,
           ...
           "molecules allocated\n Maximum allowed molecule number is %i",
           sim->mols->maxdlimit);
  else if (er == 4)
    simLog(sim, 5,
    ...
    simLog(sim, 5, "Simulation stopped by a runtime command\n");
  else if (er == 8)
    simLog(sim, 5,
           "Simulation terminated during simulation state updating\n  Out of "
           "memory\n");
  else if (er == 9)
    simLog(sim, 5, "Simulation terminated during diffusion\n  Out of memory\n");
  else if (er == 11)
    simLog(sim, 5, "Simulation terminated during filament dynamics\n");
  else if (er == 12)
    simLog(sim, 5, "Simulation terminated during lattice simulation\n");
  else if (er == 13)
    simLog(sim, 5, "Simulation terminated during reaction network expansion\n");
  else
    simLog(sim, 2, "Simulation stopped by user\n");
  simLog(sim, 2, "Current simulation time: %f\n", sim->time);

  eventcount = sim->eventcount;
  if (eventcount[ETwall])
    simLog(sim, 2, "%i wall interactions\n", eventcount[ETwall]);
  if (eventcount[ETsurf])
    simLog(sim, 2, "%i surface interactions\n", eventcount[ETsurf]);
  if (eventcount[ETdesorb])
    simLog(sim, 2, "%i desorptions\n", eventcount[ETdesorb]);
  if (eventcount[ETrxn0])
    simLog(sim, 2, "%i zeroth order reactions\n", eventcount[ETrxn0]);
  if (eventcount[ETrxn1])
    simLog(sim, 2, "%i unimolecular reactions\n", eventcount[ETrxn1]);
  if (eventcount[ETrxn2intra])
    simLog(sim, 2, "%i intrabox bimolecular reactions\n",
           eventcount[ETrxn2intra]);
  if (eventcount[ETrxn2inter])
    simLog(sim, 2, "%i interbox bimolecular reactions\n",
           eventcount[ETrxn2inter]);
  if (eventcount[ETrxn2wrap])
    simLog(sim, 2, "%i wrap-around bimolecular reactions\n",
           eventcount[ETrxn2wrap]);
  if (eventcount[ETrxn2hybrid])
    simLog(sim, 2, "%i bybrid bimolecular reactions\n",
           eventcount[ETrxn2hybrid]);
  if (eventcount[ETimport])
    simLog(sim, 2, "%i imported molecules\n", eventcount[ETimport]);
  if (eventcount[ETexport])
    simLog(sim, 2, "%i exported molecules\n", eventcount[ETexport]);
  ...
  // for testing  examples on remote servers such as Travis CI and github
  // actions.
