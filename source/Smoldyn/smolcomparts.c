...
int
compartsupdateparams_original(simptr sim);
int
compartsupdateparams_volumeSample(simptr sim);
...
// enumerated types
enum CmptLogic
compartstring2cl(char* string);
char*
compartcl2string(enum CmptLogic cls, char* string);
...
compartalloc(void);
void
compartfree(compartptr cmpt);
compartssptr
compartssalloc(compartssptr cmptss, int maxcmpt);

// data structure output

// structure set up
int
compartupdatebox(simptr sim, compartptr cmpt, boxptr bptr, double volfrac);
int
compartsupdateparams(simptr sim);
int
compartsupdatelists(simptr sim);
...
compartstring2cl(char* string)
{
    ...
    if (!strcmp(string, "equal"))
        ans = CLequal;
    else if (!strcmp(string, "equalnot"))
        ans = CLequalnot;
        ...
        ans = CLxor;
    else if (!strcmp(string, "andnot"))
        ans = CLandnot;
    else if (!strcmp(string, "ornot"))
        ans = CLornot;
    else
        ans = CLnone;
    return ans;
...
compartcl2string(enum CmptLogic cls, char* string)
{
    if (cls == CLequal)
        strcpy(string, "equal");
    else if (cls == CLequalnot)
        strcpy(string, "equalnot");
    else if (cls == CLand)
        strcpy(string, "and");
    else if (cls == CLor)
        strcpy(string, "or");
        ...
        strcpy(string, "xor");
    else if (cls == CLandnot)
        strcpy(string, "andnot");
    else if (cls == CLornot)
        strcpy(string, "ornot");
    else
        strcpy(string, "none");
...
unsigned char
getCompartmentID(char* cmptName, VolumeSamplesPtr vSamplesPtr)
{
    int c;
...
/* posincompart */
int
posincompart(simptr sim, double* pos, compartptr cmpt, int useoldpos)
{
    ...
    if (sim->volumeSamplesPtr != NULL) {
        ...
        for (int idim = 0; idim < dim; idim++) {
            ...
            idx = min((volumeSamplePtr->num[idim] - 1),
              idx); // clamp index upper bound to number of volume sample at specific
                    // dimension
            sampleIdxNeighbors[idim][0] = std::max<int>(0, (idx - 1));
            sampleIdxNeighbors[idim][1] = idx;
            sampleIdxNeighbors[idim][2] =
              std::min<int>((volumeSamplePtr->num[idim] - 1), (idx + 1));
        }
        ...
        for (int kk = 0; kk < (dim < 3 ? 1 : numNeighbors); kk++) {
            for (int jj = 0; jj < (dim < 2 ? 1 : numNeighbors); jj++) {
                for (int ii = 0; ii < numNeighbors; ii++) {
                    ...
                    int posInSample =
                      i + j * volumeSamplePtr->num[0] +
                      k * volumeSamplePtr->num[0] * volumeSamplePtr->num[1];
                    unsigned char cmptIDFoundInSample =
                      volumeSamplePtr->volsamples[posInSample];
    ...
    if ((incmpt == 0 && sameResultCount > 0) ||
        sim->volumeSamplesPtr ==
          NULL) // for original smoldyn simulations(without volumeSample)
#endif

    {
        ...
        for (k = 0; k < cmpt->npts && incmpt == 0; k++) {
            ...
            for (s = 0; s < cmpt->nsrf && !pcross; s++) {
                srf = cmpt->surflist[s];
                for (ps = (enum PanelShape)0; ps < PSMAX && !pcross;
        ...
        for (cl = 0; cl < cmpt->ncmptl; cl++) {
            ...
            if (sym == CLequal)
                incmpt = incmptl;
            else if (sym == CLequalnot)
                incmpt = !incmptl;
            else if (sym == CLand)
                incmpt = incmpt && incmptl;
            else if (sym == CLor)
                incmpt = incmpt || incmptl;
            else if (sym == CLxor)
                incmpt = (incmpt != incmptl);
            else if (sym == CLandnot)
                incmpt = incmpt && !incmptl;
            else if (sym == CLornot)
                incmpt = incmpt || !incmptl;
        }
...
/* compartrandpos */
int
compartrandpos(simptr sim, double* pos, compartptr cmpt)
{
    ...
    if (cmpt->nbox) {
        ...
        for (i = 0; i < ptmax && !done; i++) {
            ...
            if (posincompart(sim, pos, cmpt, 0))
                done = 1;
        }
    } else {
        for (i = 0; i < ptmax && !done; i++) {
            for (d = 0; d < dim; d++)
                pos[d] = unirandCCD(sim->wlist[2 * d]->pos, sim->wlist[2 * d + 1]->pos);
            if (posincompart(sim, pos, cmpt, 0))
                done = 1;
        }
    }
...
int
loadHighResVolumeSamples(simptr sim, ParseFilePtr* pfpptr, char* line2)
{ //?? needs to be added to user manual
    ...
    *pfpptr = pfp;
        ...
        *pfpptr = pfp;
        ...
        if (pfpcode == 0)
            ...
            if (!strcmp(word, "Size")) {
                ...
                lineInput >> volumeSamplesPtr->size[0] >> volumeSamplesPtr->size[1] >>
                  volumeSamplesPtr->size[2];
            } else if (!strcmp(word, "Origin")) {
                istringstream lineInput(line2);
                lineInput >> volumeSamplesPtr->origin[0] >> volumeSamplesPtr->origin[1] >>
                  volumeSamplesPtr->origin[2];
            } else if (!strcmp(word, "CompartmentHighResPixelMap")) {
                istringstream lineInput(line2);
                lineInput >> volumeSamplesPtr->nCmptIDPair;
                volumeSamplesPtr->compartmentIDPairPtr =
                  new CompartmentIdentifierPair[volumeSamplesPtr->nCmptIDPair];
                ...
                istringstream lineInput(line2);
                lineInput >> volumeSamplesPtr->num[0] >> volumeSamplesPtr->num[1] >>
                  volumeSamplesPtr->num[2];
    ...
    numVolumeSamples =
      volumeSamplesPtr->num[0] * volumeSamplesPtr->num[1] * volumeSamplesPtr->num[2];
    // cout<<pixelLine<<endl;
    ...
    returnVal = uncompress(
      volumeSamplesPtr->volsamples, &inflated_len, bytes_from_compressed, compressed_len);
    // check if the data can be properly uncompressed, Z_OK should be returned if
    ...
    /*for (unsigned long i = 0; i < inflated_len; i ++) {
            if (volumeSamplesPtr->volsamples[i] == 6) {
                    cout << "volume sample  at " << i<< " is " <<
    ((int)volumeSamplesPtr->volsamples[i])<< endl;
            }
...
#else
    throw "loadHighResVolumeSamples : function unavailable because compile does not "
          "include zlib";
#endif
...
compartssalloc(compartssptr cmptss, int maxcmpt)
{
    ...
    if (maxcmpt > cmptss->maxcmpt) { // allocate new compartment names and compartments
        ...
        for (c = 0; c < maxcmpt; c++)
            newnames[c] = NULL;
        for (c = 0; c < cmptss->maxcmpt; c++)
            newnames[c] = cmptss->cnames[c];
        for (; c < maxcmpt; c++)
            CHECKMEM(newnames[c] = EmptyString());

        CHECKMEM(newcmptlist =
                   (compartptr*)calloc(maxcmpt, sizeof(compartptr))); // compartment list
        for (c = 0; c < maxcmpt; c++)
            newcmptlist[c] = NULL;
        for (c = 0; c < cmptss->maxcmpt; c++)
            newcmptlist[c] = cmptss->cmptlist[c];
        for (; c < maxcmpt; c++) {
...
/* compartssfree */
void
compartssfree(compartssptr cmptss)
{
    int c;
    ...
    if (cmptss->maxcmpt && cmptss->cmptlist)
        for (c = 0; c < cmptss->maxcmpt; c++)
            compartfree(cmptss->cmptlist[c]);
    free(cmptss->cmptlist);
    if (cmptss->maxcmpt && cmptss->cnames)
        for (c = 0; c < cmptss->maxcmpt; c++)
            free(cmptss->cnames[c]);
    free(cmptss->cnames);
    free(cmptss);
    return;
}
...
compartoutput(simptr sim)
{
    ...
    simLog(sim,
      2,
      " Compartments allocated: %i, compartments defined: %i\n",
      cmptss->maxcmpt,
      cmptss->ncmpt);
    for (c = 0; c < cmptss->ncmpt; c++) {
        ...
        for (cl = 0; cl < cmpt->ncmptl; cl++)
            simLog(sim,
              2,
              "   %s %s\n",
              compartcl2string(cmpt->clsym[cl], string),
              cmpt->cmptl[cl]->cname);
...
/* writecomparts */
void
writecomparts(simptr sim, FILE* fptr)
{
    compartssptr cmptss;
    compartptr cmpt;
    int c, s, k, d, cl;
...
/* checkcompartparams */
int
checkcompartparams(simptr sim, int* warnptr)
{
    int error, warn, c;
    compartssptr cmptss;
    compartptr cmpt;
    ...
    if (cmptss->condition != SCok) {
        ...
        simLog(sim,
          7,
          " WARNING: compartment structure %s\n",
    ...
    for (c = 0; c < cmptss->ncmpt; c++) {
        ...
        if (cmpt->volume <= 0) {
            warn++;
            simLog(sim, 5, " WARNING: compartment %s has 0 volume\n", cmpt->cname);
        }
        if (cmpt->nbox == 0) {
            warn++;
            simLog(sim,
              5,
              " WARNING: compartment %s overlaps no virtual boxes\n",
              cmpt->cname);
        }
        if (cmpt->nbox > 0 && cmpt->cumboxvol[cmpt->nbox - 1] != cmpt->volume) {
            ...
            simLog(sim,
              10,
              " BUG: compartment %s box volumes do not add to compartment volume\n",
              cmpt->cname);
        }
    }
    if (warnptr)
...
/* compartsetcondition */
void
compartsetcondition(compartssptr cmptss, enum StructCond cond, int upgrade)
{
    if (!cmptss)
        return;
    if (upgrade == 0 && cmptss->condition > cond)
        cmptss->condition = cond;
    else if (upgrade == 1 && cmptss->condition < cond)
        cmptss->condition = cond;
    else if (upgrade == 2)
        cmptss->condition = cond;
...
int
compartenablecomparts(simptr sim, int maxcmpt)
{
    compartssptr cmptss;
...
/* compartaddcompart */
compartptr
compartaddcompart(simptr sim, const char* cmptname)
{
    int er, c;
    compartssptr cmptss;
    compartptr cmpt;
...
/* compartaddsurf */
int
compartaddsurf(compartptr cmpt, surfaceptr srf)
{
    int s;
...
/* compartaddpoint */
int
compartaddpoint(compartptr cmpt, int dim, double* point)
{
    int d, k;
    double** newpoints;
...
/* compartaddcmptl */
int
compartaddcmptl(compartptr cmpt, compartptr cmptl, enum CmptLogic sym)
{
    int cl;
    compartptr* newcmptl;
...
compartupdatebox(simptr sim, compartptr cmpt, boxptr bptr, double volfrac)
{
    ...
    for (bc = 0; bc < cmpt->nbox && cmpt->boxlist[bc] != bptr; bc++)
        ; // check for box already in cmpt
    if (bc < cmpt->nbox && volfrac == -2)
        return 0; // box is listed and volume ok, so return
    ...
    if (volfrac2 == 0) {
        if (bc == cmpt->nbox)
            return 0; // box not listed and 0 volume, so return
        cmpt->nbox--; // box was listed, so it needs to be removed
        if (cmpt->nbox == 0) {
            cmpt->volume = 0;
            return 2;
        } // last box was removed
    ...
    if (bc < cmpt->nbox) { // box was listed, so just update volume
        if (cmpt->boxfrac[bc] == volfrac2)
            return 0;                 // volume was ok, so return
...
/* compartupdatebox, the volume fraction here is the actual volume fraction for the box
 * inside the compartment*/
/* volfrac is actual volume fraction, for logic compartment the volfrac is assigned -2 */
#ifdef OPTION_VCELL
int
compartupdatebox_volumeSample(simptr sim, compartptr cmpt, boxptr bptr, double volfrac)
{
    /*int ptsmax=100;*/ // number of random points for volume determination
    int bc, max, bc2;
  ...
  char* line2)
{
    ...
    if (!strcmp(word, "name")) { // name, got[0]
        ...
        if (dim == 1)
            itct = strmathsscanf(line2, "%mlg", varnames, varvalues, nvar, &v1[0]);
        else if (dim == 2)
            itct = strmathsscanf(
              line2, "%mlg %mlg", varnames, varvalues, nvar, &v1[0], &v1[1]);
        else
            itct = strmathsscanf(
              line2, "%mlg %mlg %mlg", varnames, varvalues, nvar, &v1[0], &v1[1], &v1[2]);
...
/* loadcompart */
int
loadcompart(simptr sim, ParseFilePtr* pfpptr, char* line2)
{
    ParseFilePtr pfp;
    ...
    } // failed but error has already been reported
...
compartsupdateparams_original(simptr sim)
{
    ...
    for (c = 0; c < cmptss->ncmpt; c++) {
        ...
        for (b = 0; b < boxs->nbox; b++) { // find boxes that are in the compartment
            ...
            for (p = 0; p < bptr->npanel && !inbox; p++) {
                ...
                for (s = 0; s < cmpt->nsrf && !inbox; s++)
                    if (cmpt->surflist[s] == srf)
                        inbox = 1;
            } // a compartment surface is in the box
            if (!inbox && cmpt->ncmptl == 0) {
                boxrandpos(sim, pos, bptr);
                if (posincompart(sim, pos, cmpt, 0))
                    inbox = 2;
            } // compartment contains whole box
            if (inbox) {
        ...
        for (cl = 0; cl < cmpt->ncmptl;
             cl++) { // still finding boxes that are in compartment
            clsym = cmpt->clsym[cl];
...
{ // VCELL
    ...
    for (c = 0; c < cmptss->ncmpt; c++) {
        ...
        for (b = 0; b < boxs->nbox; b++) { // find boxes that are in the compartment
            ...
            for (k = 0; k < volumeSample->num[2]; k++) {
                ...
                double intersectZ = std::min<double>(boxHigh[2], sampleHigh[2]) -
                                    std::max<double>(boxLow[2], sampleLow[2]);
                if (intersectZ <= 0) {
                    continue;
                }
                for (j = 0; j < volumeSample->num[1]; j++) {
                    ...
                    double intersectY = std::min<double>(boxHigh[1], sampleHigh[1]) -
                                        std::max<double>(boxLow[1], sampleLow[1]);
                    if (intersectY <= 0) {
                        continue;
                    }
                    for (i = 0; i < volumeSample->num[0]; i++) {
                        ...
                        double intersectX = std::min<double>(boxHigh[0], sampleHigh[0]) -
                                            std::max<double>(boxLow[0], sampleLow[0]);
                        if (intersectX <= 0) {
                            continue;
                        }
                        // find intersect volume
                        int sampleIdx = i + j * volumeSample->num[0] +
                                        k * volumeSample->num[0] * volumeSample->num[1];
            ...
            double boxVolfrac =
              insideCmptVol / ((boxs->size[0]) * (boxs->size[1]) * (boxs->size[2]));
            if (boxVolfrac <= 1e-8)
                boxVolfrac = 0; // volume fraction is too small, consider it as 0
            if ((boxVolfrac + 1e-8) >= 1)
                boxVolfrac = 1; // if volume fraction is almost 1, consider it as 1
            if (boxVolfrac > 0) {
                int errorCode =
                  compartupdatebox_volumeSample(sim, cmpt, bptr, boxVolfrac);
                if (errorCode == -1)
                    return 1;
            }
        }
    ...
    //				filestr << "box index:" << b << "     " << "volFrac:" << boxVol[i] <<
    //",nneigh=" << bptr->nneigh <<", midneigh="<<bptr->midneigh<<", nwall=" <<bptr->nwall
    //<<",maxpanel="<<bptr->maxpanel<<",npanel="<< bptr->npanel << endl; 				break;
    //			}
    //		}
    //	}
    //	filestr.close();
...
void
comparttranslate(simptr sim, compartptr cmpt, int code, double* translate)
{
            ...
            for (m = 0; m < mols->nl[ll]; m++) {
                ...
                if (mptr->ident && mptr->mstate == MSsoln) {
                    if (posincompart(sim,
                          mptr->pos,
                          cmpt,
                          1)) { // translate molecules within compartment
                        if (code & 4)
                            for (d = 0; d < dim; d++)
                                mptr->pos[d] += translate[d];
                        ...
                        for (d = 0; d < dim; d++)
                            newpos[d] =
                              mptr->pos[d] -
                              translate[d]; // in frame of reference of moving surface
                        for (s = 0; s < cmpt->nsrf; s++) {
                            srf = cmpt->surflist[s];
                            if (srf->action[mptr->ident][MSsoln][PFfront] != SAtrans ||
                                srf->action[mptr->ident][MSsoln][PFback] != SAtrans) {
                                for (ps = (enum PanelShape)0; ps < PSMAX;
                                     ps = (enum PanelShape)(ps + 1))
                                    for (p = 0; p < srf->npanel[ps]; p++) {
                                        pnl = srf->panels[ps][p];
                                        lxp = lineXpanel(mptr->pos,
                                          newpos,
                                          pnl,
                                          dim,
                                          crsspt,
                                          ...
                                          1);
                                        if (lxp &&
                                            srf->action[mptr->ident][MSsoln][face1] !=
                                              SAtrans) {
                                            for (d = 0; d < dim; d++) {
