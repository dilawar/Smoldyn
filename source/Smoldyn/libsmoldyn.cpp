...
#define LCHECK(A, B, C, D)                                                     \
  if (!(A)) {                                                                  \
    smolSetError(B, C, D, sim ? sim->flags : "");                              \
    if (C < ECwarning)                                                         \
      goto failure;                                                            \
  } else                                                                       \
    (void)0
#define LCHECKNT(A, B, C, D)                                                   \
  if (!(A)) {                                                                  \
    smolSetErrorNT(B, C, D);                                                   \
    if (C < ECwarning)                                                         \
      goto failure;                                                            \
  } else                                                                       \
    (void)0
...
PanelShape AllPanels_arr[6] = {PanelShape::PSrect, PanelShape::PStri,
                               PanelShape::PSsph,  PanelShape::PScyl,
                               PanelShape::PShemi, PanelShape::PSdisk};
...
/* smolGetVersion */
extern CSTRING double smolGetVersion(void) { return simversionnumber(); }
...
/* smolSetLogging */
extern CSTRING void smolSetLogging(const char *logfile,
                                   void (*logFunction)(simptr, int,
                                                       const char *, ...)) {
...
/* smolSetError */
extern CSTRING void smolSetError(const char *errorfunction,
                                 enum ErrorCode errorcode,
                                 const char *errorstring, const char *flags) {
  ...
  //??	if(LibThrowThreshold<severity) throw;	//?? This is disabled for now
  //because I can't link libsmoldyn statically if not
    ...
    if (Liberrorcode == ECnotify) {
      ...
      if (!sflag)
        fprintf(stderr, "Libsmoldyn '%s' error in %s: %s\n",
                smolErrorCodeToString(Liberrorcode, string), Liberrorfunction,
                Liberrorstring);
    }
  }
  return;
...
/* smolSetErrorNT */
extern CSTRING void smolSetErrorNT(const char *errorfunction,
                                   enum ErrorCode errorcode,
...
/* smolGetError */
extern CSTRING enum ErrorCode smolGetError(char *errorfunction,
                                           char *errorstring, int clearerror) {
  enum ErrorCode erc;

  erc = Liberrorcode;
  if (errorfunction)
    strcpy(errorfunction, Liberrorfunction);
  if (errorstring)
    strcpy(errorstring, Liberrorstring);
  if (clearerror)
    smolClearError();
  return erc;
}
...
extern CSTRING char *smolErrorCodeToString(enum ErrorCode erc, char *string) {
  if (erc == ECok)
    strcpy(string, "ok");
  else if (erc == ECnotify)
    strcpy(string, "notify");
  else if (erc == ECwarning)
    strcpy(string, "warning");
  else if (erc == ECnonexist)
    strcpy(string, "nonexistant");
  else if (erc == ECall)
    strcpy(string, "all");
  else if (erc == ECmissing)
    strcpy(string, "missing");
  else if (erc == ECbounds)
    strcpy(string, "bounds");
  else if (erc == ECsyntax)
    strcpy(string, "syntax");
  else if (erc == ECerror)
    strcpy(string, "error");
  else if (erc == ECmemory)
    strcpy(string, "memory");
  else if (erc == ECbug)
    strcpy(string, "Smoldyn bug");
  else if (erc == ECsame)
    strcpy(string, "same as before");
  else
...
/* smolNewSim */
extern CSTRING simptr smolNewSim(int dim, double *lowbounds,
                                 double *highbounds) {
  ...
  for (d = 0; d < dim; d++)
    LCHECK(lowbounds[d] < highbounds[d], funcname, ECbounds,
           "lowbounds must be < highbounds");
...
extern CSTRING enum ErrorCode smolRunTimeStep(simptr sim) {
  ...
  LCHECK(er != 3, funcname, ECerror,
         "Simulation terminated during order 0 reaction\n");
  LCHECK(er != 4, funcname, ECerror,
         "Simulation terminated during order 1 reaction\n");
  LCHECK(er != 5, funcname, ECerror,
         "Simulation terminated during order 2 reaction\n");
  LCHECK(er != 6, funcname, ECerror,
         "Simulation terminated during molecule sorting\n  Out of memory");
  LCHECK(er != 7, funcname, ECnotify,
         "Simulation stopped by a runtime command");
  LCHECK(er != 8, funcname, ECerror,
         "Simulation terminated during simulation state updating\n  Out of "
         "memory");
  LCHECK(er != 9, funcname, ECerror,
         "Simulation terminated during diffusion\n  Out of memory");
  LCHECK(er != 11, funcname, ECerror,
         "Simulation terminated during filament dynamics");
  LCHECK(er != 12, funcname, ECerror,
         "Simulation terminated during lattice simulation");
  LCHECK(er != 13, funcname, ECerror,
         "Simulation terminated during reaction network expansion");
  return Libwarncode;
failure:
  return Liberrorcode;
}
...
extern CSTRING simptr smolPrepareSimFromFile(const char *filepath,
                                             const char *filename,
                                             const char *flags) {
...
/* smolLoadSimFromFile */
extern CSTRING enum ErrorCode smolLoadSimFromFile(const char *filepath,
                                                  const char *filename,
                                                  simptr *simpointer,
                                                  const char *flags) {
...
extern CSTRING enum ErrorCode
smolReadConfigString(simptr sim, const char *statement, char *parameters) {
...
/* smolSetSimTimes */
extern CSTRING enum ErrorCode smolSetSimTimes(simptr sim, double timestart,
                                              double timestop,
                                              double timestep) {
...
/* smolSetPartitions */
extern CSTRING enum ErrorCode smolSetPartitions(simptr sim, const char *method,
                                                double value) {
...
extern CSTRING enum ErrorCode smolSetGraphicsParams(simptr sim,
                                                    const char *method,
...
/* smolSetTiffParams */
extern CSTRING enum ErrorCode smolSetTiffParams(simptr sim, int timesteps,
                                                const char *tiffname,
                                                int lowcount, int highcount) {
...
/* smolSetLightParams */
extern CSTRING enum ErrorCode
smolSetLightParams(simptr sim, int lightindex, double *ambient, double *diffuse,
                   double *specular, double *position) {
  const char *funcname = "smolSetLightParams";
  int c, er;

  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(lightindex >= -1 && lightindex < MAXLIGHTS, funcname, ECbounds,
         "lightindex out of bounds");
  LCHECK(lightindex >= 0 || (!diffuse && !specular && !position), funcname,
         ECsyntax, "can only set ambient for global light");

  if (ambient) {
    for (c = 0; c < 4; c++)
      LCHECK(ambient[c] >= 0 && ambient[c] <= 1, funcname, ECbounds,
             "ambient light value out of bounds");
    er = graphicssetlight(sim, NULL, lightindex, LPambient, ambient);
    LCHECK(!er, funcname, ECmemory, "out of memory enabling graphics");
  }
  if (diffuse) {
    for (c = 0; c < 4; c++)
      LCHECK(diffuse[c] >= 0 && diffuse[c] <= 1, funcname, ECbounds,
             "diffuse light value out of bounds");
    er = graphicssetlight(sim, NULL, lightindex, LPdiffuse, diffuse);
    LCHECK(!er, funcname, ECmemory, "out of memory enabling graphics");
  }
  if (specular) {
    for (c = 0; c < 4; c++)
      LCHECK(specular[c] >= 0 && specular[c] <= 1, funcname, ECbounds,
             "specular light value out of bounds");
...
extern CSTRING enum ErrorCode smolSetBackgroundStyle(simptr sim,
                                                     double *color) {
...
/* smolSetFrameStyle */
extern CSTRING enum ErrorCode smolSetFrameStyle(simptr sim, double thickness,
                                                double *color) {
...
/* smolSetGridStyle */
extern CSTRING enum ErrorCode smolSetGridStyle(simptr sim, double thickness,
                                               double *color) {
...
extern CSTRING enum ErrorCode smolAddTextDisplay(simptr sim, char *item) {
  ...
  LCHECK(er != 2, funcname, ECsyntax,
         "listed item is not recognized or not supported");
...
/* smolAddOutputFile */
extern CSTRING enum ErrorCode smolAddOutputFile(simptr sim, char *filename,
                                                int suffix, int append) {
  const char *funcname = "smolAddOutputFile";
  int er;

  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(filename, funcname, ECmissing, "missing filename");
  LCHECK(!strchr(filename, ' '), funcname, ECwarning,
         "only first word of filename is used");
...
extern CSTRING enum ErrorCode smolAddOutputData(simptr sim, char *dataname) {
  ...
  LCHECK(!strchr(dataname, ' '), funcname, ECwarning,
         "only first word of dataname is used");
...
enum ErrorCode smolOpenOutputFiles(simptr sim, int overwrite = 0) {
  ...
  LCHECK(!err, funcname, ECerror,
         "Unable (or forbidden) to open output files for writing");
...
/* smolAddCommand */
extern CSTRING enum ErrorCode smolAddCommand(simptr sim, char type, double on,
                                             double off, double step,
                                             double multiplier,
  ...
  LCHECK(sim, funcname, ECmissing, "missing sim");
  er =
      scmdaddcommand(sim->cmds, type, on, off, step, multiplier, commandstring);
...
extern CSTRING enum ErrorCode smolAddCommandFromString(simptr sim,
                                                       char *string) {
...
/* smolGetOutputData */
extern CSTRING enum ErrorCode smolGetOutputData(simptr sim, char *dataname,
                                                int *nrow, int *ncol,
  ...
  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(dataname, funcname, ECmissing, "missing dataname");
  LCHECK(nrow && ncol && array, funcname, ECmissing,
         "missing pointer for returned data");
  LCHECK(sim->cmds && sim->cmds->ndata > 0, funcname, ECerror,
         "no data files in the sim");
...
extern CSTRING enum ErrorCode smolRunCommand(simptr sim,
                                             const char *commandstring) {
...
/* smolAddSpecies */
extern CSTRING enum ErrorCode smolAddSpecies(simptr sim, const char *species,
                                             const char *mollist) {
  ...
  if (mollist && mollist[0] != '\0') {
    ...
    LCHECK(sim->mols->listtype[ll] == MLTsystem, funcname, ECsyntax,
           "mollist is not a system list");
  } else
    ll = -1;
  i = moladdspecies(sim, species);
  LCHECK(i != -1, funcname, ECbug, "out of memory");
  LCHECK(i != -2, funcname, ECbug, "add species bug");
  LCHECK(i != -3, funcname, ECbug,
         "more species are entered than are automatically allocated");
  LCHECK(i != -4, funcname, ECsyntax,
         "'empty' is not a permitted species name");
  LCHECK(i != -5, funcname, ECwarning,
         "this species has already been declared");
  LCHECK(i != -6, funcname, ECsyntax,
         "'?' and '*' are not allowed in species names");
  if (mollist && mollist[0] != '\0')
...
/* smolGetSpeciesName */
extern CSTRING void smolGetSpeciesName(simptr sim, int speciesindex,
                                       char *species) {
  ...
  LCHECK(speciesindex < sim->mols->nspecies, funcname, ECnonexist,
...
/* smolGetSpecies */
extern CSTRING int smolGetSpecies(simptr sim, int speciesindex,
                                  char *speciesname, double *difc,
                                  double **color, double *displaysize,
                                  char **listname) {
  ...
  if (speciesindex >= 0) {
    LCHECK(speciesindex < sim->mols->nspecies, funcname, ECnonexist,
           "species doesn't exist");
    if (speciesname)
      strcpy(speciesname, sim->mols->spname[speciesindex]);
  } else {
  ...
  for (ms = (enum MolecState)0; ms < (enum MolecState)MSMAX;
       ms = (enum MolecState)(ms + 1)) {
    if (difc)
      difc[ms] = sim->mols->difc[i][ms];
    if (color) {
    ...
    if (displaysize)
      displaysize[ms] = sim->mols->display[i][ms];
    if (listname)
      strcpy(listname[ms], sim->mols->listname[sim->mols->listlookup[i][ms]]);
  }
  return i;
failure:
  return (int)Liberrorcode;
...
extern CSTRING enum ErrorCode smolAddMolList(simptr sim, const char *mollist) {
  ...
  LCHECK(ll != -2, funcname, ECwarning,
         "molecule list name has already been used");
...
extern CSTRING int smolGetMolListIndexNT(simptr sim, const char *mollist) {
  ...
  LCHECKNT(strcmp(mollist, "all"), funcname, ECall,
           "molecule list cannot be 'all'");
  ll = stringfind(sim->mols->listname, sim->mols->nlist, mollist);
...
/* smolGetMolListName */
extern CSTRING char *smolGetMolListName(simptr sim, int mollistindex,
                                        char *mollist) {
  ...
  LCHECK(sim->mols, funcname, ECnonexist, "no molecule lists defined");
  LCHECK(mollistindex < sim->mols->nlist, funcname, ECnonexist,
         "molecule list doesn't exist");
  LCHECK(mollist, funcname, ECmissing, "missing mollist");
...
/* smolSetMolList */
extern CSTRING enum ErrorCode smolSetMolList(simptr sim, const char *species,
                                             enum MolecState state,
                                             const char *mollist) {
  ...
  ll = smolGetMolListIndexNT(sim, mollist);
  LCHECK(ll >= 0, funcname, ECsame, NULL);
  LCHECK(sim->mols->listtype[ll] == MLTsystem, funcname, ECerror,
         "list is not a system list");
...
/* smolSetMaxMolecules */
extern CSTRING enum ErrorCode smolSetMaxMolecules(simptr sim,
                                                  int maxmolecules) {
...
extern CSTRING enum ErrorCode
smolAddSolutionMolecules(simptr sim, const char *species, int number,
  ...
  if (!lowposition) {
    for (d = 0; d < sim->dim; d++)
      lowpos[d] = sim->wlist[d * 2]->pos;
    low = lowpos;
  } else
    low = lowposition;
  if (!highposition) {
    for (d = 0; d < sim->dim; d++)
      highpos[d] = sim->wlist[d * 2 + 1]->pos;
    high = highpos;
  } else
    high = highposition;
...
extern CSTRING enum ErrorCode
smolAddCompartmentMolecules(simptr sim, const char *species, int number,
  ...
  LCHECK(er != 2, funcname, ECerror,
         "compartment volume is zero or nearly zero");
...
smolAddSurfaceMolecules(simptr sim, const char *species, enum MolecState state,
                        int number, const char *surface,
                        enum PanelShape panelshape, const char *panel,
                        double *position) {
  ...
  LCHECK((panelshape >= 0 && panelshape < PSMAX) || panelshape == PSall,
         funcname, ECnonexist, "invalid panelshape");
  pnl = NULL;
  ...
  if (p == (int)ECall) {
    ...
    LCHECK(p >= 0, funcname, ECsame, NULL);

  if (p >= 0) {
    LCHECK(s >= 0, funcname, ECsyntax, "needs to be specific surface");
    LCHECK(panelshape != PSall, funcname, ECsyntax,
           "needs to be specific panelshape");
    pnl = sim->srfss->srflist[s]->panels[panelshape][p];
  } else {
    LCHECK(!position, funcname, ECsyntax,
           "a panel must be specified if position is entered");
  }
  er = addsurfmol(sim, number, i, state, position, pnl, s, panelshape, NULL);
  LCHECK(er != 1, funcname, ECmemory,
         "unable to allocate temporary storage space");
...
extern CSTRING int smolGetMoleculeCount(simptr sim, const char *species,
                                        enum MolecState state) {
...
extern "C" enum ErrorCode smolSetMoleculeColor(simptr sim, const char *species,
                                               enum MolecState state,
                                               double *color) {
...
extern "C" enum ErrorCode smolSetMoleculeSize(simptr sim, const char *species,
                                              enum MolecState state,
                                              double size) {
...
extern CSTRING enum ErrorCode smolSetMoleculeStyle(simptr sim,
                                                   const char *species,
                                                   enum MolecState state,
...
/* smolSetBoundaryType */
extern CSTRING enum ErrorCode smolSetBoundaryType(simptr sim, int dimension,
                                                  int highside, char type) {
  const char *funcname = "smolSetBoundaryType";
  int er;

  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(dimension < sim->dim, funcname, ECbounds,
         "dimension cannot exceed system dimensionality");
  LCHECK(highside <= 1, funcname, ECbounds, "highside must be -1, 0, or 1");
  LCHECK(type == 'r' || type == 'p' || type == 'a' || type == 't', funcname,
         ECsyntax, "invalid type");
...
extern CSTRING enum ErrorCode smolAddSurface(simptr sim, const char *surface) {
  ...
  if (s == (int)ECnonexist)
    ...
    LCHECK(0, funcname, ECsame, NULL);
  else
    LCHECK(0, funcname, ECerror, "surface is already in system");
...
extern CSTRING int smolGetSurfaceIndex(simptr sim, const char *surface) {
  ...
  LCHECK(sim->srfss && sim->srfss->nsrf, funcname, ECnonexist,
         "no surfaces defined");
...
extern CSTRING int smolGetSurfaceIndexNT(simptr sim, const char *surface) {
  ...
  LCHECKNT(sim->srfss && sim->srfss->nsrf, funcname, ECnonexist,
           "no surfaces defined");
...
/* smolGetSurfaceName */
extern CSTRING char *smolGetSurfaceName(simptr sim, int surfaceindex,
                                        char *surface) {
  ...
  LCHECK(sim->srfss && sim->srfss->nsrf, funcname, ECnonexist,
         "no surfaces defined");
  LCHECK(surfaceindex < sim->srfss->nsrf, funcname, ECnonexist,
...
extern CSTRING enum ErrorCode
smolSetSurfaceAction(simptr sim, const char *surface, enum PanelFace face,
                     const char *species, enum MolecState state,
  ...
  if (s == (int)ECall) {
    ...
    LCHECK(s >= 0, funcname, ECsame, NULL);
  LCHECK(face == PFfront || face == PFback || face == PFboth, funcname,
         ECbounds, "invalid face");
  i = smolGetSpeciesIndexNT(sim, species);
  if (i == (int)ECall) {
    smolClearError();
    i = -5;
...
/* smolSetSurfaceRate */
extern CSTRING enum ErrorCode
smolSetSurfaceRate(simptr sim, const char *surface, const char *species,
                   enum MolecState state, enum MolecState state1,
  ...
  LCHECK(state == MSsoln || state1 == MSsoln || state1 == MSbsoln ||
             state1 == state,
         funcname, ECsyntax, "nonsensical state combination");
  LCHECK(state2 >= 0 && state2 < MSMAX1, funcname, ECbounds, "invalid state2");
  LCHECK(state1 != state2, funcname, ECsyntax,
         "cannot set rate for state1 = state2");
  ...
  LCHECK(!(isinternal && rate > 1), funcname, ECbounds,
         "internal rate needs to be <= 1");

  if (s >= 0) {
    srf = sim->srfss->srflist[s];
    er = surfsetrate(srf, i, NULL, state, state1, state2, i2, rate,
                     isinternal ? 2 : 1);
...
/* smolAddPanel */
extern CSTRING enum ErrorCode
smolAddPanel(simptr sim, const char *surface, enum PanelShape panelshape,
  ...
  s = smolGetSurfaceIndexNT(sim, surface);
  LCHECK(s >= 0, funcname, ECsame, NULL);
  LCHECK(panelshape >= 0 && panelshape < PSMAX, funcname, ECnonexist,
         "invalid panel shape");
  ...
  LCHECK(er != 4, funcname, ECbounds,
         "drawing slices and stacks need to be positive");
  LCHECK(er != 5, funcname, ECbounds,
         "cylinder ends cannot be at the same location");
  LCHECK(er != 6, funcname, ECbounds,
         "hemisphere outward pointing vector has zero length");
...
/* smolGetPanelIndex */
extern CSTRING int smolGetPanelIndex(simptr sim, const char *surface,
                                     enum PanelShape *panelshapeptr,
                                     const char *panel) {
...
/* smolGetPanelIndexNT */
extern CSTRING int smolGetPanelIndexNT(simptr sim, const char *surface,
                                       enum PanelShape *panelshapeptr,
                                       const char *panel) {
...
/* smolGetPanelName */
extern CSTRING char *smolGetPanelName(simptr sim, const char *surface,
  ...
  LCHECK(panel, funcname, ECmissing, "missing panel name");
  srf = sim->srfss->srflist[s];
  LCHECK(panelindex < srf->npanel[panelshape], funcname, ECnonexist,
         "no panel exists with this number");
...
/* smolSetPanelJump */
extern CSTRING enum ErrorCode
smolSetPanelJump(simptr sim, const char *surface, const char *panel1,
                 enum PanelFace face1, const char *panel2, enum PanelFace face2,
                 int isbidirectional) {
  ...
  LCHECK(ps1 == ps2, funcname, ECerror,
         "origin and destination jump panels need to have the same shape");
  LCHECK(p1 != p2, funcname, ECerror,
         "origin and destination jump panels cannot be the same panel");
  LCHECK(face1 == PFfront || face1 == PFback, funcname, ECsyntax,
         "jumping panel face has to be either front or back");
  LCHECK(face2 == PFfront || face2 == PFback, funcname, ECsyntax,
         "jumping panel face has to be either front or back");
  LCHECK(isbidirectional == 0 || isbidirectional == 1, funcname, ECsyntax,
         "bidirectional code has to be 0 or 1");
...
extern CSTRING enum ErrorCode
smolAddSurfaceUnboundedEmitter(simptr sim, const char *surface,
                               enum PanelFace face, const char *species,
  ...
  s = smolGetSurfaceIndexNT(sim, surface);
  LCHECK(s >= 0, funcname, ECsame, NULL);
  LCHECK(face == PFfront || face == PFback, funcname, ECsyntax,
         "jumping panel face has to be either front or back");
  i = smolGetSpeciesIndexNT(sim, species);
  LCHECK(i > 0, funcname, ECsame, NULL);
...
extern CSTRING enum ErrorCode
smolAddPanelNeighbor(simptr sim, const char *surface1, const char *panel1,
                     const char *surface2, const char *panel2, int reciprocal) {
...
/* smolSetSurfaceStyle */
extern CSTRING enum ErrorCode
smolSetSurfaceStyle(simptr sim, const char *surface, enum PanelFace face,
                    enum DrawMode mode, double thickness, double *color,
  ...
  if (mode != DMnone) {
    LCHECK(mode >= 0 && mode < DMnone, funcname, ECsyntax,
           "mode not recognized");
  ...
  if (stipplefactor >= 0) {
    LCHECK(stipplefactor > 0, funcname, ECbounds,
           "stipplefactor needs to be >0");
    er = surfsetstipple(srf, stipplefactor, -1);
    LCHECK(!er, funcname, ECbug, "BUG: error in surfsetstipple");
  }
  if (stipplepattern >= 0) {
    LCHECK(stipplepattern <= 0xFFFF, funcname, ECbounds,
           "stipplepattern needs to be between 0 and 0xFFFF");
...
/* smolAddCompartment */
extern CSTRING enum ErrorCode smolAddCompartment(simptr sim,
                                                 const char *compartment) {
  ...
  if (c == (int)ECnonexist)
    ...
    LCHECK(0, funcname, ECerror, "compartment is already in system");
...
extern CSTRING int smolGetCompartmentIndex(simptr sim,
                                           const char *compartment) {
  ...
  LCHECK(sim->cmptss && sim->cmptss->ncmpt, funcname, ECnonexist,
         "no compartments defined");
  LCHECK(strcmp(compartment, "all"), funcname, ECall,
         "compartment cannot be 'all'");
...
extern CSTRING int smolGetCompartmentIndexNT(simptr sim,
                                             const char *compartment) {
  ...
  LCHECKNT(sim->cmptss && sim->cmptss->ncmpt, funcname, ECnonexist,
           "no compartments defined");
  LCHECKNT(strcmp(compartment, "all"), funcname, ECall,
           "compartment cannot be 'all'");
  c = stringfind(sim->cmptss->cnames, sim->cmptss->ncmpt, compartment);
...
extern CSTRING char *smolGetCompartmentName(simptr sim, int compartmentindex,
                                            char *compartment) {
  const char *funcname = "smolGetCompartmentName";

  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(compartmentindex >= 0, funcname, ECbounds,
         "invalid compartment index");
  LCHECK(compartment, funcname, ECmissing, "missing compartment string");
  LCHECK(sim->cmptss && sim->cmptss->ncmpt, funcname, ECnonexist,
         "no compartments defined");
  LCHECK(compartmentindex < sim->cmptss->ncmpt, funcname, ECnonexist,
...
extern CSTRING enum ErrorCode smolAddCompartmentSurface(simptr sim,
                                                        const char *compartment,
                                                        const char *surface) {
...
extern CSTRING enum ErrorCode
smolAddCompartmentPoint(simptr sim, const char *compartment, double *point) {
...
extern CSTRING enum ErrorCode
smolAddCompartmentLogic(simptr sim, const char *compartment,
...
smolAddReaction(simptr sim, const char *reaction, const char *reactant1,
                enum MolecState rstate1, const char *reactant2,
                enum MolecState rstate2, int nproduct,
                const char **productspecies, enum MolecState *productstates,
                double rate) {
  ...
  if (reactant1 && reactant1[0] != '\0') {
    ...
    LCHECK(rstate1 >= 0 && rstate1 <= MSMAX, funcname, ECbounds,
           "invalid rstate1");
  ...
  if (reactant2 && reactant2[0] != '\0') {
    ...
    LCHECK(rctident[order] > 0, funcname, ECsame, NULL);
    LCHECK(rstate2 >= 0 && rstate2 <= MSMAX, funcname, ECbounds,
           "invalid rstate2");
  ...
  if (nproduct) {
    ...
    for (prd = 0; prd < nproduct; prd++) {
      ...
      LCHECK(productstates[prd] >= MSsoln && productstates[prd] <= MSMAX,
             funcname, ECsyntax, "invalid product state");
    }
  }
  rxn = RxnAddReaction(sim, reaction, order, rctident, rctstate, nproduct,
                       prdident, productstates, NULL, NULL);
...
extern CSTRING int smolGetReactionIndex(simptr sim, int *orderptr,
                                        const char *reaction) {
  ...
  if (orderptr && *orderptr >= 0 && *orderptr < MAXORDER) {
    order = *orderptr;
    LCHECK(sim->rxnss[order] && sim->rxnss[order]->totrxn, funcname, ECnonexist,
           "no reactions defined of this order");
...
extern CSTRING int smolGetReactionIndexNT(simptr sim, int *orderptr,
                                          const char *reaction) {
  ...
  LCHECKNT(strcmp(reaction, "all"), funcname, ECall,
           "reaction cannot be 'all'");
  if (orderptr && *orderptr >= 0 && *orderptr < MAXORDER) {
    order = *orderptr;
    LCHECKNT(sim->rxnss[order] && sim->rxnss[order]->totrxn, funcname,
             ECnonexist, "no reactions defined of this order");
    r = stringfind(sim->rxnss[order]->rname, sim->rxnss[order]->totrxn,
                   reaction);
...
/* smolGetReactionName */
extern CSTRING char *smolGetReactionName(simptr sim, int order,
                                         int reactionindex, char *reaction) {
  const char *funcname = "smolGetReactionName";

  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(order >= 0 && order < MAXORDER, funcname, ECbounds,
         "invalid reaction order");
  LCHECK(reactionindex >= 0, funcname, ECbounds, "invalid reaction name");
  LCHECK(reaction, funcname, ECmissing, "missing reaction");
  LCHECK(sim->rxnss[order] && sim->rxnss[order]->totrxn, funcname, ECnonexist,
         "no reactions defined of this order");
  LCHECK(reactionindex < sim->rxnss[order]->totrxn, funcname, ECnonexist,
...
/* smolSetReactionRate */
extern CSTRING enum ErrorCode smolSetReactionRate(simptr sim,
                                                  const char *reaction,
                                                  double rate, int isinternal) {
...
extern CSTRING enum ErrorCode smolSetReactionRegion(simptr sim,
                                                    const char *reaction,
                                                    const char *compartment,
                                                    const char *surface) {
...
extern CSTRING enum ErrorCode
smolSetReactionProducts(simptr sim, const char *reaction, enum RevParam method,
                        double parameter, const char *product,
                        double *position) {
  ...
  if (product && strlen(product) > 0) {
    ...
    for (prd = 0; prd < rxn->nprod && !done; prd++)
      if (rxn->prdident[prd] == i)
        done = 1;
    prd--;
    LCHECK(done, funcname, ECerror,
           "listed product is not a product of the listed reaction");
  }

  er = RxnSetRevparam(sim, rxn, method, parameter, prd, position, sim->dim);
  LCHECK(er != 1, funcname, ECwarning,
         "reaction product parameter was set before");
  LCHECK(er != 2, funcname, ECbounds,
         "reaction product parameter out of bounds");
...
/* smolAddPort */
extern CSTRING enum ErrorCode smolAddPort(simptr sim, const char *port,
                                          const char *surface,
                                          enum PanelFace face) {
  ...
  if (s == (int)ECmissing)
    ...
    LCHECK(s >= 0, funcname, ECsame, NULL);
  LCHECK(face == PFfront || face == PFback || face == PFnone, funcname,
         ECsyntax, "invalid face");
...
extern CSTRING int smolGetPortIndex(simptr sim, const char *port) {
  ...
  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(port, funcname, ECmissing, "missing port");
  LCHECK(sim->portss && sim->portss->nport, funcname, ECnonexist,
         "no ports defined");
...
extern CSTRING int smolGetPortIndexNT(simptr sim, const char *port) {
  ...
  LCHECKNT(sim->portss && sim->portss->nport, funcname, ECnonexist,
           "no ports defined");
...
extern CSTRING char *smolGetPortName(simptr sim, int portindex, char *port) {
  ...
  LCHECK(sim->portss && sim->portss->nport, funcname, ECnonexist,
         "no ports defined");
  LCHECK(portindex < sim->portss->nport, funcname, ECnonexist,
         "port does not exist");
...
extern CSTRING enum ErrorCode smolAddPortMolecules(simptr sim, const char *port,
                                                   int nmolec,
                                                   const char *species,
                                                   double **positions) {
...
extern CSTRING int smolGetPortMolecules(simptr sim, const char *port,
                                        const char *species,
...
/* smolAddLattice */
extern CSTRING enum ErrorCode
smolAddLattice(simptr sim, const char *lattice, const double *min,
               const double *max, const double *dx, const char *btype) {
  ...
  p = stringfind(sim->latticess->latticenames, sim->latticess->nlattice,
                 lattice);
  LCHECK(p < 0, funcname, ECsame, "lattice name already exists");
  er = latticeaddlattice(sim, &simlattice, lattice, min, max, dx, btype,
                         LATTICEnsv);
...
extern CSTRING int smolGetLatticeIndex(simptr sim, const char *lattice) {
  ...
  LCHECK(sim, funcname, ECmissing, "missing sim");
  LCHECK(lattice, funcname, ECmissing, "missing lattice");
  LCHECK(sim->latticess && sim->latticess->nlattice, funcname, ECnonexist,
         "no lattices defined");
...
extern CSTRING int smolGetLatticeIndexNT(simptr sim, const char *lattice) {
  ...
  LCHECKNT(sim->latticess && sim->latticess->nlattice, funcname, ECnonexist,
           "no lattices defined");
...
/* smolGetLatticeName */
extern CSTRING char *smolGetLatticeName(simptr sim, int latticeindex,
                                        char *lattice) {
  ...
  LCHECK(sim->latticess && sim->latticess->nlattice, funcname, ECnonexist,
         "no lattices defined");
  LCHECK(latticeindex < sim->latticess->nlattice, funcname, ECnonexist,
...
extern CSTRING enum ErrorCode
smolAddLatticeMolecules(simptr sim, const char *lattice, const char *species,
...
/* smolAddLatticePort */
extern CSTRING enum ErrorCode
smolAddLatticePort(simptr sim, const char *lattice, const char *port) {
...
extern CSTRING enum ErrorCode
smolAddLatticeSpecies(simptr sim, const char *lattice, const char *species) {
...
extern CSTRING enum ErrorCode smolAddLatticeReaction(simptr sim,
                                                     const char *lattice,
                                                     const char *reaction,
                                                     const int move) {
